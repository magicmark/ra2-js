...
import {
  nativeMapCoordinates,
  nativeDisplayPosition,
  type NativeMap,
  type NativeCell,
} from '../src/game/maps/nativeMap.ts';
...
import {
  ...
  applyShorelines,
  SHORE_CORNERS,
} from '../src/game/maps/terrainTopology.ts';
...
const hash = (x: number, y: number, seed = 0) => {
  let n = Math.imul(x + seed * 31, 374761393) ^ Math.imul(y + seed * 19, 668265263);
...
const native = (u: number, v: number) => ({
  x: Math.round(u + v + 1),
  y: Math.round(v + SIZE - u),
});
...
export function authorMap(entry: MapCatalogEntry, seed: number): NativeMap {
  ...
  const cells: NativeCell[] = nativeMapCoordinates(SIZE, SIZE).map((p) => ({
    ...p,
    tileIndex: 0,
    subTile: 0,
    height: 0,
  ...
  const sand = entry.theater === 'SNOW' ? 713 : 493,
    rough = entry.theater === 'SNOW' ? 696 : 418,
    pavement = entry.theater === 'SNOW' ? 734 : 534;
  ...
  for (const c of cells) {
    ...
    switch (seed) {
      case 0: // Parallel long lakes with a broad east-west saddle and outside routes.
        tile = Math.sin(u * 0.13 + Math.cos(v * 0.14)) + Math.cos(v * 0.18) > 0.3 ? 131 : 0;
      ...
      case 1: // Asymmetric clustered basins, none forming a wall across the map.
        tile = Math.cos(u * 0.13) + Math.sin(v * 0.12) > 0.6 ? sand : 0;
        wet =
          ellipse(u, v, 43, 31, 9, 9) < 1 ||
          ellipse(u, v, 60, 56, 10, 11) < 1 ||
          ellipse(u, v, 34, 69, 6, 7) < 1;
        break;
      ...
      case 3: // Broad wooded belts around a real native-axis three-cell road cross.
        tile = Math.abs(Math.sin((u + v) * 0.065) + Math.cos((u - v) * 0.08)) > 0.75 ? 131 : 0;
      ...
      case 4: // Harbor fingers connected through broad paved inland districts.
        ...
        break;
      case 5: {
        // Flat sand mesa; no fake cliff elevations.
      ...
      case 6: // Two transverse lakes, center causeway 20 display columns wide.
        tile =
          Math.abs(v - 47) < 8 ? rough : Math.sin(u * 0.1) + Math.cos(v * 0.13) > 0.65 ? sand : 0;
  ...
  for (const start of starts)
    for (const c of cells) {
      ...
      if (base || ore1 || ore2) {
        water.delete(c.x + 512 * c.y);

        if (base) c.tileIndex = seed === 2 || seed === 5 ? sand : 0;
      }
    }
  ...
  if (hasRoads)
    for (const c of cells) {
      ...
      if (
        !(horizontal || vertical) ||
        ['water', 'shore'].includes(nativeTileSpec(map.theater, c.tileIndex)!.kind)
      )
        continue;
      ...
      if (
        starts.some(
          (s) =>
            (horizontal && Math.abs(c.x - s.x) <= 10 && s.y + 10 >= 94 && s.y - 10 <= 96) ||
            (vertical && Math.abs(c.y - s.y) <= 10 && s.x + 10 >= 94 && s.x - 10 <= 96),
        )
      )
        continue;
      c.tileIndex = horizontal && vertical ? 295 : horizontal ? 293 : 294;
      c.subTile =
        horizontal && vertical ? (c.y - 94) * 3 + c.x - 94 : horizontal ? c.y - 94 : c.x - 94;
    }
  ...
  const clear = (x: number, y: number, radius: number, tile = 0) => {
    for (let dy = -radius; dy <= radius; dy++)
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        const columns = road && c.tileIndex !== 293 ? 3 : 1,
          rows = road && c.tileIndex !== 294 ? 3 : 1;

        const left = c.x - (road ? c.subTile % columns : 0),
          top = c.y - (road ? Math.floor(c.subTile / columns) : 0);
    ...
    for (const [cx, cy, radius] of [
      ...
      for (let y = cy - radius; y <= cy + radius; y++)
        for (let x = cx - radius; x <= cx + radius; x++) {
          ...
          if (c && Math.hypot(x - cx, y - cy) <= radius) {
            c.overlay = 102 + Math.floor(hash(x - cx, y - cy, 7) * 6);
            c.overlayData = 11;
          }
        }

      const mine = get(cx, cy)!;
      mine.overlay = 255;
      mine.overlayData = 0;
  ...
  const oilLocations = starts.map((s) => ({
    x: Math.round(s.x * 0.55 + 96 * 0.45),
    y: Math.round(s.y * 0.55 + 96 * 0.45),
  }));
  ...
  for (const [type, positions] of [
    ['CAOILD', oilLocations],
    ['CAAIRP', airportLocations],
  ] as const)
    for (const target of positions) {
      const choices = cells
        .filter((c) => {
          if (
            Math.hypot(c.x - target.x, c.y - target.y) > 24 ||
            map.structures.some((s) => Math.hypot(c.x - s.x, c.y - s.y) < 11)
          )
            return false;
          ...
          if (u < 10 || u > 85 || v < 11 || v > 80) return false;

          for (let dy = -4; dy <= 6; dy++)
            for (let dx = -4; dx <= 6; dx++)
              if (
                !dryLand(c.x + dx, c.y + dy) ||
                get(c.x + dx, c.y + dy)?.overlay !== 255 ||
                nativeTileSpec(map.theater, get(c.x + dx, c.y + dy)!.tileIndex)?.kind === 'shore'
              )
                return false;
      ...
      if (!p)
        throw new Error(`${entry.name}: no reachable neutral plot near ${target.x},${target.y}`);
      clear(p.x + 1, p.y + 1, 4, pavement);
      map.structures.push({
        id: String(map.structures.length),
        owner: 'Neutral',
        type,
        x: p.x,
        y: p.y,
  ...
  for (const c of cells) {
    const { x: u, y: v } = nativeDisplayPosition(c, SIZE),
      kind = nativeTileSpec(map.theater, c.tileIndex)!.kind;

    if (
      !['clear', 'grass', 'rough', 'sand'].includes(kind) ||
      c.overlay !== 255 ||
      u < 5 ||
      u > 90 ||
      v < 6 ||
    ...
    if (
      starts.some((s) => Math.hypot(c.x - s.x, c.y - s.y) < 23) ||
      map.structures.some((s) => Math.hypot(c.x - s.x, c.y - s.y) < 8)
    )
      continue;
    ...
    if (hash(c.x, c.y, seed + 20) < density && (seed !== 3 || c.tileIndex === 131))
      map.terrain.push({
        x: c.x,
        y: c.y,
        type: `TREE${String(trees[Math.floor(hash(c.y, c.x, seed) * trees.length)]).padStart(2, '0')}`,
      });
  }
  ...
  if (seed === 3)
    applyRoadEnds(cells, map.theater, [
      [60, 95, '-x'],
      [139, 95, '+x'],
      [95, 52, '-y'],
    ]);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  for (const [seed, entry] of MAP_CATALOG.entries()) {
    const map = authorMap(entry, seed),
      content = writeNativeMap(map, entry.description);

    writeFileSync(`${directory}/${entry.id}.map`, content.replace(/\n/g, '\r\n'));
    console.log(
      `${entry.name}: ${map.cells.length} native cells, ${map.starts.length} starts, ${map.cells.filter((c) => c.overlay !== 255).length} ore cells, ${map.structures.length} neutral tech, ${map.terrain.length} trees; ${content.length} bytes`,
    );
  }
}

...
export function encodeLzo1x(bytes: Uint8Array): Uint8Array {
  ...
  while (p + 3 < bytes.length) {
    ...
    if (length <= 33) out.push(32 | (length - 2));
    else {
      out.push(32);
      extended(length - 33);
    }

    const distance = (p - match! - 1) << 2;
    previousMatch = out.length;
    out.push(distance & 255, distance >> 8);
...
export function encodeLcw(bytes: Uint8Array): Uint8Array {
  ...
  const run = (i: number) => {
    ...
    while (i + n < bytes.length && n < 65535 && bytes[i + n] === bytes[i]) n++;
  ...
  while (p < bytes.length) {
    ...
    if (n >= 4) {
      out.push(254, n & 255, n >> 8, bytes[p]);
      p += n;
    } else {
      const start = p++;

      while (p < bytes.length && p - start < 63 && run(p) < 4) p++;
      out.push(128 | (p - start));

      for (let i = start; i < p; i++) out.push(bytes[i]);
    }
  }
...
export function packSection(data: Uint8Array, codec: 'lzo' | 'lcw'): string {
  ...
  for (let p = 0; p < data.length; p += 8192) {
    const raw = data.subarray(p, p + 8192),
      compressed = (codec === 'lzo' ? encodeLzo1x : encodeLcw)(raw);
...
export function writeNativeMap(map: NativeMap, description: string): string {
  const iso = new Uint8Array(map.cells.length * 11 + 4),
    view = new DataView(iso.buffer),
    overlay = new Uint8Array(512 * 512).fill(255),
    overlayData = new Uint8Array(512 * 512);

  map.cells.forEach((cell, i) => {
    const p = i * 11;
    view.setUint16(p, cell.x, true);
    view.setUint16(p + 2, cell.y, true);
    view.setInt32(p + 4, cell.tileIndex, true);
    iso[p + 8] = cell.subTile;
    iso[p + 9] = cell.height;
    iso[p + 10] = cell.iceGrowth;
    overlay[cell.x + 512 * cell.y] = cell.overlay;
    overlayData[cell.x + 512 * cell.y] = cell.overlayData;
  });

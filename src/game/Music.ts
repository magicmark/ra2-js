...
export class GameMusic {
  ...
  setLevel(level: number) { this.level = level; this.sync(); }
  ...
  sync() {
    ...
    if (!this.active || this.level <= 0) { this.pause(); return; }
    ...
    try {
      if (!this.buffer) {
        ...
        this.buffer = context.createBuffer(sample.channels.length, sample.channels[0].length, sample.sampleRate);
        sample.channels.forEach((channel, index) => this.buffer!.getChannelData(index).set(channel));
      }
      const source = context.createBufferSource();
      source.buffer = this.buffer; source.connect(gain);
      source.onended = () => {
        source.disconnect();
        if (this.source !== source) return;
        this.source = null; this.buffer = null; this.offset = 0;
  ...
  reset() {
    this.active = false; this.pause();
    this.index = 0; this.offset = 0; this.buffer = null; this.failed = false;
  }
}

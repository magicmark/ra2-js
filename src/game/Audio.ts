...
export class GameAudio {
  ...
  play(name: string) {
    const cue = this.originals()?.get(name), context = this.context;
    if (!cue || !context || !this.master || context.state !== 'running' || !this.enabled || this.volume <= 0) return;
    // Prevent frame/burst spam while allowing different weapons and UI/voice channels together.
    const now = context.currentTime, last = this.lastPlayed.get(name);
    if (last !== undefined && now - last < (cue.speech ? .5 : .06)) return;
    if (cue.speech && this.speech) {
      if (name !== this.speechName && !this.speechQueue.includes(name) && this.speechQueue.length < 3) this.speechQueue.push(name);
      return;
    }
    ...
    if (!buffer) {
      buffer = context.createBuffer(sample.channels.length, sample.channels[0].length, sample.sampleRate);
      sample.channels.forEach((channel, index) => buffer!.getChannelData(index).set(channel));
      this.buffers.set(sample, buffer);
    }
    const source = context.createBufferSource(), gain = context.createGain();
    source.buffer = buffer; gain.gain.setValueAtTime(cue.volume, now);
    source.connect(gain).connect(this.master);
    if (cue.speech) { this.speech = source; this.speechName = name; }
    source.onended = () => {
      this.sources.delete(source); source.disconnect(); gain.disconnect();
      if (this.speech === source) { this.speech = null; this.speechName = null; const next = this.speechQueue.shift(); if (next) this.play(next); }
    };
  ...
  effects(effects: readonly Effect[], audible: (effect: Effect) => boolean) {
    for (const effect of effects) {
      ...
      if (effect.kind === 'shot') {
        const name = effect.sourceType === 'ifv' && IFV_SOUNDS[effect.passengerType ?? ''] ? IFV_SOUNDS[effect.passengerType!]
          : effect.sourceType === 'gi' && effect.deployed ? 'GIAttackDeployed'
          : effect.sourceType === 'flak' && effect.airTarget ? 'FlakTrackAttackAir' : WEAPON_SOUNDS[effect.sourceType ?? ''];

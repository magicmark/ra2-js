...
export function createMap(width = MAP_SIZE, height = MAP_SIZE): Tile[] {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if ((((x - 12) / 4) ** 2 + ((y - 20) / 7) ** 2 < 1 || ((x - 53) / 3) ** 2 + ((y - 43) / 8) ** 2 < 1) && n > 0.23)
        tile.terrain = 'rock';
  ...
  for (const [cx, cy, rx, ry] of [[8, 51, 5, 4], [22, 51, 4, 3], [54, 11, 5, 5], [40, 8, 4, 3], [20, 27, 3, 5], [44, 39, 4, 4]]) {
    for (let y = Math.max(0, cy - ry); y <= Math.min(height - 1, cy + ry); y++) {
      for (let x = Math.max(0, cx - rx); x <= Math.min(width - 1, cx + rx); x++) {
        const t = tiles[y * width + x];
        const d = ((x - cx) / rx) ** 2 + ((y - cy) / ry) ** 2;
        if (d < 1 + noise(x + 17, y) * 0.25 && t.terrain !== 'water' && t.terrain !== 'rock' && t.terrain !== 'road') {
  ...
  for (const [left, top, right, bottom] of [[10, 37, 21, 48], [42, 12, 52, 24]])

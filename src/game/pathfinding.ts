...
export function findPath(
  ...
  const sx = Math.floor(start.x),
    ...
    tx = Math.floor(goal.x),
    ty = Math.floor(goal.y);

  if (
    ![sx, sy, tx, ty].every(Number.isFinite) ||
    sx < 0 ||
    sy < 0 ||
    sx >= width ||
    sy >= height ||
  ...
  const length = width * height,
    from = new Int32Array(length).fill(-1);

  const costs = new Float64Array(length).fill(Infinity),
    closed = new Uint8Array(length);

  const open = new MinHeap(),
    startIndex = sy * width + sx,
  ...
  while (open.length) {
    ...
    for (const [dx, dy] of directions) {
      ...
      const next = ny * width + nx,
        cost = costs[current] + (dx && dy ? Math.SQRT2 : 1);

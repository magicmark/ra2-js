...
export interface FacingTurn { target: number; initial: number; difference: number; steps: number; left: number }
const CIRCLE = 65536;
const angleUnits = (radians: number) => ((Math.round(radians / (Math.PI * 2) * CIRCLE) % CIRCLE) + CIRCLE) % CIRCLE;
export function turnFacing(current: number, target: number, rot: number, previous?: FacingTurn): { facing: number; turn?: FacingTurn } {
  if (!(rot > 0)) return { facing: target };
  const destination = angleUnits(target), initial = angleUnits(current);
  let turn = previous;

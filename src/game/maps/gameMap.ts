...
interface ShoreMasks {
  [tile: number]: number | undefined;
}

const shoreMasks: ShoreMasks = {
  1: 12,
  2: 12,
  3: 12,
  7: 14,
...
export function nativeCellTerrain(map: NativeMap, cell: NativeCell): Tile['terrain'] {
  ...
  return spec.kind === 'road' || spec.kind === 'pavement'
    ? 'road'
    : spec.kind === 'sand' || spec.kind === 'rough'
      ? 'sand'
      : 'grass';
}

/** Native cell coordinates remain unchanged; unused square-array corners block movement. */
export function nativeGameMap(map: NativeMap) {
  const width = Math.max(...map.cells.map((c) => c.x)) + 1,
    height = Math.max(...map.cells.map((c) => c.y)) + 1;
  ...
  for (const cell of map.cells) {
    ...
    const terrain = nativeCellTerrain(map, cell),
      resource = nativeOverlaySpec(cell.overlay);

    tiles[cell.y * width + cell.x] = {
      terrain,
      ore: resource ? (cell.overlayData + 1) * (resource.kind === 'gems' ? 200 : 100) : 0,
      variant: cell.subTile,
    };
  }

  for (const tree of map.terrain)
    if (tiles[tree.y * width + tree.x]) tiles[tree.y * width + tree.x].terrain = 'rock';

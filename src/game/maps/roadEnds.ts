...
export function applyRoadEnds(cells: NativeCell[], theater: NativeTheater, endpoints: readonly RoadEnd[]): void {
  const index = new Map(cells.map(c => [c.x + 512 * c.y, c]));
  for (const [x, y, direction] of endpoints) {
    const [offset, dx, dy] = ends[direction], straight = dx ? 293 : 294;
    const parts = Array.from({ length: 3 }, (_, subTile) => {
      const px = x + (dy ? subTile - 1 : 0), py = y + (dx ? subTile - 1 : 0);
      return { cell: index.get(px + 512 * py), approach: index.get(px - dx + 512 * (py - dy)), subTile };
    });
    // Also permits smaller training maps without placing clipped templates.
    if (!parts.every(({ cell, approach, subTile }) => cell?.tileIndex === straight && cell.subTile === subTile && approach?.tileIndex === straight && approach.subTile === subTile)) continue;

...
export const SHORE_BY_CORNERS: Readonly<Record<number, number>> = { 1: 37, 2: 35, 3: 17, 4: 39, 5: 25, 7: 23, 8: 33, 10: 9, 11: 15, 12: 1, 13: 31, 14: 7 };
export const SHORE_CORNERS = Object.fromEntries(Object.entries(SHORE_BY_CORNERS).map(([mask, tile]) => [88 + tile, Number(mask)]));
...
export function applyShorelines(cells: NativeCell[], water: Set<number>, sand: number): void {
  ...
  const vertices = new Set(cells.filter(c => c.x % 2 === 0 && c.y % 2 === 0 && water.has(key(c.x, c.y))).map(c => key(c.x, c.y)));
  const blocks = cells.filter(c => c.x % 2 === 0 && c.y % 2 === 0);
  const maskAt = (x: number, y: number) => [[0, 0], [2, 0], [0, 2], [2, 2]].reduce((mask, [dx, dy], bit) => mask | (vertices.has(key(x + dx, y + dy)) ? 1 << bit : 0), 0);
  ...
  do {
    ...
    for (const c of blocks) {
      ...
      if (mask === 6 || mask === 9) { vertices.delete(key(c.x + 2, c.y + (mask === 9 ? 2 : 0))); changed = true; }
    }
  } while (changed);
  ...
  for (const anchor of blocks) {
    ...
    for (let subTile = 0; subTile < 4; subTile++) {
      ...
      if (!c) continue;
      c.tileIndex = tile; c.subTile = subTile;
  ...
  for (const c of shoreCells) for (let dy = -2; dy <= 2; dy++) for (let dx = -2; dx <= 2; dx++) {
    const near = index.get(key(c.x + dx, c.y + dy));
    if (near && near.tileIndex !== 314 && SHORE_CORNERS[near.tileIndex] === undefined) { near.tileIndex = sand; near.subTile = 0; }
  }
}
...
export function applyLandTransitions(cells: NativeCell[], theater: NativeTheater): void {
  const sand = theater === 'SNOW' ? 713 : 493, rough = theater === 'SNOW' ? 696 : 418, pavement = theater === 'SNOW' ? 734 : 534;
  const transitions = new Map([[131, 132], [rough, rough + 1], [sand, sand + 1], [pavement, theater === 'SNOW' ? 749 : 463]]);
  const index = new Map(cells.map(c => [key(c.x, c.y), c]));
  ...
  for (const c of cells) {
    ...
    if (directions.some(([dx, dy]) => { const other = original.get(key(c.x + dx, c.y + dy)); return other !== undefined && transitions.has(other) && other !== c.tileIndex && other < c.tileIndex; })) {
      c.tileIndex = 0; c.subTile = 0;
    }
  }
  ...
  for (const c of cells) {
    ...
    const mask = directions.reduce((mask, [dx, dy], bit) => {
      const near = index.get(key(c.x + dx, c.y + dy)), material = near && materials.get(key(near.x, near.y));
      // Shore sand is the same base material. Roads also cut across, rather
      // than erase, the surrounding ground artwork.
      const joins = material === undefined || material === c.tileIndex || (c.tileIndex === sand && SHORE_CORNERS[material] !== undefined) || (material >= 293 && material <= 295);
      return mask | (joins ? 0 : 1 << bit);
    }, 0);
    if (mask) { c.tileIndex = first + mask; c.subTile = 0; }
  }
}

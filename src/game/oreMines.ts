...
export const ORE_MINE_ANIMATION = { frames: 11, ticksPerFrame: 3, probability: 0.003 } as const;

export const DEFAULT_ORE_MINES: readonly Vec2[] = [
  [8, 51],
  [20, 51],
  [54, 11],
  [40, 8],
...
export interface OreMine extends Vec2 {
...
export function initializeOreMines(state: GameState): OreMine[] {
  const positions = state.nativeMap
    ? state.nativeMap.terrain.filter((t) => t.type.toUpperCase() === 'TIBTRE01')
    : DEFAULT_ORE_MINES;
  ...
  return positions.map(({ x, y }) => {
    ...
    if (tile) {
      ...
      if (tile.nativeArt) tile.nativeArt.terrain = 'rock';
    }
    ...
    return {
      x,
      y,
      randomState: (Math.imul(x + 1, 73856093) ^ Math.imul(y + 1, 19349663)) >>> 0,
      fieldCells,
      initialOre: fieldCells.reduce((sum, i) => sum + state.tiles[i].ore, 0),
    };
  });
}

function random(mine: OreMine): number {
  let n = mine.randomState;
  n ^= n << 13;
  n ^= n >>> 17;
  n ^= n << 5;
  mine.randomState = n >>> 0;

  return mine.randomState / 4294967296;
}

export function oreMineFrame(mine: OreMine | undefined, time: number): number {
  return mine?.animationStartedAt === undefined
    ? 0
    : Math.min(
        ORE_MINE_ANIMATION.frames - 1,
        Math.floor(((time - mine.animationStartedAt) * 30) / ORE_MINE_ANIMATION.ticksPerFrame),
      );
}
...
export function updateOreMines(state: GameState, defs: Record<string, UnitDef>): void {
  for (const mine of state.oreMines ?? []) {
    ...
    if (mine.fieldCells.reduce((sum, i) => sum + state.tiles[i].ore, 0) > mine.initialOre * 0.25) {
    ...
    if (
      state.time - mine.animationStartedAt <
      (ORE_MINE_ANIMATION.frames * ORE_MINE_ANIMATION.ticksPerFrame) / 30
    )
      continue;
    mine.animationStartedAt = undefined;
    const choices: Vec2[] = [];
        ...
        if (
          state.entities.some((e) => {
            ...
            const def = defs[e.type],
              building = def && (def.category === 'structures' || def.category === 'defenses');

            const left = building ? e.x : Math.floor(e.x),
              top = building ? e.y : Math.floor(e.y),
              [w, h] = building ? def.footprint : [1, 1];
    ...
    const p = choices[Math.floor(random(mine) * choices.length)],
      tile = state.tiles[p.y * state.width + p.x];

...
export const isWarFactory = (type: string): boolean =>
  type === 'warfactory' || type === 'warfactory_soviet';
 ...
 */
export function factoryExitLane(factory: Vec2) {
  return {
    start: { x: factory.x + 3, y: factory.y + 1 },
    end: { x: factory.x + 5.5, y: factory.y + 1 },
  };
}
...
export function factoryForExit(unit: Entity, entities: Entity[]): Entity | undefined {
  return (
    unit.factoryExit &&
    entities.find((e) => e.id === unit.factoryExit!.factoryId && e.hp > 0 && !e.selling)
  );
}

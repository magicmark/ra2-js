...
export function placementCells(
  ...
  const [width, height] = def.footprint,
    entities = state.entities.filter((entity) => entity.hp > 0);
  ...
  return Array.from({ length: width * height }, (_, index) => {
    ...
    if (valid) {
      ...
      valid =
        ...
        tile.ore <= 0 &&
        (side !== 0 || !!state.explored[ty * state.width + tx]);
    }

    if (valid)
      valid = !entities.some((entity) => {
        if (entity.transportId !== undefined || defs[entity.type].movement === 'air') return false;
        const other = defs[entity.type];

        return isBuilding(other)
          ? tx < entity.x + other.footprint[0] &&
              tx + 1 > entity.x &&
              ty < entity.y + other.footprint[1] &&
              ty + 1 > entity.y
          : entity.x > tx - 0.3 &&
              entity.x < tx + 1.3 &&
              entity.y > ty - 0.3 &&

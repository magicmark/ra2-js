...
export class Game implements GameAPI {
  ...
  get nativeGameSpeedIndex(): number {
    return nativeGameSpeedIndex(this.state.speed);
  }
  setGameSpeed(speed: number): void {
    if (Number.isFinite(speed)) this.state.speed = clamp(speed, 0.25, 4);
  }
  configureLocalTools(options: Partial<{ instantBuild: boolean; free: boolean }>): void {
    if (!import.meta.env.DEV) return;

    if (options.instantBuild === true || options.instantBuild === false)
      this.localTools.instantBuild = options.instantBuild;

    if (options.free === true || options.free === false) this.localTools.free = options.free;
  }
  placeLocalEnemy(type: string, x: number, y: number): Entity | undefined {
    ...
    if (
      ...
      isBuilding(def) ||
      def.mapOnly ||
      !this.isPassable(tx, ty, def) ||
      this.mobileOccupies(tx, ty, -1, def)
    )
      return;
    const entity = this.spawn(type, 1, tx + 0.5, ty + 0.5);
    entity.order = 'guard';
    this.updateVision();
  ...
  setAnimationDefinitions(
    ...
    const valid = (definition: AnimationDefinition, allowEmpty = false) =>
      definition &&
      Number.isInteger(definition.frames) &&
      definition.frames >= (allowEmpty ? 0 : 1) &&
      Number.isInteger(definition.ticksPerFrame) &&
      definition.ticksPerFrame >= (allowEmpty ? 0 : 1);

    for (const [name, definition] of Object.entries(effects))
      if (!valid(definition)) throw new Error(`Invalid original animation timing: ${name}`);

    for (const [name, definition] of Object.entries(infantry)) {
      if (
        !definition ||
        !Number.isInteger(definition.fireFrame) ||
        definition.fireFrame < 0 ||
        !definition.sequences ||
        Object.values(definition.sequences).some((sequence) => !valid(sequence, true))
      )
        throw new Error(`Invalid original infantry timing: ${name}`);
    }

    this.effectAnimations = structuredClone({
      ...NATIVE_EFFECT_TIMINGS,
      ...Object.fromEntries(
        Object.entries(effects).map(([name, definition]) => [name.toUpperCase(), definition]),
      ),
    });
    this.infantryAnimations = structuredClone(
      Object.fromEntries(
        Object.entries(infantry).map(([name, definition]) => [name.toLowerCase(), definition]),
      ),
    );
  }

  restart(): void {
    const speed = Number.isFinite(this.state?.speed)
      ? clamp(this.state.speed, 0.25, 4)
      : DEFAULT_GAME_SPEED;
    ...
    const terrain = this.nativeMap
      ? nativeGameMap(this.nativeMap)
      : { width: MAP_SIZE, height: MAP_SIZE, tiles: createMap() };
    ...
    this.state = {
      ...
      entities: [],
      sides: [
        side(0, 'allied', 'British Command', '#2269d4'),
        side(1, 'soviet', 'Soviet AI', '#ff1919'),
      ],
      time: 0,
      paused: false,
      speed,
      ...
      effects: [],
      events: [],
      fog: new Uint8Array(terrain.width * terrain.height),
      explored: new Uint8Array(terrain.width * terrain.height),
    };
    this.state.oreMines = initializeOreMines(this.state);
    this.vision = [this.state.fog, new Uint8Array(terrain.width * terrain.height)];

    if (this.nativeMap) {
      for (const [sideId, waypoint] of [
        [0, 0],
        [1, 3],
      ]) {
        ...
        const layout: [string, number, number][] = [
          ['conyard', -2, -2],
          [sideId ? 'power_soviet' : 'power', 3, -2],
          [sideId ? 'refinery_soviet' : 'refinery', -3, 4],
          [sideId ? 'barracks_soviet' : 'barracks', 4, 4],
          [sideId ? 'warminer' : 'miner', -4.5, 7.5],
          [sideId ? 'rhino' : 'grizzly', 5.5, 1.5],
          ...[3.5, 4.5, 5.5, 6.5].map((dx): [string, number, number] => [
            sideId ? 'conscript' : 'gi',
            dx,
            3.5,
          ]),
      ...
      for (const structure of this.nativeMap.structures) {
        const spec = Object.entries(NATIVE_STRUCTURE_SPECS).find(
        ...
        entity.hp = (entity.maxHp * structure.health) / 256;
        entity.nativeType = structure.type;
      }
    } else {
    ...
    this.updateVision();
    this.notify(
      'Battle control online. Build your base, harvest ore and destroy the Soviet base.',
      'info',
    );
  }
  ...
  canBuild(type: string, sideId = 0): ReturnType<GameAPI['canBuild']> {
    const def = this.defs[type],
      side = this.state.sides[sideId];

    if (!def || !side) return { ok: false, reason: 'Unknown unit or side' };

    if (def.mapOnly)
      return { ok: false, reason: 'Capture this neutral structure with an Engineer' };

    if (this.state.winner !== null || side.defeated)
      return { ok: false, reason: 'Battle has ended' };

    if (def.faction !== 'both' && def.faction !== side.faction)
      return { ok: false, reason: 'Unavailable to this faction' };

    const owned = this.state.entities.filter(
      (e) => e.side === sideId && e.hp > 0 && !e.selling && !e.constructing,
    );
    ...
    if (
      def.buildLimit &&
      owned.filter((e) => e.type === type).length +
        this.state.entities.filter((e) => e.side === sideId && e.type === type && e.constructing)
          .length +
        side.queues[def.category].filter((i) => i.type === type).length >=
        def.buildLimit
    )
    ...
    if (
      ...
      this.state.entities.filter(
        (e) => e.side === sideId && this.defs[e.type].factory === 'radar' && e.hp > 0,
      ).length +
        side.queues.vehicles.filter((i) => this.defs[i.type].factory === 'radar').length >=
        owned.filter((e) => e.type === 'radar').length * 4
    )
      return {
        ok: false,
        reason: 'Airforce Command has four aircraft pads. Build another command.',
      };

    if (!owned.some((e) => this.produces(e, def)))
      return {
        ok: false,
        reason: `Requires ${def.category === 'infantry' ? 'Barracks' : def.category === 'vehicles' ? 'War Factory' : 'Construction Yard'}`,
      };
    ...
    if (
      !isBuilding(def) &&
      owned.filter((e) => !isBuilding(this.defs[e.type])).length + queuedUnits >= 100
    )
  ...
  cancelBuild(category: Category, sideId = 0, itemId?: number): void {
    ...
    const queue = side.queues[category],
      index =
        itemId === undefined ? queue.length - 1 : queue.findIndex((item) => item.id === itemId);
    ...
    if (canceled) {
      ...
      if (sideId === 0)
        this.notify(`${this.defs[canceled.type].name} canceled. Credits refunded.`, 'info');
    }
  }
  ...
  place(type: string, x: number, y: number, sideId = 0): boolean {
    ...
    if (!item || !item.ready || item.type !== type || !this.canPlace(type, x, y, sideId))
      return false;
    queue.shift();
    const entity = this.spawn(type, sideId, x, y);

    if (type !== 'wall' && type !== 'butchers')
      entity.constructing = { startedAt: this.state.time, duration: BUILDING_CONSTRUCTION_SECONDS };
    this.rebuildBlocked();
    this.updatePower();
    this.updateVision();
    ...
    if (sideId === 0)
      this.notify(
        `${def.name} ${entity.constructing ? 'construction underway' : 'online'}`,
        'success',
      );
  ...
  commandPath(id: number): { points: Vec2[]; attack: boolean } | undefined {
    ...
    if (target && revealedEntity(this.state, this.defs, target))
      return { points: [this.center(target)], attack: true };

    if (isBuilding(def))
      return entity.rally ? { points: [{ ...entity.rally }], attack: false } : undefined;

    const escort = this.state.entities.find(
      (e) => e.id === memory?.escortId && e.hp > 0 && e.side === entity.side,
    );

    const destination = escort
      ? this.center(escort)
      : entity.order === 'move'
        ? memory?.destination
    ...
    return {
      points: [destination, ...(memory?.waypoints ?? [])].map((point) => ({ ...point })),
      attack: false,
    };
  }
  ...
  orderMove(ids: number[], x: number, y: number, attackMove = false): void {
    ...
    for (const entity of own.filter((e) => isBuilding(this.defs[e.type])))
      entity.rally = this.constrain({ x, y });
    this.issueMove(
      own.filter((e) => !isBuilding(this.defs[e.type])),
      this.constrain({ x, y }),
      attackMove,
    );
  ...
  orderAttack(ids: number[], targetId: number, force = false): void {
    ...
    for (const entity of units) {
      ...
      if (
        (!def.damage && entity.type !== 'engineer' && def.ability !== 'spy') ||
        entity.id === targetId ||
        !this.canTarget(entity, target)
      )
        continue;

      if (
        entity.type === 'engineer' &&
        (!isBuilding(this.defs[target.type]) ||
          (target.side === entity.side && target.hp >= target.maxHp))
      )
        continue;
  ...
  deploy(ids: number[]): void {
    for (const entity of this.commandable(ids, 0)) {
      if (entity.type === 'mcv') {
        this.deployMcv(entity);
        continue;
      }

      if (this.defs[entity.type].passengers) {
        this.unloadTransport(entity);
        continue;
      }
  ...
  scatter(ids: number[]): void {
    ...
    const center = {
      x: units.reduce((sum, e) => sum + e.x, 0) / units.length,
      y: units.reduce((sum, e) => sum + e.y, 0) / units.length,
    };

    for (const entity of units) {
      const angle =
        distance(entity, center) > 0.1
          ? Math.atan2(entity.y - center.y, entity.x - center.x)
          : entity.id * 2.399963;

      this.issueMove(
        [entity],
        this.constrain({ x: entity.x + Math.cos(angle) * 2, y: entity.y + Math.sin(angle) * 2 }),
        false,
      );
    }
  }
  ...
  orderForceMove(ids: number[], x: number, y: number): void {
    ...
    for (const entity of this.commandable(ids, 0)) {
      ...
      if (this.defs[entity.type].crusher && this.isPassable(Math.floor(x), Math.floor(y))) {
        const destination = { x: Math.floor(x) + 0.5, y: Math.floor(y) + 0.5 };
        entity.path = this.path(entity, destination);
  ...
  orderGuard(ids: number[], x: number, y: number, escortId?: number): void {
    if (
      escortId !== undefined &&
      !this.state.entities.some((e) => e.id === escortId && e.side === 0 && e.hp > 0)
    )
      return;
    this.orderMove(
      ids.filter((id) => id !== escortId),
      x,
      y,
      true,
    );
    ...
    for (const entity of this.commandable(ids, 0))
      if (entity.id !== escortId) this.memory(entity).escortId = escortId;
  }
  ...
  orderForceFire(ids: number[], x: number, y: number): void {
    ...
    const units = this.commandable(ids, 0).filter(
      (e) => e.type !== 'george' && this.defs[e.type].damage > 0,
    );
  ...
  sell(id: number): boolean {
    ...
    this.state.sides[0].money += Math.floor(
      this.defs[entity.type].cost * 0.5 * (entity.hp / entity.maxHp),
    );
    ...
    for (const other of this.state.entities)
      if (other.targetId === entity.id) other.targetId = null;
  ...
  canRepair(id: number): boolean {
    ...
    return (
      !!entity &&
      isBuilding(this.defs[entity.type]) &&
      this.state.winner === null &&
      (this.repairs.has(id) ||
        (entity.hp < entity.maxHp &&
          (this.state.sides[0].money >= 1 || (import.meta.env.DEV && this.localTools.free))))
    );
  }
  ...
  repair(id: number): boolean {
    ...
    if (this.repairs.has(id)) {
      ...
      this.notify('Repair canceled', 'info');
  ...
  private step(dt: number): void {
    ...
    this.aiTimer -= dt;
    this.updatePower();
    this.gapGenerators = this.state.entities.filter(
      (e) => e.type === 'gap_generator' && this.operational(e),
    );
    this.tickStorms();
    ...
    for (const entity of this.state.entities.slice()) {
      ...
      if (entity.constructing) {
        if (
          this.state.time + 1e-9 >=
          entity.constructing.startedAt + entity.constructing.duration
        ) {
          entity.constructing = undefined;
          this.finishConstruction(entity);
          this.updatePower();
          this.updateVision();
        } else continue;
      }
      ...
      if (entity.factoryExit) {
        this.tickFactoryExit(entity, dt);
        continue;
      }
      ...
      if (entity.type === 'tech_oil' && entity.side >= 0) {
        ...
        if (entity.harvestTimer >= 100 / 30) {
          entity.harvestTimer -= 100 / 30;
          this.state.sides[entity.side].money += 20;
        }
      }
      ...
      if (memory.escortId !== undefined && !entity.targetId && memory.repath <= 0) {
        const escorted = this.state.entities.find(
          (other) => other.id === memory.escortId && other.hp > 0 && other.side === entity.side,
        );
        ...
        if (!escorted) memory.escortId = undefined;
          ...
          if (destination) {
            entity.path = this.path(entity, destination, true);
            entity.order = 'move';
            memory.destination = destination;
            memory.attackMove = true;
            memory.repath = 0.5;
          }
        }
      }
      ...
      if (
        def.harvester &&
        (entity.order === 'harvest' || entity.order === 'return' || entity.order === 'idle')
      )
        this.tickHarvester(entity, dt);

      if (def.damage > 0 || entity.type === 'engineer' || def.ability === 'spy')
        this.tickCombat(entity, dt);
    ...
    const sold = this.state.entities.filter(
      (e) => e.selling && this.state.time - e.selling.startedAt + 1e-9 >= e.selling.duration,
    );

    for (const entity of sold) this.removeEntity(entity, false);
    this.updateInspections(dt);

    if (destroyed.length || sold.length) {
      this.rebuildBlocked();
      this.updatePower();
    }

    if (this.visionTimer <= 0 || destroyed.length || sold.length) {
  ...
  private updateQueues(dt: number): void {
    for (const side of this.state.sides) {
      ...
      for (const category of CATEGORIES) {
        ...
        const missing = !item.ready
          ? this.defs[item.type].requires.find(
              (required) =>
        ...
        const producer = this.state.entities.find(
          (e) =>
            e.side === side.id &&
            e.hp > 0 &&
            !e.selling &&
            !e.constructing &&
            e.transportId === undefined &&
            this.produces(e, this.defs[item.type]) &&
        ...
        const productionRate = lowPower
          ? clamp(side.power / Math.max(1, side.powerUsed), 0.5, 0.8)
          : 1;
        ...
        const requestedProgress = Math.min(
          1 - item.progress,
          instant ? 1 : (dt / def.buildTime) * productionRate,
        );
        ...
        item.progress = Math.min(
          1,
          item.progress + (def.cost > 0 && !free ? payment / def.cost : requestedProgress),
        );
        ...
        if (isBuilding(def)) {
          ...
          if (side.id === 0)
            this.notify(
              `${def.name} ready. Choose a location in your base.`,
              'success',
          ...
          for (const candidate of this.state.entities.filter(
            (e) =>
              e.side === side.id &&
              e.hp > 0 &&
              !e.selling &&
          ...
          if (source.rally && unit.type !== 'george' && !def.harvester && !unit.factoryExit)
            this.issueMove([unit], source.rally, false);
  ...
  private updateInspections(dt: number): void {
    ...
    for (const entity of entities) {
      entity.inspectedBy = undefined;
      entity.inspectionProgress = undefined;
    }
    ...
    for (const george of entities.filter((e) => e.type === 'george').sort((a, b) => a.id - b.id)) {
      ...
      if (
        george.hp <= 0 ||
        george.side < 0 ||
        george.transportId !== undefined ||
        george.factoryExit ||
        (george.disabledUntil ?? 0) > this.state.time
      ) {
      ...
      const eligible = (target: Entity) =>
        target.id !== george.id &&
        target.type !== 'george' &&
        target.side === george.side &&
        target.hp > 0 &&
        ['infantry', 'vehicles'].includes(this.defs[target.type].category) &&
      ...
      const target =
        entities.find((e) => e.id === previous?.targetId && eligible(e)) ??
        (!previous && this.state.time >= (memory.inspectionReadyAt ?? 0)
          ? entities
              .filter(eligible)
              .sort((a, b) => distance(george, a) - distance(george, b) || a.id - b.id)[0]
          : undefined);
      ...
      if (previous || target) {
        memory.inspectionReadyAt = this.state.time + 5;
        memory.wanderUntil = 0;
      }
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        memory.wanderRandom =
          (Math.imul(memory.wanderRandom ?? george.id, 1664525) + 1013904223) >>> 0;
        const angle = (memory.wanderRandom / 0x100000000) * Math.PI * 2;

        const destination = this.nearestOpen(
          this.constrain({ x: george.x + Math.cos(angle) * 4, y: george.y + Math.sin(angle) * 4 }),
          george.id,
          undefined,
          2,
          this.defs.george,
  ...
  private tickRepair(entity: Entity, dt: number): void {
    const side = this.state.sides[entity.side],
      def = this.defs[entity.type],
      memory = this.memory(entity);

    // RepairRate=.016 authored minutes (900 frames/minute), RepairStep=8.
    memory.repairTimer = (memory.repairTimer ?? 0) + dt;

    if (memory.repairTimer < 0.48) return;
    memory.repairTimer -= 0.48;
    const hp = Math.min(entity.maxHp - entity.hp, 8);

    const price =
      import.meta.env.DEV && entity.side === 0 && this.localTools.free
        ? 0
        : (hp / entity.maxHp) * def.cost * 0.15;
  ...
  private updatePower(): void {
    ...
    for (const entity of this.state.entities) {
      ...
      const power = this.defs[entity.type].power,
        side = this.state.sides[entity.side];
  ...
  private issueMove(units: Entity[], destination: Vec2, attackMove: boolean): void {
    ...
    for (const entity of sorted) {
      const moving = entity.previous
        ? distance(entity, entity.previous) > 1e-6
      ...
      this.memories.set(entity.id, {
        destination: target,
        attackMove: entity.type !== 'george' && attackMove,
        repath: 1,
        stuck: 0,
        idleTimer: 0,
        turningBeforeMove: !moving && !!this.defs[entity.type].rot,
      });
    }
  }
  ...
  private moveEntity(entity: Entity, dt: number): void {
    ...
    if (entity.deployed || entity.deployment || (entity.disabledUntil ?? 0) > this.state.time)
      return;

    if (memory.avoidancePoint && !entity.path.includes(memory.avoidancePoint))
      memory.avoidancePoint = undefined;
    ...
    if (def.movement === 'teleport') {
      ...
      if (this.mobileOccupies(Math.floor(target.x), Math.floor(target.y), entity.id)) {
        entity.path = [];

        return;
      }

      entity.disabledUntil =
        this.state.time + Math.max(16, (distance(entity, target) * 256) / 48) / 30;
      entity.x = target.x;
      entity.y = target.y;
    ...
    const amount = Math.min(d, travel),
      nx = entity.x + (d > 0 ? (dx / d) * amount : 0),
      ny = entity.y + (d > 0 ? (dy / d) * amount : 0);

    // Cell-center spacing is also enforced during travel, not just at orders.
    if (memory.forceMove && def.crusher) {
      for (const other of this.state.entities) {
        if (
          other.side !== entity.side &&
          other.hp > 0 &&
          other.transportId === undefined &&
          this.sameLayer(entity, other) &&
          this.defs[other.type].category === 'infantry' &&
          Math.hypot(other.x - nx, other.y - ny) < 0.48
        ) {
          other.hp = 0;
    ...
    const collision = this.state.entities.find(
      (other) =>
        other.id !== entity.id &&
        other.hp > 0 &&
        other.transportId === undefined &&
        this.sameLayer(entity, other) &&
        !isBuilding(this.defs[other.type]) &&
        Math.hypot(other.x - nx, other.y - ny) < 0.48,
    );
    ...
    if (collision) {
      ...
      if (
        memory.stuck > 0.15 &&
        !memory.avoidancePoint &&
        d > 0.001 &&
        (memory.detourRetryAt ?? 0) <= this.state.time
      ) {
        // A failed route search must not repeat every logic frame in a crowd.
        memory.detourRetryAt = this.state.time + 0.5;

        const forward = Math.atan2(dy, dx),
          detourLength = def.category === 'vehicles' ? 1 : 0.5;
        ...
        for (const turn of [
          ...
          const angle = forward + turn,
            detour = {
              x: entity.x + Math.cos(angle) * detourLength,
              y: entity.y + Math.sin(angle) * detourLength,
            };

          let clear = true,
            previous = { x: Math.floor(entity.x), y: Math.floor(entity.y) };

          for (let step = 1; step <= 10; step++) {
            const px = entity.x + ((detour.x - entity.x) * step) / 10,
              py = entity.y + ((detour.y - entity.y) * step) / 10;

            const tx = Math.floor(px),
              ty = Math.floor(py);

            if (
              !this.isPassable(tx, ty, entity) ||
              (tx !== previous.x &&
                ty !== previous.y &&
                (!this.isPassable(tx, previous.y, entity) ||
                  !this.isPassable(previous.x, ty, entity))) ||
              this.state.entities.some(
                (other) =>
                  other.id !== entity.id &&
                  other.hp > 0 &&
                  !isBuilding(this.defs[other.type]) &&
                  Math.hypot(other.x - px, other.y - py) <
                    (def.category === 'vehicles' ? 0.49 : 0.48),
              )
            ) {
              clear = false;
              break;
          ...
          if (!onward.length && distance(detour, final) > 0.2) continue;
          entity.path = [detour, ...onward];
          memory.avoidancePoint = detour;
          memory.stuck = 0;
    ...
    if (d <= amount + 1e-6) {
      ...
      if (entity.path.length && travel - amount > 1e-8 && def.speed > 0)
        this.moveEntity(entity, (travel - amount) / def.speed);
    }
  }
  ...
  private tickHarvester(entity: Entity, dt: number): void {
    const def = this.defs[entity.type],
      memory = this.memory(entity),
      capacity = def.capacity ?? 700;

    const refineries = this.state.entities.filter(
      (e) =>
        e.side === entity.side &&
        e.hp > 0 &&
        !e.selling &&
        !e.constructing &&
        e.transportId === undefined &&
        (e.type === 'refinery' || e.type === 'refinery_soviet'),
    );
    ...
    if (entity.order === 'return') {
      const nearest = refineries.sort(
        (a, b) => this.distanceToEntity(entity, a) - this.distanceToEntity(entity, b),
      )[0];

      if (
        entity.type === 'miner' &&
        entity.cargo >= capacity &&
        this.distanceToEntity(entity, nearest) >= 1.1
      ) {
        ...
        entity.x = dock.x;
        entity.y = dock.y;
        entity.previous = { ...dock };
        entity.harvestTimer = 0;
        memory.heading = undefined;
        memory.oreTarget = undefined;
        memory.repath = 0;
      ...
      if (this.distanceToEntity(entity, nearest) < 1.1) {
        ...
        if (entity.harvestTimer >= 1.1) {
          this.state.sides[entity.side].money +=
            entity.cargo *
            (1 +
              0.25 *
                this.state.entities.filter(
                  (e) => e.side === entity.side && e.type === 'ore_purifier' && this.operational(e),
                ).length);
          entity.cargo = 0;
          entity.harvestTimer = 0;
          entity.order = 'harvest';
    ...
    if (memory.oreTarget) {
      const tile =
        this.state.tiles[
          Math.floor(memory.oreTarget.y) * this.state.width + Math.floor(memory.oreTarget.x)
        ];
  ...
  private findOre(entity: Entity): Vec2 | undefined {
    ...
    for (const candidate of candidates) {
      if (distance(entity, candidate.point) < 0.75 || this.path(entity, candidate.point).length)
  ...
  private tickCombat(entity: Entity, _dt: number): void {
    ...
    const def = this.weaponFor(entity),
      memory = this.memory(entity);

    if (
      entity.deployment ||
      !this.operational(entity) ||
      (this.defs[entity.type].ammo && entity.ammo === 0) ||
      this.memory(entity).transportTarget !== undefined
    )
      return;
    ...
    const c4 =
      ...
      this.state.entities.some(
        (e) =>
          e.id === entity.targetId &&
          (isBuilding(this.defs[e.type]) || this.defs[e.type].movement === 'water'),
      );

    const range = c4 ? 0.9 : entity.deployed ? (def.deployedRange ?? def.range) : def.range;
    ...
    if (memory.groundTarget) {
      const to = memory.groundTarget,
        center = this.center(entity);
      ...
      if (distance(center, to) <= range) {
        ...
        if (def.turret) memory.turretAim = aim;
        else memory.heading = aim;

        if (entity.cooldown <= 0 && this.isAimed(entity, aim)) {
          const hit = this.state.entities.find(
            (e) => e.id !== entity.id && e.hp > 0 && this.distanceToEntity(to, e) < 0.5,
          );
    ...
    const pursuingKnownContact =
      target &&
      entity.side === 0 &&
      entity.order === 'attack' &&
      revealedEntity(this.state, this.defs, target);

    if (
      target &&
      ((!pursuingKnownContact && !this.visibleTo(target, entity.side)) ||
        !this.canTarget(entity, target))
    ) {
      this.cancelFireIntent(entity, memory);
      entity.targetId = null;
    ...
    if (!target && entity.type !== 'engineer' && def.ability !== 'spy' && memory.idleTimer <= 0) {
      ...
      const candidates = this.state.entities.filter(
        (other) =>
          ...
          !other.selling &&
          other.transportId === undefined &&
          this.canTarget(entity, other) &&
          this.visibleTo(other, entity.side) &&
          this.distanceToEntity(entity, other) <=
            (isBuilding(def) || def.harvester || entity.deployed || memory.holdPosition
              ? range
              : def.sight),
      );
    ...
    if (def.ability === 'spy' && isBuilding(this.defs[target.type]) && d <= 0.9) {
      this.infiltrate(entity, target);

      return;
    }

    if (c4 && d <= 0.9) {
      this.damageTarget(entity, target, target.maxHp * 10);
      entity.cooldown = 1;
    ...
    if (entity.type === 'engineer' && isBuilding(this.defs[target.type]) && d <= 0.9) {
      ...
      if (target.type === 'tech_oil' && target.side === -1)
        this.state.sides[entity.side].money += 1000;
      ...
      this.notify(
        `${this.defs[target.type].name} ${repairing ? 'repaired' : 'captured'}`,
        entity.side === 0 ? 'success' : 'warning',
      );
      this.updatePower();
    ...
    if (d <= range && entity.type !== 'engineer' && def.ability !== 'spy') {
      ...
      const center = this.center(entity),
        to = this.center(target);
      ...
      if (entity.order !== 'attack' && d > def.sight + 1) {
        entity.targetId = null;

        return;
      }

      const firingPoint = this.nearestFiringPosition(
        entity,
        target,
        entity.type === 'engineer' || def.ability === 'spy' ? 0.7 : range * 0.85,
      );
  ...
  private cancelFireIntent(entity: Entity, memory: UnitMemory): void {
    ...
    if (
      !entity.deployment &&
      entity.infantryAnimation &&
      ['FireUp', 'FireFly', 'DeployedFire', 'WetAttack'].includes(entity.infantryAnimation.sequence)
    )
  ...
  private animationInterval(definition: AnimationDefinition): number {
    return definition.normalized
      ? nativeNormalizedInterval(definition.ticksPerFrame, this.nativeGameSpeedIndex)
      : definition.ticksPerFrame;
  }
  private beginDeployment(entity: Entity, target: boolean): void {
    ...
    entity.infantryAnimation = {
      sequence: target ? 'Deploy' : 'Undeploy',
  ...
  private updateInfantryAnimation(entity: Entity): void {
    ...
    if (!definition)
      throw new Error(`Missing original infantry timing: ${entity.type} ${sequence}`);

    if (
      this.state.time + 1e-9 <
      startedAt + definition.frames * this.animationInterval(definition) * STEP
    )
      return;
    entity.infantryAnimation = undefined;
  ...
  private cancelInfantryIdle(entity: Entity): void {
    ...
    if (memory.idleActionTurning) {
      memory.heading = entity.facing;
      memory.bodyTurn = undefined;
    }

    memory.idleActionPending = undefined;
    memory.idleActionTurning = false;
    memory.idleActionAt = undefined;
  }
  private updateInfantryIdle(entity: Entity): void {
    ...
    if (def.category !== 'infantry' || entity.type === 'rocketeer' || entity.type === 'george')
      return;

    const memory = this.memory(entity),
      definition = this.infantryAnimations[def.sprite];
    ...
    if (memory.idleActionAt === undefined)
      memory.idleActionAt =
        this.state.time + random(Math.floor(frequency * 450), Math.floor(frequency * 1800)) * STEP;
    ...
    const choice = random(0, 10),
      sequence = [3, 4, 5].includes(choice)
        ? 'Idle1'
        : [1, 2, 7].includes(choice)
          ? 'Idle2'
          : undefined;
    ...
    if (sequence) {
      ...
      memory.idleActionPending = sequence;
      memory.idleActionTurning = true;
    } else if (choice !== 0) {
      memory.heading = (random(0, 7) * Math.PI) / 4;
      memory.idleActionTurning = true;
    }
  }
  ...
  private randomCombatFrames(min: number, max: number): number {
    return min + (this.randomCombatValue() % (max - min + 1));
  }
  private addAnimation(
    ...
    const name = animation.toUpperCase(),
      definition = this.effectAnimations[name];

    if (!definition) throw new Error(`Missing original animation timing: ${name}`);

    const animationTicksPerFrame = this.animationInterval(definition),
      life = definition.frames * animationTicksPerFrame * STEP;
  ...
  private missileTargetHeight(target: Entity | undefined): number {
    ...
    const airborne =
      target &&
      this.defs[target.type].movement === 'air' &&
      !target.landed &&
      target.rearm === undefined;
  ...
  private tickMissiles(dt: number): void {
    ...
    for (const effect of this.state.effects.slice()) {
      const flight = effect.missile;

      if (!flight || !effect.to) continue;

      const target = this.state.entities.find(
        (e) => e.id === flight.targetId && e.hp > 0 && !e.selling && e.transportId === undefined,
      );
      ...
      const dx = effect.to.x - effect.x,
        dy = effect.to.y - effect.y,
      ...
      effect.x += dx * fraction;
      effect.y += dy * fraction;
      effect.height =
        (effect.height ?? 0) + (flight.targetHeight - (effect.height ?? 0)) * fraction;

      if (fraction < 1) continue;

      const hit = flight.groundTarget
        ? this.state.entities.find(
            (e) =>
              e.hp > 0 &&
              !e.selling &&
              e.transportId === undefined &&
              this.distanceToEntity(effect.to!, e) < 0.5,
          )
        : target;
  ...
  private fireProjectile(
    ...
    if (
      def.ability === 'carrier' &&
      (!target || this.distanceToEntity(this.center(entity), target) > def.range)
    )
      return;
    ...
    if (def.category === 'infantry') {
      const swimming =
        entity.type === 'tanya' &&
        this.state.tiles[Math.floor(entity.y) * this.state.width + Math.floor(entity.x)]
          ?.terrain === 'water';

      const sequence = swimming
        ? 'WetAttack'
        : entity.deployed
          ? 'DeployedFire'
          : entity.type === 'rocketeer'
            ? 'FireFly'
            : 'FireUp';

      const infantry = this.infantryAnimations[def.sprite],
        definition = infantry?.sequences[sequence];

      if (!definition)
        throw new Error(`Missing original infantry timing: ${def.sprite} ${sequence}`);
      ...
      if (
        this.state.time + 1e-9 <
        memory.fireIntentAt + infantry.fireFrame * this.animationInterval(definition) * STEP
      )
        return;
      memory.fireIntentAt = undefined;
    }
    ...
    if (def.ability === 'carrier') {
      if (
        target &&
        this.state.entities.filter((e) => e.type === 'hornet' && e.homeId === entity.id && e.hp > 0)
          .length < 3
      ) {
        const plane = this.spawn('hornet', entity.side, entity.x, entity.y);
        plane.rank = entity.rank;
        plane.homeId = entity.id;
        plane.targetId = target.id;
        plane.order = 'attack';
      }
    ...
    const burst = def.burst ?? 1,
      nextBurstIndex = (memory.burstIndex ?? 0) + 1;

    entity.cooldown =
      nextBurstIndex < burst
        ? this.randomCombatFrames(3, 5) * STEP
        : fireRate + this.randomCombatFrames(0, 2) * STEP;
    ...
    this.state.effects.push({
      ...
      passengerType: entity.type === 'ifv' ? entity.passengers?.[0]?.type : undefined,
      deployed: entity.deployed,
      airTarget: !!target && this.defs[target.type].movement === 'air',
      startedAt: this.state.time,
      damage,
    });

    if (def.ability === 'prism') {
      const origin = this.center(entity),
        facing = entity.turretFacing ?? entity.facing;
      ...
      const tower = isBuilding(def),
        forward = tower ? 0 : 48 / 256;

      this.addPrismBeam(
        { x: origin.x + Math.cos(facing) * forward, y: origin.y + Math.sin(facing) * forward },
        to,
        entity.side,
        tower ? (378 / 256) * 30 + 4 : (184 / 256) * 30,
      );
    }

    if (def.projectile === 'DRAGON') {
      const facing = entity.turretFacing ?? entity.facing,
        origin = this.center(entity),
        flh = IFV_MISSILE_FLH;
      ...
      const height = flh.height,
        groundTarget = !!memory.groundTarget;
      ...
      this.state.effects.push({
        ...
        startedAt: this.state.time,
        missile: {
          image: def.projectile,
          targetId: groundTarget ? undefined : target?.id,
          groundTarget,
          impact: def.impact,
          verses: verses?.slice(),
          previous: { x, y, height },
          targetHeight: this.missileTargetHeight(groundTarget ? undefined : target),
          facing,
          trail: [],
        },
      });
      ...
      this.damageTarget(
        entity,
        target,
        entity.type === 'attack_dog' ? target.maxHp : damage,
        verses,
      );
        ...
        for (const other of this.state.entities
          .filter(
            (e) =>
              ...
              e.side >= 0 &&
              e.hp > 0 &&
              this.canTarget(entity, e) &&
              distance(this.center(e), to) < 2,
          )
          .slice(0, 3)) {
          this.addPrismBeam(to, this.center(other), entity.side, 0, true);
          this.damageTarget(entity, other, damage * 0.5, verses);
        }

      if (def.ability === 'chrono' && target.hp > 0)
        target.disabledUntil = this.state.time + fireRate + 0.2;
    }
    ...
    if (def.ammo) {
      ...
      if (!entity.ammo) {
        memory.returnTarget = entity.targetId ?? undefined;
        entity.targetId = null;
        entity.path = [];
      }
    }

    if (def.impact && !def.projectile)
      this.addAnimation(def.impact, to, entity.side, 'impact', damage);
  }
  ...
  private updateFacing(entity: Entity): void {
    ...
    entity.facing = body.facing;
    memory.bodyTurn = body.turn;

    if (def.turret) {
      if (!entity.targetId && !memory.groundTarget && entity.path.length)
        memory.turretAim = memory.heading;

      const turret = turnFacing(
        entity.turretFacing ?? entity.facing,
        memory.turretAim ?? entity.turretFacing ?? entity.facing,
        def.rot ?? 0,
        memory.turretTurn,
      );

      entity.turretFacing = turret.facing;
      memory.turretTurn = turret.turn;
    }
  }

  private damageTarget(
    attacker: Pick<Entity, 'side'>,
    target: Entity,
    damage: number,
    verses?: number[],
  ): void {
  ...
  private nearestFiringPosition(entity: Entity, target: Entity, range: number): Vec2 | undefined {
    const c = this.center(target),
      radius = Math.ceil(range + Math.max(...this.defs[target.type].footprint));
    ...
    for (let y = Math.floor(c.y) - radius; y <= Math.floor(c.y) + radius; y++)
      for (let x = Math.floor(c.x) - radius; x <= Math.floor(c.x) + radius; x++) {
        ...
        if (
          this.isPassable(x, y, entity) &&
          !this.mobileOccupies(x, y, entity.id, entity) &&
          this.distanceToEntity(p, target) <= range
        )
          candidates.push(p);
      }
    ...
    for (const candidate of candidates.slice(0, 32))
      if (distance(entity, candidate) < 0.2 || this.path(entity, candidate).length)
        return candidate;
  ...
  private operational(entity: Entity): boolean {
    ...
    return (
      ...
      !entity.constructing &&
      entity.transportId === undefined &&
      (entity.disabledUntil ?? 0) <= this.state.time &&
      (!def.powered || (!!side && side.power >= side.powerUsed))
    );
  }
  private produces(producer: Entity, def: UnitDef): boolean {
    ...
    return (
      !!this.defs[producer.type].producer?.includes(def.category) &&
      (def.category !== 'vehicles' || !['radar', 'shipyard'].includes(producer.type))
    );
  }
  private finishConstruction(entity: Entity): void {
    if (entity.type === 'refinery' || entity.type === 'refinery_soviet')
      this.spawnFrom(entity, entity.type === 'refinery' ? 'miner' : 'warminer');

    if (entity.side === 0)
      this.notify(`${this.defs[entity.type].name} construction complete`, 'success');
  }
  ...
  private weaponFor(entity: Entity): UnitDef {
    const def = this.defs[entity.type],
      passenger = entity.type === 'ifv' ? entity.passengers?.[0] : undefined;

    return passenger && IFV_WEAPONS[passenger.type]
      ? { ...def, ...IFV_WEAPONS[passenger.type], projectile: undefined }
      : def;
  }
  private canTarget(attacker: Entity, target: Entity): boolean {
    const def = this.weaponFor(attacker),
      other = this.defs[target.type];

    if (
      target.transportId !== undefined ||
      target.hp <= 0 ||
      (!def.damage && def.ability !== 'spy' && attacker.type !== 'engineer')
    )
      return false;
    ...
    if (
      other.ability === 'mirage' &&
      !target.factoryExit &&
      target.side !== attacker.side &&
      !target.path.length &&
      this.state.time - (target.firedAt ?? -Infinity) > 3 &&
    ...
    return (
      def.targets ?? (attacker.type === 'flak' ? ['land', 'water', 'air'] : ['land', 'water'])
    ).some(
      (kind) =>
        kind === layer || (kind === 'infantry' && other.category === 'infantry' && layer !== 'air'),
    );
  }
  private deployMcv(entity: Entity): void {
    const x = Math.floor(entity.x) - 1,
      y = Math.floor(entity.y) - 1,
      def = this.defs.conyard;

    for (let dy = 0; dy < def.footprint[1]; dy++)
      for (let dx = 0; dx < def.footprint[0]; dx++) {
        const tx = x + dx,
          ty = y + dy;

        if (
          !this.isPassable(tx, ty) ||
          this.mobileOccupies(tx, ty, entity.id, entity) ||
          !this.state.explored[ty * this.state.width + tx] ||
          this.state.tiles[ty * this.state.width + tx].ore
        ) {
          this.notify('Move the MCV to clear, explored ground before deploying.', 'warning');
  ...
  canEnterTransport(entity: Entity, transport: Entity): boolean {
    const unit = this.defs[entity.type],
      def = this.defs[transport.type];

    if (
      entity.id === transport.id ||
      entity.side !== transport.side ||
      entity.hp <= 0 ||
      entity.selling ||
      entity.constructing ||
      entity.factoryExit ||
      entity.transportId !== undefined ||
      !this.operational(transport) ||
      !def.passengers ||
      isBuilding(unit) ||
      unit.movement === 'air' ||
      unit.movement === 'water' ||
      unit.passengers ||
      (def.infantryOnly && unit.category !== 'infantry')
    )
      return false;

    const load = (transport.passengers ?? []).reduce(
      (n, p) => n + (this.defs[p.type].size ?? 1),
      0,
    );
  ...
  enterTransport(ids: number[], targetId: number): boolean {
    const transport = this.state.entities.find(
      (e) => e.id === targetId && e.side === 0 && this.operational(e),
    );
    ...
    for (const entity of this.commandable(ids, 0)) {
      ...
      this.issueMove([entity], transport, false);
      this.memory(entity).transportTarget = transport.id;
      assigned = true;
    }
  ...
  private unloadTransport(entity: Entity): void {
    ...
    for (const passenger of entity.passengers ?? []) {
      ...
      passenger.transportId = undefined;
      passenger.x = spot.x;
      passenger.y = spot.y;
      passenger.previous = { ...spot };
      passenger.order = 'guard';
      passenger.path = [];
      this.memories.delete(passenger.id);
    }
    ...
    if (remaining.length)
      this.notify('Move the transport closer to clear land to unload.', 'warning');
  }
  private infiltrate(spy: Entity, target: Entity): void {
    const side = this.state.sides[spy.side],
      enemy = this.state.sides[target.side];

    if (!enemy || target.side === spy.side) return;

    if (target.type.startsWith('refinery')) {
      const credits = Math.floor(enemy.money / 2);
      enemy.money -= credits;
      side.money += credits;
    } else if (this.defs[target.type].power > 0) {
      for (const e of this.state.entities)
        if (e.side === target.side && this.defs[e.type].power > 0)
          e.disabledUntil = this.state.time + 60;
    } else if (this.defs[target.type].producer?.includes('infantry'))
      this.veteranTraining.add(`${spy.side}:infantry`);
    else if (this.defs[target.type].producer?.includes('vehicles'))
      this.veteranTraining.add(`${spy.side}:vehicles`);
    else if (target.type.startsWith('radar')) {
      this.vision[target.side]?.fill(0);
      target.disabledUntil = this.state.time + 60;
    } else target.disabledUntil = this.state.time + 60;
    target.infiltrated = true;
    this.removeEntity(spy, false);
    this.updatePower();
  ...
  private tickEliteHealing(entity: Entity, dt: number): void {
    ...
    entity.selfHealTimer =
      entity.hp === entity.maxHp ? 0 : entity.selfHealTimer - ELITE_HEAL.seconds;
  }
  ...
  private tickAllied(entity: Entity, dt: number): boolean {
    ...
    if (def.recharge && this.operational(entity))
      entity.recharge = Math.min(def.recharge, (entity.recharge ?? 0) + dt);

    if (memory.transportTarget !== undefined) {
      const transport = this.state.entities.find(
        (e) => e.id === memory.transportTarget && e.side === entity.side && this.operational(e),
      );

      if (!transport || !this.canEnterTransport(entity, transport)) {
        memory.transportTarget = undefined;
      ...
      if (distance(entity, transport) < 2) {
        ...
        entity.transportId = transport.id;
        entity.selected = false;
        entity.path = [];
        entity.targetId = null;
        memory.transportTarget = undefined;

        return entity.transportId !== undefined;
      }
    ...
    if (entity.landed) {
      const home = this.state.entities.find(
        (e) => e.id === entity.homeId && e.side === entity.side && e.hp > 0 && !e.selling,
      );

      if (!home) {
        entity.landed = false;
        entity.rearm = undefined;
      } else if (entity.ammo !== 0) {
        if (!entity.path.length && entity.targetId === null && entity.order !== 'attack')
          return true;
        entity.landed = false;
      }
    }
    ...
    if (def.ammo && entity.ammo === 0) {
      let home = this.state.entities.find(
        (e) => e.id === entity.homeId && e.side === entity.side && this.operational(e),
      );
      ...
      if (def.factory === 'radar') {
        ...
        if (!home || entity.airfieldPad === undefined) {
          home = this.state.entities.find(
            (e) =>
              e.type === 'radar' &&
              e.side === entity.side &&
              this.operational(e) &&
              this.freeAirfieldPad(e, entity.id) !== undefined,
          );
        ...
        home ??= this.state.entities.find(
          (e) => e.type === 'radar' && e.side === entity.side && this.operational(e),
        );

      if (!home) {
        const unpowered = this.state.entities.find(
          (e) =>
            e.type === 'radar' &&
            e.side === entity.side &&
            e.hp > 0 &&
      ...
      const point =
        def.factory === 'radar'
          ? airfieldPad(this.center(home), entity.airfieldPad!)
          : this.center(home);

      if (distance(entity, point) > (def.factory === 'radar' ? 0.15 : 0.6)) {
        entity.landed = false;
        entity.rearm = undefined;

        if (entity.order !== 'return' || !entity.path.length) {
          entity.path = [point];
          entity.order = 'return';
        }
      } else {
        ...
        entity.path = [];
        entity.rearm = (entity.rearm ?? 0) + dt;

        if (entity.type === 'hornet') {
          this.removeEntity(entity, false);
    ...
    if (def.category === 'vehicles' && def.movement !== 'air' && entity.hp < entity.maxHp) {
      const repairer = this.state.entities.find(
        (e) =>
          e.side === entity.side &&
          this.operational(e) &&
          ((e.type === (def.movement === 'water' ? 'shipyard' : 'service_depot') &&
            this.distanceToEntity(entity, e) < 1.6) ||
            (e.type === 'ifv' &&
              e.passengers?.some((p) => p.type === 'engineer') &&
              distance(e, entity) < 3)),
      );

      if (repairer) {
        const side = this.state.sides[entity.side],
          perHp = (def.cost / def.hp) * 0.25,
          hp = Math.min(entity.maxHp - entity.hp, 20 * dt, side.money / perHp);
  ...
  activateSuperweapon(
    ...
    if (this.state.winner !== null || ![destination.x, destination.y].every(Number.isFinite))
      return false;
    ...
    if (
      ...
      x >= this.state.width ||
      y >= this.state.height ||
      (side === 0 && !this.state.explored[y * this.state.width + x])
    )
      return false;

    const building = this.state.entities.find(
      (e) =>
        e.side === side &&
        this.defs[e.type].superweapon === type &&
        this.operational(e) &&
        (e.recharge ?? 0) + 1e-8 >= this.defs[e.type].recharge!,
    );
    ...
    if (type === 'chronosphere') {
      ...
      if (
        ...
        (side === 0 && !this.state.explored[sy * this.state.width + sx])
      )
        return false;
      ...
      const units = this.state.entities.filter(
        (e) =>
          e.hp > 0 &&
          !e.transportId &&
          this.defs[e.type].category === 'vehicles' &&
          this.defs[e.type].movement !== 'air' &&
          distance(e, source) <= 2.5,
      );

      const reserved = new Set<number>(),
        moves: { unit: Entity; spot: Vec2 }[] = [];

      for (const unit of units) {
        const desired = {
          x: destination.x + unit.x - source.x,
          y: destination.y + unit.y - source.y,
        };

        const spot = this.nearestOpen(desired, unit.id, reserved, 3);

        if (!spot) continue;
        reserved.add(Math.floor(spot.y) * this.state.width + Math.floor(spot.x));
        moves.push({ unit, spot });
      }

      if (!moves.length) {
        this.notify('Choose vehicles with a clear destination.', 'warning');
      ...
      for (const { unit, spot } of moves) {
        ...
        unit.disabledUntil = this.state.time + 2;
        this.memories.delete(unit.id);
      }
    } else
      this.storms.push({
        point: { ...destination },
        side,
        starts: this.state.time + 250 / 30,
        ends: this.state.time + 430 / 30,
        next: this.state.time + 250 / 30,
      });
    building.recharge = 0;
    this.updateVision();
    this.notify(
      type === 'weather' ? 'Lightning storm approaching.' : 'Chronosphere activated.',
      'warning',
    );
  ...
  private tickStorms(): void {
    for (const storm of this.storms) {
      ...
      storm.next += 10 / 30;
      const attacker = { side: storm.side };

      for (const target of this.state.entities)
        if (
          target.hp > 0 &&
          target.transportId === undefined &&
          distance(this.center(target), storm.point) <= 3.5
        )
          this.damageTarget(attacker, target, 250);
      this.addAnimation('TWLT070', storm.point, storm.side, 'explosion');
      this.addAnimation(
        `WCLBOLT${1 + (this.randomCombatValue() % 3)}`,
        storm.point,
        storm.side,
        'impact',
      );
  ...
  private updateAI(): void {
    ...
    const owns = (type: string) =>
      this.state.entities.some((e) => e.side === 1 && e.type === type && e.hp > 0);

    const count = (type: string) =>
      this.state.entities.filter((e) => e.side === 1 && e.type === type && e.hp > 0).length;

    for (const category of ['structures', 'defenses'] as const) {
    ...
    if (
      count('sentry') < 3 &&
      !side.queues.defenses.length &&
      side.money > 2200 &&
      this.canBuild('sentry', 1).ok
    )
      this.build('sentry', 1);

    const army = this.state.entities.filter(
      (e) => e.side === 1 && !isBuilding(this.defs[e.type]) && !this.defs[e.type].harvester,
    );
    ...
    if (
      side.money > reserve &&
      side.queues.infantry.length < 2 &&
      army.length < 26 &&
      this.canBuild('conscript', 1).ok
    )
      this.build('conscript', 1);

    if (side.queues.vehicles.length < 1 && owns('warfactory_soviet')) {
      const wanted =
        count('warminer') < 2
          ? 'warminer'
          : this.wave % 3 === 2 && count('flak') < 3
            ? 'flak'
            : 'rhino';

      if (
        army.length < 28 &&
        side.money > (wanted === 'warminer' ? 1400 : 1100) &&
        this.canBuild(wanted, 1).ok
      )
        this.build(wanted, 1);
    }
    ...
    if (this.automaticSovietWaves && this.state.time >= this.nextAttack) {
      ...
      if (attackers.length >= 4) {
        ...
        const playerBase = this.state.entities.find(
          (e) =>
            e.side === 0 && isBuilding(this.defs[e.type]) && !this.defs[e.type].mapOnly && e.hp > 0,
        );

        const destination =
          this.nativeMap && playerBase
            ? this.center(playerBase)
            : { x: 16 + (this.wave % 3) * 2, y: 42 };
    ...
    if (side.money > 300) {
      for (const e of this.state.entities)
        if (e.side === 1 && isBuilding(this.defs[e.type]) && e.hp < e.maxHp * 0.8)
          this.repairs.add(e.id);
    }
  }
  ...
  private findAIPlacement(type: string): Vec2 | undefined {
    ...
    for (
      let y = nativeStart ? Math.max(1, anchor.y - 22) : 4;
      y < (nativeStart ? Math.min(this.state.height - 1, anchor.y + 23) : 31);
      y++
    )
      for (
        let x = nativeStart ? Math.max(1, anchor.x - 22) : 34;
        x < (nativeStart ? Math.min(this.state.width - 1, anchor.x + 23) : 59);
        x++
      )
        if (this.canPlace(type, x, y, 1)) candidates.push({ x, y });
  ...
  private spawn(type: string, side: number, x: number, y: number): Entity {
    ...
    const entity: Entity = {
      ...
      hp: def.hp,
      maxHp: def.hp,
      facing: side === 0 ? -Math.PI / 4 : Math.PI * 0.75,
      path: [],
      targetId: null,
      cooldown: 0,
      order: def.harvester ? 'harvest' : 'guard',
      cargo: 0,
      harvestTimer: 0,
      selected: false,
      anim: 0,
      previous: { x, y },
      previousFacing: side === 0 ? -Math.PI / 4 : Math.PI * 0.75,
    };

    if (def.turret) {
      entity.turretFacing = entity.facing;
      entity.previousTurretFacing = entity.facing;
    }
    ...
    if (
      side === 1 &&
      def.producer?.some((category) => category === 'infantry' || category === 'vehicles')
    )
      entity.rally = this.nativeMap
        ? { x: x + def.footprint[0] / 2, y: y + def.footprint[1] + 2.5 }
        : { x: 46.5, y: 27.5 };
  ...
  private spawnFrom(producer: Entity, type: string): Entity | undefined {
    if (isWarFactory(producer.type) && this.defs[type].movement !== 'air') {
      ...
      if (
        this.state.entities.some((e) => e.hp > 0 && e.factoryExit?.factoryId === producer.id) ||
        !this.isPassable(Math.floor(end.x), Math.floor(end.y), this.defs[type]) ||
        this.mobileOccupies(Math.floor(end.x), Math.floor(end.y), -1, this.defs[type])
      )
        return undefined;
      ...
      unit.factoryExit = {
        ...
        rally: producer.rally && { ...producer.rally },
      };
      unit.order = 'move';
    ...
    if (this.defs[type].factory === 'radar') {
      ...
      unit.homeId = producer.id;
      unit.airfieldPad = pad;
    ...
    const target = {
      x: producer.x + def.footprint[0] * 0.5,
      y: producer.y + def.footprint[1] + 0.5,
    };
  ...
  private tickFactoryExit(unit: Entity, dt: number): void {
    const exit = unit.factoryExit!,
      def = this.defs[unit.type];
    ...
    const nx = Math.min(exit.end.x, unit.x + def.speed * dt),
      ny = exit.end.y;
      ...
      this.state.entities
        .filter(
          (e) =>
            ...
            e.side === home.side &&
            e.homeId === home.id &&
            this.defs[e.type].factory === 'radar',
        )
        .map((e) => e.airfieldPad),
    );
  ...
  private removeEntity(entity: Entity, explosion: boolean): void {
    ...
    if (explosion) {
      ...
      if (choices?.length)
        this.addAnimation(
          choices[this.randomCombatValue() % choices.length],
          this.center(entity),
          entity.side,
          'explosion',
        );
      ...
      if (entity.side === 0)
        this.notify(
          `${this.defs[entity.type].name} lost`,
          'warning',
          isBuilding(this.defs[entity.type]) ? undefined : 'EVA_UnitLost',
        );
    }

    for (const passenger of entity.passengers ?? []) {
      passenger.hp = 0;
      this.removeEntity(passenger, false);
    }
      ...
      for (let y = Math.floor(entity.y); y < entity.y + def.footprint[1]; y++)
        for (let x = Math.floor(entity.x); x < entity.x + def.footprint[0]; x++)
          if (x >= 0 && y >= 0 && x < this.state.width && y < this.state.height)
            this.blocked[y * this.state.width + x] = 1;
    }
  }
  ...
  private isPassable(x: number, y: number, unit?: Entity | UnitDef): boolean {
    ...
    const index = y * this.state.width + x,
      terrain = this.state.tiles[index].terrain;
    ...
    return (
      ...
      (def?.movement === 'water'
        ? terrain === 'water'
        : terrain !== 'rock' && (def?.movement === 'amphibious' || terrain !== 'water'))
    );
  }
  ...
  private mobileOccupies(x: number, y: number, except: number, unit?: Entity | UnitDef): boolean {
    ...
    return this.state.entities.some(
      (e) =>
        ...
        e.hp > 0 &&
        e.transportId === undefined &&
        (this.defs[e.type].movement === 'air') === airborne &&
        !isBuilding(this.defs[e.type]) &&
        Math.floor(e.x) === x &&
        Math.floor(e.y) === y,
    );
  }
  ...
  private path(entity: Entity, destination: Vec2, avoidUnits = false): Vec2[] {
    if (this.defs[entity.type].movement === 'air' || this.defs[entity.type].movement === 'teleport')
      return this.isPassable(Math.floor(destination.x), Math.floor(destination.y), entity)
        ? [{ ...destination }]
        : [];
    ...
    if (avoidUnits)
      for (const other of this.state.entities)
        if (
          ...
          occupied.add(Math.floor(other.y) * this.state.width + Math.floor(other.x));
    ...
    return findPath(
      ...
      entity,
      destination,
      (x, y) => this.isPassable(x, y, entity) && !occupied.has(y * this.state.width + x),
    );
  }
  ...
  private nearestOpen(
    ...
    const cx = Math.floor(destination.x),
      cy = Math.floor(destination.y);
    ...
    for (let radius = 0; radius <= maxRadius; radius++) {
      ...
      for (let dy = -radius; dy <= radius; dy++)
        for (let dx = -radius; dx <= radius; dx++) {
          ...
          if (
            !this.isPassable(x, y, unit) ||
            reserved?.has(y * this.state.width + x) ||
            this.mobileOccupies(x, y, entityId, unit)
          )
            continue;
      ...
      if (points.length)
        return points.sort((a, b) => distance(a, destination) - distance(b, destination))[0];
    }

    return undefined;
  }

  private nearestReachableAround(
    entity: Entity,
    building: Entity,
    requirePath = true,
  ): Vec2 | undefined {
    const def = this.defs[building.type],
      points: Vec2[] = [],
      bx = Math.floor(building.x),
      by = Math.floor(building.y);

    for (let y = by - 1; y <= by + def.footprint[1]; y++)
      for (let x = bx - 1; x <= bx + def.footprint[0]; x++)
        if (this.isPassable(x, y, entity) && !this.mobileOccupies(x, y, entity.id, entity))
          points.push({ x: x + 0.5, y: y + 0.5 });
    points.sort((a, b) => distance(entity, a) - distance(entity, b));

    return points.find(
      (p) => !requirePath || distance(entity, p) < 0.2 || this.path(entity, p, true).length > 0,
    );
  }
  ...
  private updateVision(): void {
    ...
    for (const entity of this.state.entities) {
      ...
      const radius = this.defs[entity.type].sight,
        center = this.center(entity),
        fog = this.vision[entity.side];

      if (!fog) continue;

      for (
        let y = Math.max(0, Math.floor(center.y - radius));
        y <= Math.min(this.state.height - 1, Math.ceil(center.y + radius));
        y++
      )
        for (
          let x = Math.max(0, Math.floor(center.x - radius));
          x <= Math.min(this.state.width - 1, Math.ceil(center.x + radius));
          x++
        )
          if (Math.hypot(x + 0.5 - center.x, y + 0.5 - center.y) <= radius)
            fog[y * this.state.width + x] = 1;
    }

    for (let i = 0; i < this.state.fog.length; i++)
      if (this.state.fog[i]) this.state.explored[i] = 1;

    for (const entity of this.state.entities)
      if (isBuilding(this.defs[entity.type]) && revealedEntity(this.state, this.defs, entity))
  ...
  private visibleTo(entity: Entity, side: number): boolean {
    ...
    if (
      entity.side !== side &&
      this.gapGenerators.some(
        (e) => e.side === entity.side && distance(this.center(e), this.center(entity)) < 5,
      )
    )
      return false;

    const fog = this.vision[side],
      def = this.defs[entity.type];
  ...
  private checkVictory(): void {
    ...
    if (surviving.length < this.state.sides.length) {
      ...
      this.notify(
        this.state.winner === 0
          ? 'Mission accomplished. Soviet base eliminated.'
          : 'Mission failed. Your base has been destroyed.',
        this.state.winner === 0 ? 'success' : 'warning',
        this.state.winner === 0 ? 'EVA_MissionAccomplished' : 'EVA_MissionFailed',
      );
    }
  }
  ...
  private center(entity: Entity): Vec2 {
    ...
    return isBuilding(def)
      ? { x: entity.x + def.footprint[0] / 2, y: entity.y + def.footprint[1] / 2 }
  ...
  private distanceToEntity(point: Vec2, entity: Entity): number {
    ...
    return Math.hypot(
      Math.max(entity.x - point.x, point.x - (entity.x + def.footprint[0]), 0),
      Math.max(entity.y - point.y, point.y - (entity.y + def.footprint[1]), 0),
    );
  }
  private memory(entity: Entity): UnitMemory {
    ...
    if (!memory) {
      memory = { repath: 0, stuck: 0, idleTimer: 0 };
      this.memories.set(entity.id, memory);
    }
  ...
  private constrain(point: Vec2): Vec2 {
    return {
      x: clamp(point.x, 0, this.state.width - 0.001),
      y: clamp(point.y, 0, this.state.height - 0.001),
    };
  }

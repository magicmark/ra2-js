...
const theaterName = (value: string) =>
  value === 'SNOW' ? 'Snow' : value === 'URBAN' ? 'Urban' : 'Temperate';
...
export function startMapViewer() {
  ...
  try {
    remembered = localStorage.getItem(SOURCE_KEY) ?? '';
  } catch {
    /* Optional preference. */
  }

  let source = assetSourceUrl(
    new URLSearchParams(location.search).get('asset_url') || remembered || DEFAULT_ASSET_URL,
  );
  ...
  app.innerHTML = `<div class="map-shell">
    ...
    <aside class="map-sidebar" aria-label="Map selector"><label class="map-mobile-picker" for="map-mobile-select">Battlefield<select id="map-mobile-select">${MAP_CATALOG.map((entry) => `<option value="${entry.id}">${escape(entry.name)}</option>`).join('')}</select></label><div class="map-library-heading"><span class="map-eyebrow">BATTLEFIELDS</span><span class="map-count">${MAP_CATALOG.length.toString().padStart(2, '0')}</span></div><p class="map-library-intro">Six starting positions per map.</p>
      <nav class="map-list" aria-label="Choose a map">${MAP_CATALOG.map((entry) => `<a class="map-choice" href="/maps/${entry.id}" data-map-id="${entry.id}"><span class="map-choice-copy"><strong>${escape(entry.name)}</strong><small>${theaterName(entry.theater)} <span>·</span> <span data-map-size="${entry.id}">${entry.size.join(' × ')} full map</span></small></span><span class="map-choice-arrow" aria-hidden="true">↗</span></a>`).join('')}</nav>
  ...
  for (const link of app.querySelectorAll<HTMLAnchorElement>('[data-map-id]'))
    link.href = `/maps/${link.dataset.mapId}${location.search}`;

  // Native preview has no Game instance, game tick, AI, or visibility state.
  const canvas = el<HTMLCanvasElement>('map-viewer-canvas'),
  ...
  let selected: MapCatalogEntry | undefined,
    map: NativeMap | null = null,
    displayed: NativeMap | null = null;
  ...
  const diagnostic = {
    ...
    get ready() {
      return displayed === map && !!map;
    },
    get loading() {
      return mapLoading || assetBusy;
    },
    get error() {
      return currentError || assetError;
    },
    fogDisabled: true,
    simulation: false,
    fitMap: () => fitMap(),
  ...
  function updateStatus() {
    ...
    for (const id of ['map-fit', 'map-zoom-in', 'map-zoom-out'])
      el<HTMLButtonElement>(id).disabled = !displayed;
    el('map-status-text').textContent = assetBusy
      ? assets.status.message
      : `Loading ${selected?.name ?? 'map'}…`;
  }
    ...
    try {
      ...
      for (const marker of markers) {
        ...
        marker.button.hidden =
          p.x < 0 || p.y < 0 || p.x > renderer.camera.width || p.y > renderer.camera.height;
      }
    } catch (error) {
  ...
  function fitMap() {
    if (displayed) {
      renderer.fitNativeMap(displayed, Math.min(56, canvas.clientWidth * 0.07));
      fitted = true;
      redraw();
    }
  }
  ...
  function fail(message: string) {
    ...
    displayed = null;
    el('map-starts').replaceChildren();
    markers.length = 0;
    updateStatus();
  }

  function showMap() {
    if (!map || !assets.ready || assetBusy) {
      updateStatus();
    ...
    try {
      renderer.fitNativeMap(map, Math.min(56, canvas.clientWidth * 0.07));
      displayed = map;
      currentError = '';
      fitted = true;
      el('map-starts').replaceChildren();
      ...
      for (const start of map.starts) {
        ...
        button.className = 'map-start-marker';
        button.textContent = `${start.index + 1}`;
        button.setAttribute('aria-label', `Player start ${start.index + 1}`);
        button.title = `Player start ${start.index + 1} · ${start.x}, ${start.y}`;
        button.addEventListener('click', () => {
          renderer.camera.center(start.x, start.y);
          renderer.camera.setZoom(1);
          fitted = false;
          redraw();
          canvas.focus();
        });
        markers.push({ button, x: start.x, y: start.y });
        el('map-starts').append(button);
      }
  ...
  async function select(entry: MapCatalogEntry, navigation: 'push' | 'replace' | 'none' = 'push') {
    ...
    el('map-starts').replaceChildren();
    markers.length = 0;
    renderer.gl.begin(canvas.clientWidth || 1, canvas.clientHeight || 1);
    renderer.gl.flush();
    app.querySelector('.map-shell')!.classList.toggle('is-preview', previewMode);

    const pathname = `/maps/${entry.id}${previewMode ? '/preview' : ''}`,
      path = `${pathname}${location.search}`;

    if (navigation === 'replace') history.replaceState(null, '', path);
    else if (navigation === 'push' && location.pathname !== pathname)
      history.pushState(null, '', path);

    for (const link of app.querySelectorAll<HTMLAnchorElement>('[data-map-id]')) {
      const active = link.dataset.mapId === entry.id;
    ...
    el('map-preview-title').textContent = entry.name;
    el('map-preview-title').hidden = !previewMode;
    ...
    el('map-title').textContent = entry.name;
    el('map-description').textContent = entry.description;
    el('map-kicker').textContent = `${theaterName(entry.theater)} / ${entry.style}`;
    el('map-facts').innerHTML =
      `<span><strong>6</strong> player starts</span><span>Loading dimensions…</span><span class="map-fog-tag">Fog disabled</span>`;
    const preview = el<HTMLAnchorElement>('map-preview');
    preview.hidden = false;
    preview.href = `/maps/${entry.id}/preview${location.search}`;
    preview.title = 'Explore the full map with no fog or active match';
    const download = el<HTMLAnchorElement>('map-download');
    download.hidden = false;
    download.href = entry.path;
    download.download = `${entry.id}.map`;
    updateStatus();
    ...
    try {
      ...
      if (!loaded) {
        ...
        loaded = parseNativeMap(await response.text());
      ...
      const fullSize = map.size.slice(2).join(' × '),
        playableSize = map.localSize.slice(2).join(' × ');

      app.querySelector<HTMLElement>(`[data-map-size="${entry.id}"]`)!.textContent =
        `${fullSize} full map`;
      el('map-facts').innerHTML =
        `<span><strong>${map.starts.length}</strong> player starts</span><span><strong>${fullSize}</strong> full map</span><span><strong>${playableSize}</strong> playable area</span><span><strong>${theaterName(map.theater)}</strong> theater</span><span><strong>${map.structures.filter((value) => value.type.toUpperCase() === 'CAOILD').length}</strong> oil derricks</span><span><strong>${map.structures.filter((value) => value.type.toUpperCase() === 'CAAIRP').length}</strong> airports</span><span class="map-fog-tag">Fog disabled</span><span class="map-size-note">Full map includes the outer border. Dimensions use the map grid; isometric tiles appear twice as wide as tall.</span>`;
      showMap();
    } catch (error) {
      if (
        ticket === generation &&
  ...
  function route() {
    ...
    if (match && !match[1]) {
      void select(MAP_CATALOG[0], 'replace');
    ...
    mapLoading = false;
    previewMode = false;
    app.querySelector('.map-shell')!.classList.remove('is-preview');
    el('map-preview-title').hidden = true;
    el<HTMLAnchorElement>('map-back').href = '/maps/';
    el('map-back').textContent = '← Back to maps';
    renderer.gl.begin(canvas.clientWidth || 1, canvas.clientHeight || 1);
    renderer.gl.flush();
    document.title = 'Map not found · Map Viewer · Red Alert II';
    el('map-title').textContent = 'Map not found';
    el('map-description').textContent = 'Choose one of the eight battlefields in the sidebar.';
    el('map-preview').hidden = el('map-download').hidden = true;
    el('map-facts').replaceChildren();

    for (const link of app.querySelectorAll('[data-map-id]')) {
      link.classList.remove('is-selected');
      link.removeAttribute('aria-current');
    }

    el('map-retry').textContent = 'Open first map';
    fail('This map link does not match the library. Select a map to continue.');
  }
  ...
  async function loadAssets(files?: File[]) {
    ...
    try {
      ...
      if (files) await assets.importFiles(files, options);
      else await assets.download(options);
    } catch (error) {
      assetError = error instanceof Error ? error.message : String(error);
      el('map-assets-detail').textContent = assetError;
    } finally {
      assetBusy = false;
      showMap();
    }
  ...
  app.querySelector<HTMLElement>('.map-list')!.addEventListener('click', (event) => {
    if (!(event.target instanceof Element)) return;

    const click = event,
      link = event.target.closest<HTMLAnchorElement>('[data-map-id]');

    if (!link || click.button || click.ctrlKey || click.metaKey || click.shiftKey || click.altKey)
      return;
    event.preventDefault();
    el('map-retry').textContent = 'Retry map';
    void select(MAP_CATALOG.find((value) => value.id === link.dataset.mapId)!);
  });
  el<HTMLSelectElement>('map-mobile-select').addEventListener('change', () => {
    const entry = MAP_CATALOG.find(
      (value) => value.id === el<HTMLSelectElement>('map-mobile-select').value,
    );
  ...
  for (const id of ['map-preview', 'map-back'])
    el<HTMLAnchorElement>(id).addEventListener('click', (event) => {
      if (
        ...
        event.altKey ||
        !selected ||
        (id === 'map-back' && !previewMode)
      )
        return;
      event.preventDefault();
      previewMode = id === 'map-preview';
      void select(selected);

      if (previewMode) canvas.focus();
      else el('map-preview').focus();
    });
  window.addEventListener('popstate', route);
  el('map-retry').addEventListener('click', () => {
    if (selected) {
      cache.delete(selected.id);
      void select(selected, 'none');
    } else void select(MAP_CATALOG[0]);
  });
  el('map-assets-form').addEventListener('submit', (event) => {
    event.preventDefault();
    void loadAssets();
  });
  el<HTMLInputElement>('map-asset-files').addEventListener('change', () => {
    const input = el<HTMLInputElement>('map-asset-files'),
      files = [...(input.files ?? [])];

    input.value = '';

    if (files.length) void loadAssets(files);
  });

  const zoom = (factor: number, anchor?: { x: number; y: number }) => {
    if (!displayed) return;
    renderer.camera.setZoom(renderer.camera.zoom * factor, anchor);
    fitted = false;
    redraw();
  };
  ...
  canvas.addEventListener(
    ...
    (event) => {
      ...
      zoom(Math.exp(-Math.max(-120, Math.min(120, event.deltaY)) * 0.0025), {
  ...
  canvas.addEventListener('pointermove', (event) => {
    ...
    if (other) {
      ...
      if (before > 0)
        zoom(after / before, {
          x: (event.clientX + other.x) / 2 - rect.left,
          y: (event.clientY + other.y) / 2 - rect.top,
        });
  ...
  document.addEventListener('keydown', (event) => {
    ...
    const pan = { ArrowLeft: [50, 0], ArrowRight: [-50, 0], ArrowUp: [0, 50], ArrowDown: [0, -50] }[
      event.key
    ];

    if (pan) {
      event.preventDefault();
      renderer.camera.pan(pan[0], pan[1]);
      fitted = false;
      redraw();
    } else if (['f', 'F', '0', 'Home'].includes(event.key)) {
      event.preventDefault();
      fitMap();
  ...
  canvas.addEventListener('webglcontextlost', (event) => {
    event.preventDefault();
    fail('The graphics context was interrupted. Reload this page to restore the map.');
  });
  route();
  void assets
    .initialize({ url: source, nativeMaps: true, onProgress: progress })
    .catch((error) => {

...
export class Camera {
  ...
  private nativeBounds=new WeakMap<NativeMap,{left:number;right:number;top:number;bottom:number}>();
  ...
  private nativeLimits(map:NativeMap){
    const cached=this.nativeBounds.get(map);if(cached)return cached;
    ...
    const bounds={left:-(width-1)*30,right:(width-1)*30,top:(width+2)*15,bottom:(width+2*height-high)*15};
    this.nativeBounds.set(map,bounds);return bounds;
  }
  constrainedPosition(width:number,height:number,x=this.x,y=this.y,map?:NativeMap):Vec2{
    if(map){
      const b=this.nativeLimits(map),halfW=this.width/2/this.zoom,halfH=this.height/2/this.zoom;
      const clamp=(value:number,low:number,high:number)=>low>high?(low+high)/2:Math.max(low,Math.min(high,value));
      return {x:clamp(x,b.left+halfW,b.right-halfW),y:clamp(y,b.top+halfH,b.bottom-halfH)};
    }
    return this.project(Math.max(-3,Math.min(width+3,x/60+y/30)),Math.max(-3,Math.min(height+3,y/30-x/60)));
  }
  constrain(width:number,height:number,map?:NativeMap){
    if(map){
      const b=this.nativeLimits(map);
      this.minZoom=Math.max(.45,this.width/Math.max(1,b.right-b.left),this.height/Math.max(1,b.bottom-b.top));
      this.maxZoom=Math.max(2.4,this.minZoom);
    }else {this.minZoom=.45;this.maxZoom=2.4;}
    this.zoom=Math.max(this.minZoom,Math.min(this.maxZoom,this.zoom));
    const p=this.constrainedPosition(width,height,this.x,this.y,map);this.x=p.x;this.y=p.y;
  }
}

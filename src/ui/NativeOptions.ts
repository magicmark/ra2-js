...
export type OptionsScreen = 'options' | 'controls' | 'sound' | 'keyboard' | 'assets' | 'briefing' | 'abort';
...
export const parentOptionsScreen = (screen: OptionsScreen): OptionsScreen | null =>
  screen === 'sound' || screen === 'keyboard' ? 'controls' : screen === 'options' || screen === 'briefing' ? null : 'options';
const escape = (value: string) => value.replace(/[&<>"']/g, char => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[char]!);
const text = (value: string) => `<span data-native="${escape(value)}">${escape(value)}</span>`;
const nav = (screen: string, label: string, extra = '') => `<button type="button" class="native-menu-button" data-options-screen="${screen}" ${extra}>${text(label)}</button>`;
const slider = (id: string, label: string, max = 6) => `<div class="native-setting"><label for="${id}">${text(label)}</label><input type="range" id="${id}" min="0" max="${max}" step="1" aria-label="${label}" /><output for="${id}" id="${id}-value"></output></div>`;
const checkbox = (id: string, label: string) => `<label class="native-checkbox" id="${id}-row"><input id="${id}" type="checkbox" /><i aria-hidden="true"></i>${text(label)}</label>`;
...
export class NativeOptions {
  ...
  constructor(private root: HTMLElement, private game: GameAPI, private actions: UIActions, private hooks: OptionsHooks) {
    try { this.preferences = readOptionsPreferences(localStorage); } catch { /* Optional preferences never block play. */ }
    root.addEventListener('click', event => {
      const target = (event.target as Element).closest<HTMLElement>('[data-options-screen],[data-options-action]');
      if (!target) return;
      if (target.dataset.optionsScreen) this.show(target.dataset.optionsScreen as OptionsScreen);
      else switch (target.dataset.optionsAction) {
        case 'back': this.back(); break;
        case 'abort': this.hooks.close(); this.actions.onAbort?.(); break;
        case 'preview-sound': this.actions.onPreviewSound?.(); break;
        case 'assign': this.assign(); break;
        case 'reset-bindings': this.actions.onResetBindings?.(); this.refreshBindings(); this.hooks.bindingsChanged(); this.describeBinding('Default shortcuts restored.'); break;
      }
    });
    ...
    root.addEventListener('change', event => {
      ...
      if (input.id === 'target-lines') { this.actions.onTargetLines?.(input.checked); this.remember('targetLines',input.checked); }
      if (input.id === 'tooltips') { this.hooks.tooltips(input.checked); this.remember('tooltips',input.checked); }
      if (input.id === 'show-hidden') this.actions.onShowHidden?.(input.checked);
      if (input.id === 'keyboard-category') this.refreshCommands();
      if (input.id === 'keyboard-command') { this.selectedBinding = input.value; this.capturedKey = ''; this.input('binding-new').value = ''; this.describeBinding(); }
      this.update();
    });
    this.input('binding-new').addEventListener('keydown', event => {
      if (event.key === 'Tab' || event.key === 'Escape') return;
      event.preventDefault(); event.stopPropagation();
      if (event.ctrlKey || event.altKey || event.metaKey || event.key.length !== 1) { this.capturedKey = ''; this.input('binding-new').value = ''; this.assignButton().disabled = true; this.message('Choose a letter or ?; browser and navigation shortcuts are reserved.'); return; }
      this.capturedKey = event.key.toUpperCase(); this.input('binding-new').value = this.capturedKey;
      const result = this.actions.inspectBinding?.(this.selectedBinding, this.capturedKey);
      this.message(result?.ok ? result.conflict ? `Currently assigned to ${result.conflict.label}. Assign will replace it.` : 'Not currently assigned.' : result?.reason ?? 'Choose a command first.');
  ...
  private input(id: string) { return this.el<HTMLInputElement>(id); }
  private assignButton() { return this.root.querySelector<HTMLButtonElement>('[data-options-action="assign"]')!; }
  private paint(element: HTMLElement, value: string) { this.hooks.paint(element, value); }
  private remember<K extends keyof OptionsPreferences>(key: K, value: OptionsPreferences[K]) {
    this.preferences[key] = value;
    try { localStorage.setItem(OPTIONS_STORAGE_KEY,JSON.stringify(this.preferences)); } catch { /* Continue with session preferences. */ }
  }
  applyPreferences() {
  ...
  show(screen: OptionsScreen) {
    this.screen = screen; this.root.querySelector<HTMLElement>('.native-options')!.dataset.screen = screen;
    this.root.querySelectorAll<HTMLElement>('[data-options-page]').forEach(page => page.hidden = page.dataset.optionsPage !== screen);
    this.root.querySelectorAll<HTMLElement>('[data-options-nav]').forEach(nav => nav.hidden = nav.dataset.optionsNav !== screen);
    const titles: Record<OptionsScreen,string> = { options:'Game Options', controls:'Game Options', sound:'Sound Options', keyboard:'Keyboard', assets:'Game Files', briefing:'Briefing', abort:'Game Options' };
    this.paint(this.el('settings-title'), titles[screen]);
    this.paint(this.root.querySelector('.options-back')!, screen === 'options' ? 'Resume Mission' : screen === 'briefing' ? 'Return' : 'Back');
    this.root.querySelectorAll<HTMLElement>('[data-native]').forEach(label => this.paint(label,label.dataset.native!));
    this.el('options-help').textContent = screen === 'options' ? '' : screen === 'keyboard' ? 'Select a command, press a new key, then choose Assign.' : '';
    if (screen === 'keyboard') this.refreshBindings();
    this.update();
    this.root.querySelector<HTMLElement>(screen === 'controls' ? '#game-speed' : screen === 'sound' ? '#effects-volume' : screen === 'keyboard' ? '#keyboard-category' : screen === 'assets' ? '#asset-source' : '.options-back')?.focus({preventScroll:true});
  }
  back() { const parent = parentOptionsScreen(this.screen); if (parent) this.show(parent); else this.hooks.close(); }
  update() {
    ...
    this.input('game-speed').dataset.tooltip = 'Adjust the simulation speed. Fastest is capped at 4× in this browser version.';
    this.input('scroll-rate').dataset.tooltip = 'Adjust camera movement from arrows and screen edges.';
    ...
    this.root.querySelector<HTMLElement>('[data-options-action="preview-sound"]')!.hidden = !this.actions.onPreviewSound;
  }
  private refreshBindings() {
    ...
    const select = this.el<HTMLSelectElement>('keyboard-category'), previous = select.value;
  ...
  private refreshCommands() {
    ...
    const rows = this.bindingRows.filter(row=>row.category===category), select = this.el<HTMLSelectElement>('keyboard-command');
    ...
    this.selectedBinding = select.value; this.capturedKey=''; this.input('binding-new').value=''; this.describeBinding();
  }
  private describeBinding(message = '') {
    ...
    this.message(message); this.assignButton().disabled=true;
  }
  private message(message: string) { this.el('binding-assignment').textContent=message; }
  private assign() {
    if (!this.capturedKey) return;
    const result=this.actions.onAssignBinding?.(this.selectedBinding,this.capturedKey);
    if (!result?.ok) {this.message(result?.reason ?? 'Shortcut was not assigned.');return;}
    this.refreshBindings(); this.hooks.bindingsChanged(); this.describeBinding('Shortcut assigned.'); this.input('binding-new').focus();
  }
}

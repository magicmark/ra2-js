...
export class TooltipDelay {
  ...
  private target: Element | string | null = null;
  private x = 0;
  private y = 0;
  constructor(private hide: () => void) {}
  point(target: Element | string | null, x: number, y: number, show: () => void) {
...
export class Tooltips {
  ...
  constructor(
    ...
    this.box.className = 'native-tooltip';
    this.box.hidden = true;
    this.box.setAttribute('role', 'tooltip');
    document.body.append(this.box);
    this.delay = new TooltipDelay(() => {
      this.box.hidden = true;
    });
    ...
    for (const name of ['pointerdown', 'keydown', 'scroll'])
      document.addEventListener(name, () => this.clear(), { ...options, capture: true });
  ...
  private move(event: PointerEvent) {
    ...
    if (!this.enabled || event.pointerType === 'touch' || !this.hooks.allowed(element)) {
      this.clear();
    ...
    let key: Element | string | null = hint,
      content: TooltipContent | null = hint?.dataset.tooltip
        ? { title: hint.dataset.tooltip }
        : null;
    ...
    if (element.id === 'game-canvas') {
      ...
      content = entity;
      key = entity ? `entity:${entity.id}` : null;
    }
    ...
    this.delay.point(key, event.clientX, event.clientY, () => {
      ...
      for (const paragraph of [content!.title, content!.description]) {
        ...
        for (const line of paragraph.split('\n')) {
          ...
          const append = () => {
            ...
            this.hooks.paint(label, row);
            this.box.append(label);
          };

          for (const word of words) {
            if (row && row.length + word.length > 45) {
              append();
              row = '';
            }

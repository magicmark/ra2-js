  ...
  constructor(private game: GameAPI, private actions: UIActions, options: { mobile?: boolean; assetSource?: string } = {}) {
    ...
    this.root.innerHTML = `
      ...
      <nav class="mobile-commandbar" aria-label="Mobile commands">
        <span class="mobile-console-label" aria-hidden="true">TACTICAL CONTROL</span>
        <span class="mobile-console-screen" aria-hidden="true"></span>
        <button class="active" data-mode="select" aria-pressed="true" aria-label="Select units mode; cancel building placement">${icon('select')}<span>SELECT</span></button>
        <button data-mode="pan" aria-pressed="false" aria-label="Pan camera mode">${icon('pan')}<span>PAN</span></button>
        <button data-mode="attack" aria-pressed="false" aria-label="Attack move mode">${icon('target')}<span>ATTACK</span></button>

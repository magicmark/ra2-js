...
import { mobileCameo } from './MobileCameos';
  ...
  constructor(private game: GameAPI, private actions: UIActions, options: { mobile?: boolean; assetSource?: string } = {}) {
    ...
    this.root.innerHTML = `
      ...
      <nav class="mobile-commandbar" aria-label="Mobile commands">
        <button class="active" data-mode="select" aria-label="Select units mode; cancel building placement" aria-pressed="true">${mobileCameo('select')}<span class="command-caption">SELECT</span></button>
        <button data-mode="pan" aria-label="Pan camera mode" aria-pressed="false">${mobileCameo('pan')}<span class="command-caption">PAN</span></button>
        <button data-mode="attack" aria-label="Attack move mode" aria-pressed="false">${mobileCameo('attack')}<span class="command-caption">ATTACK</span></button>
        <button data-action="stop" aria-label="Stop selected units">${mobileCameo('stop')}<span class="command-caption">STOP</span></button>
        <button class="mobile-build-button" data-action="build-toggle" aria-expanded="false">${mobileCameo('build')}<span class="command-caption">BUILD</span><i id="mobile-ready-dot"></i></button>
      </nav>

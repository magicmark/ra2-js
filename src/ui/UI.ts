...
export class UI {
  ...
  constructor(private game: GameAPI, private actions: UIActions, options: { mobile?: boolean; assetSource?: string } = {}) {
    ...
    this.root.innerHTML = `
      ...
      <nav class="mobile-commandbar handset" aria-label="Mobile commands">
        <div class="handset-ident" aria-hidden="true"><b>★</b><span>FIELD COMMAND</span><i></i></div>
        <div class="handset-modes" role="group" aria-label="Command modes">
          <button class="active" data-mode="select" aria-label="Select units mode; cancel building placement" aria-pressed="true">${icon('select')}<span>SELECT</span></button>
          <button data-mode="pan" aria-label="Pan camera mode" aria-pressed="false">${icon('pan')}<span>PAN</span></button>
          <button data-mode="attack" aria-label="Attack move mode" aria-pressed="false">${icon('target')}<span>ATTACK</span></button>
        </div>
        <div class="handset-actions" role="group" aria-label="Unit and production actions">
          <button data-action="stop" aria-label="Stop selected units">${icon('stop')}<span>STOP</span></button>
          <button class="mobile-build-button" data-action="build-toggle" aria-expanded="false">${icon('structures')}<span>BUILD</span><i id="mobile-ready-dot"></i></button>
        </div>
      </nav>

...
export interface UIActions {
  ...
  onCommand?(
    ...
    options: { shift?: boolean; ctrl?: boolean; clear?: boolean },
  ): void;
  getPlanning?(): boolean;
  onRadar?(
    x: number,
    y: number,
    modifiers: { button?: number; shift?: boolean; ctrl?: boolean; alt?: boolean },
  ): void;
...
const COMMANDS: [ControlCommand, string][] = [
  ['team1', 'Team 1 · click to create or select; right click to disband'],
  ['team2', 'Team 2 · click to create or select; right click to disband'],
  ['type', 'Select units of the same type'],
  ['deploy', 'Deploy'],
  ['guard', 'Guard area'],
...
const SOURCE_HINT =
  'Press Enter to use saved game files, or load this URL if they are not available.';
...
const icons = {
  ...
  infantry:
    '<path d="M5 21v-4c0-3 3-5 7-5s7 2 7 5v4M8 7a4 4 0 0 1 8 0v2c0 2-2 3-4 3s-4-1-4-3ZM6 7h12M9 18v3M15 18v3"/>',
  ...
  settings:
    '<path d="m10 2 4 0 1 3 3 2 3-.2 2 3-2 3 0 3-3 2-3-.2-2 3-4 0-1-3-3-2-3 .2-2-3 2-3V9l3-2 3 .2z" transform="translate(1 0) scale(.91)"/><circle cx="12" cy="12" r="3"/>',
  ...
  radar:
    '<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4"/><path d="m12 12 7-7M12 1v3M1 12h3M12 20v3M20 12h3"/>',
...
const icon = (name: keyof typeof icons) =>
  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${icons[name] ?? icons.target}</svg>`;
...
export class UI {
  ...
  private assetStatus: AssetStatus = {
    ...
    detail: 'Choose your original game files to begin.',
    progress: null,
  };
  ...
  constructor(
    ...
    options: { mobile?: boolean; assetSource?: string } = {},
  ) {
    ...
    this.options = new NativeOptions(this.el('settings-modal'), this.game, this.actions, {
      ...
      tooltips: (enabled) => {
        if (enabled !== undefined) {
          ...
          this.tooltips?.setEnabled(enabled);
        }
    ...
    this.tooltips = new Tooltips({
      paint: (element, text) => this.nativeText(element, text),
      allowed: (element) =>
        !this.loading && (!this.isModalOpen() || !!element.closest('#settings-modal')),
      battlefield: (x, y) => this.actions.getBattlefieldTooltip?.(x, y) ?? null,
    });
    this.refreshBindingHints();
    this.root.addEventListener('click', (event) => this.handleClick(event));
    this.el('build-grid').addEventListener('scroll', () => this.updateScrollButtons(), {
      passive: true,
    });
    this.root.addEventListener('contextmenu', (event) => {
      if (!(event.target instanceof Element)) return;
      const command = event.target.closest<HTMLElement>('[data-command]')?.dataset.command;

      if (command) {
        event.preventDefault();

        if (!this.isModalOpen() && (command === 'team1' || command === 'team2'))
          this.actions.onCommand?.(command, { clear: true });

        return;
      }

      const card = event.target.closest<HTMLElement>('[data-build]');

      if (!card) return;
      event.preventDefault();
      const def = this.game.defs[card.dataset.build!];
    ...
    this.el<HTMLSelectElement>('startup-map-select').addEventListener('change', () => {
      const id = this.el<HTMLSelectElement>('startup-map-select').value,
        query = new URLSearchParams(location.search);

      if (id) query.set('map', id);
      else query.delete('map');
      location.assign(`/${query.size ? '?' + query.toString() : ''}`);
    });

    for (const [formId, inputId] of [
      ['startup-source-form', 'startup-asset-source'],
      ['settings-source-form', 'asset-source'],
    ]) {
      this.el<HTMLFormElement>(formId).addEventListener('submit', (event) => {
        event.preventDefault();
        event.stopPropagation();
        this.submitAssetSource(this.el<HTMLInputElement>(inputId));
      });
      this.el<HTMLInputElement>(inputId).addEventListener('keydown', (event) => {
        if (event.key === 'Enter' && event.repeat) event.preventDefault();
      });
      this.el<HTMLInputElement>(inputId).addEventListener('input', () => {
        const sourceInput = this.el<HTMLInputElement>(inputId);
        this.assetSourceEdited = true;
        ...
        this.el<HTMLInputElement>(otherId).removeAttribute('aria-invalid');
        this.el('startup-source-error').textContent = '';
        this.el('asset-error').textContent = this.assetStatus.cacheWarning ?? '';
        this.updateAssetSourceLabels();
      });
    }

    for (const id of ['asset-import', 'startup-asset-import'])
      this.el<HTMLInputElement>(id).addEventListener('change', () => {
        ...
        this.setAssetSubmitting(true);
        this.actions.onAssetImport(files);
      });
    ...
    minimap.addEventListener('pointerdown', (event) => {
      ...
      this.actions.onRadar?.(
        Math.max(0, Math.min(state.width - 1, (nx - 0.5 + ny) * state.width)),
        Math.max(0, Math.min(state.height - 1, (ny - nx + 0.5) * state.height)),
        { button: event.button, shift: event.shiftKey, ctrl: event.ctrlKey, alt: event.altKey },
      );
    });
  ...
  private refreshBindingHints() {
    ...
    const bindings = supplied?.length
      ? supplied
      : BINDING_DEFAULTS.map((row) => ({ ...row, key: row.defaultKey }));

    const byId = new Map<string, BindingInfo>(bindings.map((row) => [row.id, row]));
    ...
    const hint = (selector: string, id: string, label: string) => {
      ...
      this.root.querySelectorAll<HTMLElement>(selector).forEach((element) => {
        element.dataset.tooltip = `${label}${key ? ` · ${key}` : ''}`;
      });
    };

    for (const mode of ['repair', 'sell'])
      hint(`[data-mode="${mode}"]`, mode, `${mode === 'repair' ? 'Repair' : 'Sell'} mode`);

    for (const category of CATEGORIES)
      hint(`[data-category="${category}"]`, category, CATEGORY_NAMES[category]);
    hint('[data-action="help"]', 'briefing', 'Briefing');

    for (const [command, id, label] of [
      ['type', 'selectType', 'Select units of the same type'],
      ['deploy', 'deploy', 'Deploy'],
      ['guard', 'guard', 'Guard area'],
      ['planning', 'planning', 'Planning mode'],
    ])
      hint(`[data-command="${command}"]`, id, label);
    const help = this.el('binding-help');
    help.innerHTML = bindings
      .map(
        (row) =>
          `<div><span>${escapeHTML(row.label)}</span><kbd>${escapeHTML(row.key ?? 'Unassigned')}</kbd></div>`,
      )
      .join('');
  }
  private selected() {
    return this.game.state.entities.filter((entity) => entity.selected && entity.side === 0);
  }
  private centerBase() {
    const base =
      this.game.state.entities.find(
        (entity) =>
          entity.side === 0 &&
          /yard|conyard|construction/i.test(`${entity.type} ${this.game.defs[entity.type]?.name}`),
      ) ?? this.game.state.entities.find((entity) => entity.side === 0);
  ...
  private handleClick(event: MouseEvent) {
    if (!(event.target instanceof Element)) return;
    const target = event.target.closest<HTMLElement>('button, [data-action], [data-mode]');

    if (!target || !this.root.contains(target)) return;

    const category = CATEGORIES.find((category) => category === target.dataset.category);
    ...
    if (target.dataset.command) {
      const command = COMMANDS.find(([command]) => command === target.dataset.command)?.[0];

      if (command && !this.isModalOpen())
        this.actions.onCommand?.(command, { shift: event.shiftKey, ctrl: event.ctrlKey });
      this.update();
    ...
    if (target.dataset.mode) {
      // SAFETY: data-mode is authored here using the select/pan/attack/repair/sell ControlMode literals.
      const requested = target.dataset.mode as ControlMode;

      const mode =
        (requested === 'repair' || requested === 'sell') && this.mode === requested
          ? 'select'
          : requested;

      this.setMode(mode);
      this.actions.onMode(mode);
    ...
    if (target.dataset.superweapon) {
      ...
      this.showToast(
        mode === 'weather'
          ? 'Choose the lightning storm target.'
          : 'Choose the vehicles to chronoshift, then their destination.',
      );
    ...
    if (target.dataset.build) {
      ...
      const def = this.game.defs[type];
      const queue = this.game.state.sides[0].queues[def.category];
      const ready = queue.find((item) => item.type === type && item.ready);

      if (queue[0]?.type === type && queue[0].paused) {
        this.game.toggleBuildPause(def.category);
        this.update();
      ...
      if (ready) this.beginPlacement(type);
        ...
        else if (!this.game.build(type))
          this.showToast('Production unavailable. Check credits and requirements.');
    ...
    switch (target.dataset.action) {
      ...
      case 'build-up':
        this.el('build-grid').scrollBy({ top: -100, behavior: 'smooth' });
        break;
      case 'build-down':
        this.el('build-grid').scrollBy({ top: 100, behavior: 'smooth' });
        break;
      ...
      case 'pause':
        ...
        this.actions.onPause?.(this.game.state.paused);
        this.update();
        break;
      case 'settings':
        ...
        break;
      case 'help':
        this.openSettings('briefing');
        break;
      case 'close-settings':
        this.closeSettings();
        break;
      case 'zoom-in':
        this.actions.onZoom(0.15);
        break;
      case 'zoom-out':
        this.actions.onZoom(-0.15);
        break;
      case 'stop':
        this.game.stop(this.selected().map((entity) => entity.id));
        this.showToast('Selected units holding position.');
        break;
      case 'repair':
        for (const entity of this.selected()) this.game.repair(entity.id);
        break;
      case 'sell':
        for (const entity of this.selected()) this.game.sell(entity.id);
        this.lastSelection = '';
        this.renderSelection();
        break;
      case 'build-toggle': {
        const open = !document.documentElement.classList.contains('build-panel-open');
        document.documentElement.classList.toggle('build-panel-open', open);
        this.root
          .querySelector('.mobile-build-button')
          ?.setAttribute('aria-expanded', String(open));
        break;
      }

      case 'queue-pause':
        this.game.toggleBuildPause(this.category);
        this.update();
        break;
      case 'queue-cancel':
        this.game.cancelBuild(this.category);
        this.update();
        break;
      case 'restart':
        this.closeSettings();
        ...
        this.el('victory-panel').hidden = true;
        this.showToast('New operation initialized. Good luck, commander.');
        this.update();
        break;
    }
  }
  ...
  private onKey(event: KeyboardEvent) {
    ...
    if (this.loading) {
      ...
      if (event.key === 'Tab') {
        const focusable = [
          ...this.el('loading-screen').querySelectorAll<HTMLElement>(
  ...
  private beginPlacement(type: string) {
    ...
    this.showToast(
      `Place ${this.game.defs[type].name} near your base. ${this.mobile ? 'Select cancels.' : 'Escape cancels.'}`,
    );
    this.closeBuildPanel();
  }
  openOptions() {
    if (!this.loading) this.openSettings('options');
  }
  openBriefing() {
    if (!this.loading) this.openSettings('briefing');
  }
  isModalOpen() {
    return this.loading || !this.el('settings-modal').hidden;
  }

  private closeBuildPanel() {
    document.documentElement.classList.remove('build-panel-open');
    this.root.querySelector('.mobile-build-button')?.setAttribute('aria-expanded', 'false');
  }
  private openSettings(screen: OptionsScreen) {
    ...
    this.tooltips.clear();
    this.refreshBindingHints();
    this.options.show(screen);
  }
  private closeSettings() {
    this.tooltips.clear();
    this.el('settings-modal').hidden = true;
  ...
  setLoading(loading: boolean) {
    ...
    if (this.loading && this.el('settings-modal').hidden)
      this.el<HTMLInputElement>('startup-asset-source').focus({ preventScroll: true });
    else if (!this.loading && this.el('loading-screen').contains(document.activeElement))
  ...
  private updateAssetSourceLabels() {
    const normalize = (source: string) => {
      try {
        return assetCacheKey(new URL(source, location.href).href);
      } catch {
        return source;
      }
    };
    ...
    const saved =
      ...
      !!this.assetStatus.source &&
      normalize(this.el<HTMLInputElement>('startup-asset-source').value.trim()) ===
        normalize(this.assetStatus.source);

    const readyTitle = this.assetStatus.cacheWarning
      ? 'Game files are ready for this session'
      : 'Saved game files are ready';

    const readyHint = this.assetStatus.cacheWarning
      ? 'Press Enter to continue with the game files already loaded.'
      : 'Press Enter to continue with your saved game files.';

    this.el('startup-submit-label').textContent = saved ? 'Continue' : 'Load & play';
    this.el('startup-source-hint').textContent = saved
      ? `${readyTitle}. Press Enter to continue.`
      : SOURCE_HINT;
    ...
    if (this.loading && this.assetStatus.ready && !this.assetSubmitting) {
      ...
      this.el('loading-detail').textContent = saved
        ? readyHint
        : 'Press Enter to load the URL above. Saved files for this source are reused when available.';
    }
  }
  ...
  private setAssetSubmitting(submitting: boolean) {
    ...
    for (const id of ['startup-asset-source', 'asset-source'])
      this.el<HTMLInputElement>(id).readOnly = submitting;

    for (const id of ['startup-asset-import', 'asset-import'])
      this.el<HTMLInputElement>(id).disabled = submitting;
    this.root
      .querySelectorAll<HTMLButtonElement>('.asset-submit-button')
      .forEach((button) => (button.disabled = submitting));
    this.root
      .querySelectorAll<HTMLFormElement>('.source-form')
      .forEach((form) => form.setAttribute('aria-busy', String(submitting)));
  }
  ...
  private submitAssetSource(input: HTMLInputElement) {
    ...
    try {
      ...
      this.setAssetSubmitting(false);
      input.setAttribute('aria-invalid', 'true');
      const message = 'Enter a valid game archive URL to continue.';
      this.el('startup-source-error').textContent = message;
      this.el('asset-error').textContent = message;
  ...
  setFont(font: NativeFont) {
    this.font = font;
    this.nativeLabels = new WeakMap();
    this.options.applyPreferences();
    this.refreshBindingHints();
    this.update();
  }
  setCursors(cursors: NativeCursors) {
    ...
    this.root.classList.add('native-cursors');
    this.paintCursor();
  }
  setCursor(name: NativeCursorName) {
    if (name !== this.cursorName) {
      this.cursorName = name;
      this.cursorStarted = performance.now();
    }
  ...
  private paintCursor() {
    ...
    const style = this.isModalOpen()
      ? ''
      : this.cursors.css(this.cursorName, performance.now() - this.cursorStarted);
  ...
  showToast(text: string) {
    this.el('notifications').textContent = text;
    this.el('notifications').classList.add('visible');
    window.clearTimeout(this.toastTimer);
    this.toastTimer = window.setTimeout(
      () => this.el('notifications').classList.remove('visible'),
      4300,
    );
  }
  ...
  setAssetStatus(status: AssetStatus) {
    ...
    const readyTitle = status.cacheWarning
      ? 'Game files are ready for this session'
      : 'Saved game files are ready';

    const readyHint = status.cacheWarning
      ? 'Press Enter to continue with the game files already loaded.'
      : 'Press Enter to continue with your saved game files.';

    const label = status.error
      ? 'Could not prepare game files'
      : status.ready && this.loading
        ? readyTitle
        : status.ready
    ...
    this.el('loading-detail').textContent =
      status.ready && this.loading
        ? readyHint
        : waiting
          ? (status.detail ??
            'Press Enter to start. You can edit the URL or import your own game files.')
          : (status.error ?? status.detail ?? status.phase);
    this.el('startup-source-error').textContent = status.error ?? '';
    this.el('loading-progress').style.width =
      `${Math.max(0, Math.min(1, status.progress ?? 0)) * 100}%`;
    this.el('loading-progress').classList.toggle(
      'indeterminate',
      status.progress == null && !status.error && !status.ready && !waiting && this.assetSubmitting,
    );
    ...
    this.el('asset-detail').textContent = status.error
      ? 'Could not prepare game files. Retry loading or import your files.'
      : (status.detail ?? status.phase);
    this.el('download-progress').style.width = `${progress}%`;
    this.el('download-progress').classList.toggle(
      'indeterminate',
      status.progress == null && !status.error && !status.ready && !waiting && this.assetSubmitting,
    );
    this.el('asset-error').textContent = [status.error, status.cacheWarning]
      .filter(Boolean)
      .join(' ');
    ...
    if (
      status.source &&
      !this.assetSourceEdited &&
      document.activeElement !== this.el('asset-source') &&
      document.activeElement !== this.el('startup-asset-source')
    )
      this.setAssetSource(status.source);
    this.updateAssetSourceLabels();
  }

  private renderCards() {
    this.root.querySelectorAll<HTMLElement>('[data-category]').forEach((button) => {
    ...
    const defs = Object.values(this.game.defs).filter(
      (def) =>
        !def.mapOnly &&
        def.category === this.category &&
        (def.faction === faction || def.faction === 'both') &&
        def.cost > 0 &&
        def.id !== 'conyard' &&
        this.cameos.has(def.id),
    );

    if (faction === 'allied' && this.category === 'structures')
      defs.sort((a, b) => {
        const rank = (id: string) => {
          const index = ALLIED_STRUCTURE_ORDER.indexOf(id);
    ...
    this.el('build-grid')
      ...
      .forEach((card) => this.cardElements.set(card.dataset.build!, card));
    this.updateCardVisibility();
    this.updateScrollButtons();
  }

  private updateCardVisibility(
    owned = this.game.state.entities.filter(
      (entity) =>
        entity.side === 0 &&
        entity.hp > 0 &&
        !entity.selling &&
    ...
    const ownedTypes = new Set(owned.map((entity) => entity.type)),
      side = this.game.state.sides[0];

    let visible = 0;

    for (const [type, card] of this.cardElements) {
      const def = this.game.defs[type],
        building = def.category === 'structures' || def.category === 'defenses';
      ...
      const unlocked =
        def.requires.every((required) => ownedTypes.has(required)) &&
        owned.some((entity) =>
          def.factory
            ? entity.type === def.factory
            : this.game.defs[entity.type].producer?.includes(def.category),
        );
  ...
  private updateScrollButtons() {
    ...
    this.root.querySelector<HTMLButtonElement>('[data-action="build-up"]')!.disabled =
      grid.scrollTop <= 0.5;
    this.root.querySelector<HTMLButtonElement>('[data-action="build-down"]')!.disabled =
      grid.scrollTop >= grid.scrollHeight - grid.clientHeight - 0.5;
  }
  ...
  private renderSelection() {
    ...
    const signature =
      selected.map((entity) => `${entity.id}:${entity.type}`).join(',') + `:${inspection}`;
    ...
    if (!selected.length) {
      this.el('selection-panel').innerHTML =
        `<div class="briefing-icon">${icon('star')}</div><div class="briefing-copy"><span class="eyebrow">COMMANDER'S BRIEFING</span><h3>Expand your foothold.</h3><p>Keep your miners working. Build a strike force.<br/>The battlefield is yours to command.</p></div><button class="briefing-help" data-action="help" aria-label="Read the field manual">${icon('help')}</button>`;

      return;
    }

    const def = this.game.defs[selected[0].type],
      cameo = this.cameos.get(def.id);

    if (!cameo) {
      this.el('selection-panel').hidden = true;
      this.el('selection-panel').replaceChildren();

      return;
    }

    const building =
      selected.length === 1 && (def.category === 'structures' || def.category === 'defenses');

    this.el('selection-panel').innerHTML =
      `<img class="selection-cameo" src="${escapeHTML(cameo)}" alt=""/><div class="selection-copy"><span class="eyebrow">${selected.length > 1 ? `${selected.length} UNITS SELECTED` : `${CATEGORY_NAMES[def.category]} / FRIENDLY`}</span><h3>${escapeHTML(selected.length > 1 ? 'Strike group' : def.name)}</h3><div class="selection-health"><i id="selection-health-fill"></i></div><span class="selection-status" id="selection-status"></span></div><div class="selection-actions">${building ? `<button data-action="repair" data-tooltip="Repair selected building" aria-label="Repair selected building">${icon('wrench')}</button><button data-action="sell" data-tooltip="Sell selected building" aria-label="Sell selected building">${icon('sell')}</button>` : `<button data-action="stop" data-tooltip="Stop selected units" aria-label="Stop selected units">${icon('stop')}</button><button data-mode="attack" data-tooltip="Attack-move mode" aria-label="Attack move mode">${icon('target')}</button>`}</div>`;

    if (selected.length === 1 && selected[0].type === 'george')
      panel.querySelector('[data-mode="attack"]')?.remove();

    if (inspection) {
      panel
        .querySelector('.selection-copy')!
        .insertAdjacentHTML(
          'beforeend',
          '<progress class="inspection-progress" id="inspection-progress" max="1" value="0" aria-label="Continuous inspection progress"></progress><small class="inspection-help">Autonomous inspection · 3 seconds · no upgrades</small>',
        );
  ...
  private drawMinimap() {
    ...
    const point = (x: number, y: number) => ({
      x: ((x / state.width - y / state.height) / 2 + 0.5) * canvas.width,
      y: ((x / state.width + y / state.height) / 2) * canvas.height,
    });
    ...
    for (let y = 0; y < state.height; y++)
      for (let x = 0; x < state.width; x++) {
        ...
        const p = point(x, y);
        c.globalAlpha = 1;
        c.fillStyle = tile.ore > 0 ? '#dfbd3c' : colors[tile.terrain];
        c.fillRect(p.x, p.y, canvas.width / state.width + 0.5, canvas.height / state.height + 0.5);
      }
    ...
    if (corners?.length) {
      ...
      corners.forEach((corner, index) => {
        const p = point(corner.x, corner.y);

        if (index) c.lineTo(p.x, p.y);
  ...
  private renderQueue() {
    const queue = this.game.state.sides[0].queues[this.category],
      active = queue[0];

    const panel = this.el('queue-panel');

    if (!this.mobile) {
      if (panel.childElementCount) panel.replaceChildren();

      return;
    }

    if (!active) {
      if (!panel.querySelector('.queue-idle'))
        panel.innerHTML =
          '<div class="queue-idle"><span class="status-dot"></span> CONSTRUCTION QUEUE IDLE</div>';
      panel.classList.remove('has-production');
    ...
    panel.querySelector<HTMLElement>('.eyebrow')!.textContent = active.ready
      ? 'AWAITING DEPLOYMENT'
      : active.paused || active.blockedFunds || active.blockedPrerequisite
        ? 'PRODUCTION ON HOLD'
        : 'PRODUCTION IN PROGRESS';
    panel.querySelector<HTMLElement>('.active-queue strong')!.textContent =
      this.game.defs[active.type]?.name ?? active.type;
    const pauseButton = panel.querySelector<HTMLButtonElement>('[data-action="queue-pause"]')!;
    pauseButton.hidden = active.ready;
    pauseButton.setAttribute(
      'aria-label',
      `${active.paused ? 'Resume' : 'Pause'} current production`,
    );
    ...
    panel.querySelector<HTMLElement>('.queue-track i')!.style.width =
      `${Math.max(0, Math.min(1, active.progress)) * 100}%`;
    panel.querySelector<HTMLElement>('.queue-total')!.textContent =
      active.blockedPrerequisite ??
      (queue.length > 1
        ? `${queue.length - 1} more in queue`
        : active.ready
          ? 'Click the highlighted card to place'
  ...
  update() {
    ...
    this.el('power-resource').dataset.tooltip =
      `Power: ${side.power} generated / ${side.powerUsed} used`;

    const radarOnline =
      side.power >= side.powerUsed &&
      state.entities.some(
        (entity) =>
          entity.side === 0 &&
          entity.hp > 0 &&
          !entity.constructing &&
          !entity.selling &&
          entity.type.startsWith('radar'),
      );

    this.radarOnline = radarOnline;
    this.el('radar-frame').classList.toggle('online', radarOnline);
    this.el('minimap').setAttribute(
      'aria-label',
      radarOnline
        ? 'Tactical radar. Click to order selected units; with no selection, center the camera.'
        : 'Radar offline. Build an Airforce Command to activate.',
    );
    ...
    const friendly = state.entities.filter(
      (entity) =>
        entity.side === 0 &&
        entity.hp > 0 &&
        !entity.selling &&
    ...
    for (const category of CATEGORIES) {
      const queue = side.queues[category],
        ready = queue.some((item) => item.ready);
      ...
      tab.classList.toggle(
        ...
        !friendly.some((entity) => this.game.defs[entity.type]?.producer?.includes(category)),
      );
    }

    const planning = !!this.actions.getPlanning?.(),
      planningButton = this.root.querySelector('[data-command="planning"]')!;

    planningButton.classList.toggle('active', planning);
    planningButton.setAttribute('aria-pressed', String(planning));
    this.el('production-counter').textContent = String(totalQueue).padStart(2, '0');
    this.el('mobile-ready-dot').classList.toggle('visible', hasReady);

    for (const [type, card] of this.cardElements) {
      const def = this.game.defs[type],
        available = this.game.canBuild(type),
        queue = side.queues[def.category];

      const matching = queue.filter((item) => item.type === type),
        ready = matching.some((item) => item.ready),
        active = queue[0]?.type === type ? queue[0] : null;

      card.classList.toggle('unavailable', !available.ok && !ready);
      card.classList.toggle('producing', !!active && !ready);
      ...
      this.nativeText(
        card.querySelector<HTMLElement>('.card-queue')!,
        matching.length > 1 ? String(matching.length) : '',
      );
      this.nativeText(
        card.querySelector<HTMLElement>('.card-state')!,
        ready
          ? 'Ready'
          : active?.paused || active?.blockedFunds || active?.blockedPrerequisite
            ? 'On Hold'
      ...
      card.querySelector<HTMLElement>('.card-progress')!.style.width = active
        ? `${active.progress * 100}%`
        : '0';
      ...
      reason.textContent =
        active?.blockedPrerequisite ?? (!available.ok && !ready && !active ? available.reason : '');
    }
    ...
    for (const card of this.root.querySelectorAll<HTMLButtonElement>('[data-superweapon]')) {
      const building = state.entities.find(
        (e) =>
          ...
          !e.selling &&
          !e.constructing &&
          this.game.defs[e.type].superweapon === card.dataset.superweapon,
      );
      ...
      const def = this.game.defs[building.type],
        remaining = Math.max(0, Math.ceil(def.recharge! - (building.recharge ?? 0)));
    ...
    if (selected.length) {
      const totalHealth = selected.reduce((sum, entity) => sum + entity.hp, 0),
        maxHealth = selected.reduce((sum, entity) => sum + entity.maxHp, 0);

      const health = this.el('selection-health-fill');

      if (health) {
        health.style.width = `${(totalHealth / Math.max(1, maxHealth)) * 100}%`;
        health.classList.toggle('damaged', totalHealth < maxHealth * 0.4);
      }
      ...
      if (status)
        status.textContent =
          selected.length === 1
            ? `${Math.ceil(selected[0].hp)} / ${selected[0].maxHp} HP · ${selected[0].type === 'george' || selected[0].inspectedBy !== undefined || (selected[0].rank ?? 0) > 0 ? inspectionStatus(selected[0], state, this.game.defs) : selected[0].cargo > 0 ? `${Math.floor(selected[0].cargo)} ORE` : selected[0].order.toUpperCase()}`
            : `${Math.ceil(totalHealth)} HP · ${selected.length} UNITS`;
      const progress = this.root.querySelector<HTMLProgressElement>('#inspection-progress');

      if (progress)
        progress.value =
          selected[0].type === 'george'
            ? (selected[0].inspection?.elapsed ?? 0) / INSPECTION_RULES.seconds
            : (selected[0].inspectionProgress ?? 0);
    }
    ...
    if (this.radarOnline && performance.now() - this.lastMinimap > 350) {
      this.drawMinimap();
      this.lastMinimap = performance.now();
    }

    if (state.winner !== null && state.winner !== this.lastWinner) {
      this.lastWinner = state.winner;
      const won = state.winner === 0;
      this.el('victory-panel').hidden = false;
      this.el('victory-panel').innerHTML =
        `<div class="victory-emblem">${icon('star')}</div><span class="eyebrow">OPERATION IRON HORIZON</span><h2>${won ? 'MISSION ACCOMPLISHED' : 'MISSION FAILED'}</h2><p>${won ? 'The enemy command has fallen. The battlefield is yours.' : 'Our foothold has been lost. Regroup and return to the field.'}</p><div class="result-stats"><span><b>${side.kills}</b> ENEMY LOSSES</span><span><b>${String(Math.floor(state.time / 60)).padStart(2, '0')}:${String(Math.floor(state.time % 60)).padStart(2, '0')}</b> ELAPSED TIME</span></div><button class="primary-button" data-action="restart">NEW OPERATION ${icon('target')}</button>`;
    }

    if (state.winner === null && this.lastWinner !== null) {
      this.lastWinner = null;
      this.el('victory-panel').hidden = true;
    }
  }

  getBattlefieldRect(): DOMRect {
    return document.querySelector('#battlefield')!.getBoundingClientRect();
  }
  destroy() {

...
export type ControlRenderer = Pick<
  Renderer,
  | 'camera'
  | 'placement'
  | 'selectionBox'
...
> & {
  canvas: Pick<
    HTMLCanvasElement,
    'addEventListener' | 'removeEventListener' | 'dispatchEvent' | 'focus' | 'setPointerCapture'
  > & {
...
export interface ControlCallbacks {
  ...
  mode(value: ControlMode): void;
  ack(selection?: readonly string[]): void;
  category?(category: Category): void;
  options?(): void;
  briefing?(): void;
  enabled?(): boolean;
  cursor?(name: NativeCursorName): void;
}

export const detectMobile = () =>
  new URLSearchParams(location.search).get('force_mobile') === '1' ||
  /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent) ||
  (navigator.maxTouchPoints > 1 && matchMedia('(pointer: coarse)').matches);

interface StoredBindings {
  version: 1;
  bindings: Record<BindingId, string | null>;
}

function isBindingKey(value: unknown): value is string | null {
  return value === null || (typeof value === 'string' && /^[a-z?]$/.test(value));
}

function isStoredBindings(value: unknown): value is StoredBindings {
  if (
    !value ||
    typeof value !== 'object' ||
    !('version' in value) ||
  ...
  return BINDING_DEFAULTS.every((binding) => {
    const entry = Object.entries(bindings).find(([id]) => id === binding.id);

    return !!entry && isBindingKey(entry[1]);
  });
}
...
export class Controls {
  ...
  private bindings = new Map<BindingId, string>(
    BINDING_DEFAULTS.map((binding) => [binding.id, binding.defaultKey.toLowerCase()]),
  );
  private state: GameAPI['state'];

  constructor(
    readonly renderer: ControlRenderer,
    ...
    canvas.addEventListener(
      ...
      (e) => {
        ...
        this.renderer.camera.setZoom(
          this.renderer.camera.zoom * Math.exp(-e.deltaY * 0.0015),
          this.local(e),
        );
  ...
  private ownUnits(): Entity[] {
    return this.game.state.entities.filter(
      (e) =>
        e.type !== 'george' &&
        e.side === 0 &&
        e.hp > 0 &&
        e.transportId === undefined &&
        !isBuilding(this.game.defs[e.type]),
    );
  }
  private ids(): number[] {
    return this.game.state.entities.filter((e) => e.selected && e.side === 0).map((e) => e.id);
  }
  private engineerRepair(target: Entity | null | undefined): boolean {
    return (
      ...
      isBuilding(this.game.defs[target.type]) &&
      this.game.state.entities.some(
        (entity) =>
          entity.selected && entity.side === 0 && entity.hp > 0 && entity.type === 'engineer',
      )
    );
  }
  private select(ids: number[], additive = false, feedback = true): void {
    ...
    if (previous.length && previous.join(',') !== this.ids().join(','))
  ...
  private selectionAck(ids: number[]): void {
    const types = this.game.state.entities
      .filter(
        (e) =>
          ids.includes(e.id) &&
          e.selected &&
  ...
  get scrollRate(): number {
    return this.scrollMultiplier;
  }
  setScrollRate(multiplier: number): void {
    if (Number.isFinite(multiplier))
      this.scrollMultiplier = Math.max(0.25, Math.min(3, multiplier));
  }
  getBindings(): BindingInfo[] {
    return BINDING_DEFAULTS.map((binding) => ({
      ...binding,
      key: this.bindings.get(binding.id)?.toUpperCase() ?? null,
    }));
  }
  inspectBinding(id: string, value: string): BindingResult {
    if (!BINDING_DEFAULTS.some((binding) => binding.id === id))
    ...
    if (!/^[a-z?]$/.test(key))
      return {
        ok: false,
        reason:
          'Choose a letter A–Z or ?. Team, bookmark, navigation, and browser keys stay reserved.',
      };

    const conflict = BINDING_DEFAULTS.find(
      (binding) => binding.id !== id && this.bindings.get(binding.id) === key,
    );

    const result: BindingResult = { ok: true };

    if (conflict) result.conflict = { id: conflict.id, label: conflict.label };
  ...
  assignBinding(id: string, value: string): BindingResult {
    ...
    const binding = BINDING_DEFAULTS.find((binding) => binding.id === id);

    if (!binding) return { ok: false, reason: 'Unknown command.' };
    this.bindings.set(binding.id, value.trim().toLowerCase());
    this.keys.clear();
    this.waypointOrders.clear();
    this.saveBindings();

    if (result.conflict) result.replaced = result.conflict.id;
  ...
  private loadBindings(): void {
    try {
      ...
      if (!isStoredBindings(saved)) return;

      const restored = new Map<BindingId, string>(),
        seen = new Set<string>();

      for (const binding of BINDING_DEFAULTS) {
        const value = saved.bindings[binding.id];

        if (value === null) continue;

        if (seen.has(value)) return;
        restored.set(binding.id, value);
        seen.add(value);
      }

      this.bindings = restored;
    } catch {
      /* Browser storage may be disabled; native defaults remain usable. */
    }
  }
  private saveBindings(): void {
    try {
      localStorage.setItem(
        'ra2-keyboard-bindings:v1',
        JSON.stringify({
          version: 1,
          bindings: Object.fromEntries(
            BINDING_DEFAULTS.map((binding) => [binding.id, this.bindings.get(binding.id) ?? null]),
          ),
        }),
      );
    } catch {
      /* Binding changes remain active for this session without storage. */
    }
  }
  get cursor(): NativeCursorName {
    ...
    if (!this.pointers.size) {
      ...
      if (horizontal || vertical) {
        ...
        const wanted = camera.constrainedPosition(
          state.width,
          state.height,
          camera.x + (horizontal === 'w' ? -1 : horizontal === 'e' ? 1 : 0) / camera.zoom,
          camera.y + (vertical === 'n' ? -1 : vertical === 's' ? 1 : 0) / camera.zoom,
          state.nativeMap,
        );

        const blocked = Math.hypot(wanted.x - camera.x, wanted.y - camera.y) < 1e-6;

        // SAFETY: nonempty vertical/horizontal components form the eight compass directions; both cursor variants exist for each.
        return `scroll-${direction}${blocked ? '-blocked' : ''}` as NativeCursorName;
      }
    }

    if (this.mode === 'pan' || (this.pointers.size && (this.button === 1 || this.button === 2)))
      return 'pan';

    if (['chrono-source', 'chrono-destination', 'weather'].includes(this.mode)) return 'attack';

    const target = this.renderer.pick(p.x, p.y),
      own = target?.side === 0;

    if (this.mode === 'repair')
      return target && this.game.canRepair(target.id) ? 'repair' : 'repair-blocked';

    if (this.mode === 'sell')
      return own && target && isBuilding(this.game.defs[target.type]) ? 'sell' : 'sell-blocked';

    const point = camera.world(p.x, p.y),
    ...
    const selected = state.entities.filter(
      (entity) =>
        entity.selected && entity.side === 0 && entity.hp > 0 && entity.transportId === undefined,
    );
        ...
        ? movers.some(
            (def) =>
              ...
              (def.movement === 'water'
                ? tile.terrain === 'water'
                : tile.terrain !== 'rock' &&
                  (def.movement === 'amphibious' || tile.terrain !== 'water')),
          )
      ...
      (airborne ||
        !state.entities.some((entity) => {
          ...
          return (
            ...
            isBuilding(def) &&
            x >= entity.x &&
            x < entity.x + def.footprint[0] &&
            y >= entity.y &&
            y < entity.y + def.footprint[1]
          );
        }));

    if (this.renderer.placement)
      return this.game.canPlace(this.renderer.placement, x, y) ? 'deploy' : 'deploy-blocked';
    ...
    if ((mobile && ctrl && shift) || this.mode === 'attack')
      return clear ? 'attackmove' : 'attackmove-blocked';
    ...
    if (
      ...
      isBuilding(this.game.defs[target.type]) &&
      selected.some(
        (entity) => entity.type === 'engineer' || this.game.defs[entity.type].ability === 'spy',
      )
    )
      return 'enter';

    if (target && own && selected.some((e) => this.game.canEnterTransport?.(e, target)))
      return 'enter';

    if (target && own && target.type === 'george') return 'default';

    if (target && own)
      return target.selected && this.game.defs[target.type].deployedRange !== undefined
        ? 'deploy'
        : 'select';
    ...
    if (
      ...
      tile &&
      tile.ore > 0 &&
      selected.some((entity) => this.game.defs[entity.type].harvester)
    )
      return 'attack';
  ...
  private updateCursor(): void {
    ...
    if (cursor !== this.lastCursor) {
      ...
      this.callbacks.cursor?.(cursor);
    }
  }
  private trackModifiers(e: PointerEvent | KeyboardEvent): void {
    this.modifiers = {
      ctrl: e.ctrlKey || ('metaKey' in e && e.metaKey),
      shift: e.shiftKey,
      alt: e.altKey,
    };
  }
  setMode(mode: ControlMode): void {
    ...
    this.mode = mode;
    this.callbacks.mode(mode);

    if (!this.callbacks.cursor)
      this.renderer.canvas.style.cursor =
        mode === 'pan'
          ? 'grab'
          : mode === 'attack' || mode === 'repair' || mode === 'sell'
            ? 'crosshair'
            : 'default';
    this.updateCursor();
  }
  setPlacement(type: string): void {
    this.setMode('select');
    this.renderer.placement = type;
    this.callbacks.toast(
      `Place ${this.game.defs[type].name} near your base. Green tiles are valid.`,
    );

    if (!this.callbacks.cursor) this.renderer.canvas.style.cursor = 'crosshair';
    this.updateCursor();
  }
  cancelPlacement(): void {
    this.renderer.placement = null;

    if (!this.callbacks.cursor) this.renderer.canvas.style.cursor = 'default';
    this.updateCursor();
  }
  ...
  radarOrder(
    ...
    if (!ids.length || ['pan', 'repair', 'sell'].includes(this.mode) || this.renderer.placement) {
      this.renderer.camera.center(x, y);
    ...
    const target = s.entities.find((entity) => {
      ...
      return isBuilding(def)
        ? x >= entity.x &&
            ...
            y >= entity.y &&
            y < entity.y + def.footprint[1]
        : Math.hypot(x - entity.x, y - entity.y) < 0.6;
    });
    ...
    if (
      ...
      !modifiers.ctrl &&
      !modifiers.alt &&
      this.game.enterTransport?.(ids, target.id)
    )
      return;

    if (modifiers.ctrl && modifiers.alt)
      this.game.orderGuard(ids, x, y, target?.side === 0 ? target.id : undefined);
    else if (modifiers.ctrl && modifiers.shift) this.game.orderMove(ids, x, y, true);
    else if (modifiers.ctrl) {
      if (target) this.game.orderAttack(ids, target.id, true);
      else this.game.orderForceFire(ids, x, y);
    } else if (modifiers.alt) this.game.orderForceMove(ids, x, y);
    else if (this.planning) {
      this.queueWaypoint(ids, { x, y });

      return;
    } else if (target && (target.side !== 0 || this.engineerRepair(target)))
      this.game.orderAttack(ids, target.id);
    else if (
      x >= 0 &&
      y >= 0 &&
      x < s.width &&
      y < s.height &&
      s.tiles[Math.floor(y) * s.width + Math.floor(x)].ore > 0 &&
      s.entities.some((entity) => ids.includes(entity.id) && this.game.defs[entity.type].harvester)
    )
      this.game.orderHarvest(ids, x, y);
    else this.game.orderMove(ids, x, y, this.mode === 'attack');
    this.callbacks.ack();

    if (this.mode === 'attack') this.setMode('select');
  }
  ...
  private move(e: PointerEvent): void {
    ...
    const p = this.local(e);
    this.renderer.pointer = p;
    this.renderer.hoverId = this.renderer.pick(p.x, p.y)?.id ?? null;
    this.hover = e.pointerType === 'touch' ? null : p;
    this.trackModifiers(e);
    this.updateCursor();
    ...
    if (this.pointers.size >= 2) {
      ...
      if (this.pinchDistance > 0)
        this.renderer.camera.setZoom(
          (this.renderer.camera.zoom * d) / this.pinchDistance,
          this.pinchCenter ?? center,
        );

      if (this.pinchCenter)
        this.renderer.camera.pan(center.x - this.pinchCenter.x, center.y - this.pinchCenter.y);
    ...
    if (this.moved) {
      if (this.mode === 'pan' || this.button === 1 || this.button === 2)
        this.renderer.camera.pan(p.x - this.last.x, p.y - this.last.y);
      else if (!this.renderer.placement && this.mode === 'select')
        this.renderer.selectionBox = { from: this.start, to: p };
    }
  ...
  private up(e: PointerEvent): void {
    ...
    if (this.renderer.selectionBox) {
      const { from, to } = this.renderer.selectionBox,
      ...
      const ids = this.ownUnits()
        .filter((entity) => {
          const q = this.renderer.entityPoint(entity);
  ...
  private click(p: Vec2, e: PointerEvent): void {
    if (e.button === 2) {
      ...
      if (!this.planning) this.waypointOrders.clear();

      return;
    }

    const w = this.renderer.camera.world(p.x, p.y),
      x = Math.floor(w.x),
      y = Math.floor(w.y),
      s = this.game.state;
    ...
    if (this.renderer.placement) {
      if (this.game.place(this.renderer.placement, x, y)) {
        this.callbacks.ack();
    ...
    const entity = this.renderer.pick(p.x, p.y),
      ids = this.ids();

    if (this.mode === 'repair' || this.mode === 'sell') {
      if (entity && entity.side === 0) {
        const done =
          this.mode === 'repair' ? this.game.repair(entity.id) : this.game.sell(entity.id);
    ...
    if (ids.length && e.ctrlKey && e.altKey)
      this.game.orderGuard(ids, w.x, w.y, entity?.side === 0 ? entity.id : undefined);
    ...
    else if (entity?.side === 0 && this.game.enterTransport?.(ids, entity.id)) {
      this.callbacks.ack();
      ...
      const now = performance.now(),
        double = this.lastClick.id === entity.id && now - this.lastClick.time < 320;

      if (e.shiftKey && entity.selected)
        this.select(
          ids.filter((id) => id !== entity.id),
          false,
          false,
        );
      else if (double && this.game.defs[entity.type].deployedRange === undefined)
        this.select(
          this.ownUnits()
            .filter((other) => other.type === entity.type && this.renderer.visible(other))
            .map((other) => other.id),
          e.shiftKey,
        );
      else if (
        entity.selected &&
        !e.shiftKey &&
        (this.game.defs[entity.type].deployedRange !== undefined ||
          entity.type === 'mcv' ||
          this.game.defs[entity.type].passengers)
      ) {
        this.game.deploy([entity.id]);
        this.callbacks.ack();
      } else this.select([entity.id], e.shiftKey);
      this.lastClick = { id: entity.id, time: now };

      return;
    } else if (ids.length) {
      if (this.planning) {
        this.queueWaypoint(ids, w);
      ...
      if (entity && entity.side !== 0) this.game.orderAttack(ids, entity.id);
        ...
        x < s.width &&
        y < s.height &&
        s.tiles[y * s.width + x].ore > 0 &&
        ids.some(
          (id) => this.game.defs[s.entities.find((entity) => entity.id === id)!.type].harvester,
        )
      )
        this.game.orderHarvest(ids, x, y);
  ...
  private superweaponClick(point: Vec2): boolean {
    ...
    if (this.mode === 'chrono-source') {
      this.chronoSource = { ...point };
      this.setMode('chrono-destination');
      this.callbacks.toast('Choose the Chronosphere destination. Right click cancels.');

      return true;
    }

    const success = this.game.activateSuperweapon?.(
      this.mode === 'weather' ? 'weather' : 'chronosphere',
      point,
      this.chronoSource,
    );
    ...
    if (success) this.setMode('select');
      ...
      this.callbacks.toast(
        'Choose an explored, valid target. The weapon must be charged and powered.',
      );

    return true;
  }
  get planning(): boolean {
    const key = this.bindings.get('planning');

    return this.planningMode || (key !== undefined && this.keys.has(key));
  }
  ...
  private queueWaypoint(ids: number[], point: Vec2): void {
    ...
    this.callbacks.toast(
      this.planningMode
        ? 'Waypoint set. Turn off planning to move.'
        : 'Waypoint set. Release Z to move.',
    );
  }
  ...
  executeCommand(
    ...
    if (command === 'deploy') {
      this.game.deploy(ids);
      this.callbacks.ack();
    } else if (command === 'guard') {
      this.game.guard(ids);
      this.callbacks.toast('Guarding current area.');
      ...
      this.callbacks.toast(
        this.planningMode
          ? 'Planning: click destinations, then turn off planning to move.'
          : 'Waypoint orders issued.',
      );
    } else if (command === 'type') {
      const types = new Set(this.game.state.entities.filter((e) => e.selected).map((e) => e.type)),
        now = performance.now(),
        all = now - this.lastTypeTime < 400;

      this.select(
        this.ownUnits()
          .filter((e) => types.has(e.type) && (all || this.renderer.visible(e)))
          .map((e) => e.id),
        options.shift,
      );
      ...
      if (options.clear) {
        ...
        this.callbacks.toast(`Control group ${key} disbanded.`);
      } else
        this.selectGroup(
          key,
          !!options.ctrl || (!this.groups.has(key) && ids.length > 0),
          !!options.shift,
        );
    }
  }
  ...
  private selectGroup(key: string, assign: boolean, additive: boolean): void {
    if (assign) {
      this.groups.set(key, this.ids());
      this.callbacks.toast(`Control group ${key} assigned.`);
    } else {
      const group = this.groups.get(key);

      if (group) {
        this.select(group, additive);
        const now = performance.now();
  ...
  private commitWaypoints(): void {
    if (this.enabled())
      for (const [id, route] of this.waypointOrders) this.game.orderWaypoints([id], route);
    this.waypointOrders.clear();
  }
  ...
  private key(e: KeyboardEvent): void {
    ...
    if (
      !this.enabled() ||
      target?.closest('input,select,textarea,[contenteditable="true"],[role="dialog"],dialog[open]')
    ) {
      this.keys.clear();
    ...
    if (key === 'escape') {
      ...
      this.game.stop(ids);
      this.callbacks.toast('Units stopped.');
    } else if (command === 'guard') this.executeCommand('guard');
    else if (command === 'scatter') {
      this.game.scatter(ids);
      this.callbacks.ack();
    } else if (command === 'deploy') this.executeCommand('deploy');
    else if (command === 'repair' || command === 'sell')
      this.setMode(this.mode === command ? 'select' : command);
      ...
      if (previous) {
        this.game.select(previous);
        this.selectionAck(previous);
      }
    } else if (command === 'health') {
      const units = this.ownUnits();

      for (let attempt = 0; attempt < 3; attempt++) {
        const band = this.healthBand;
        this.healthBand = (this.healthBand + 1) % 3;

        const members = units.filter(
          (e) => (e.hp / e.maxHp > 0.5 ? 0 : e.hp / e.maxHp > 0.25 ? 1 : 2) === band,
        );

        if (members.length) {
          this.select(members.map((e) => e.id));
          break;
        }
      }
    } else if (command === 'follow') {
      this.followId = this.followId === ids[0] ? null : (ids[0] ?? null);
    } else if (command === 'home') {
      const yard = this.game.state.entities.find((e) => e.side === 0 && e.type === 'conyard');

      if (yard) {
        const [w, h] = this.game.defs[yard.type].footprint;
        this.followId = null;
        this.renderer.camera.center(yard.x + w / 2, yard.y + h / 2);
      }
    } else if (
      command === 'structures' ||
      command === 'defenses' ||
      ...
      this.callbacks.category?.(command);
    else if (/^f[1-4]$/.test(key)) {
      e.preventDefault();
      const camera = this.renderer.camera;

      if (ctrl) {
        this.bookmarks.set(key, { x: camera.x, y: camera.y });
        this.callbacks.toast(`Bookmark ${key.toUpperCase()} set.`);
      } else {
        const point = this.bookmarks.get(key);
      ...
      this.renderer.camera.setZoom(this.renderer.camera.zoom * (key === '-' ? 0.85 : 1.15));
      this.callbacks.zoom(this.renderer.camera.zoom);
  ...
  private centerSelection(): void {
    ...
    if (entities.length)
      this.renderer.camera.center(
        entities.reduce((sum, e) => sum + e.x, 0) / entities.length,
        entities.reduce((sum, e) => sum + e.y, 0) / entities.length,
      );
  }
  tick(dt: number): void {
    ...
    const speed = 500 * this.scrollMultiplier * Math.min(dt, 0.25),
    ...
    if (!this.mobile && this.hover && !this.pointers.size) {
      if (this.hover.x < 12) dx += speed;
      else if (this.hover.x > camera.width - 12) dx -= speed;

      if (this.hover.y < 12) dy += speed;
      else if (this.hover.y > camera.height - 12) dy -= speed;
    }
    ...
    if (this.followId !== null) {
      ...
      if (unit) {
        const previous = unit.previous ?? unit,
          alpha = this.game.interpolation;

        camera.center(
          previous.x + (unit.x - previous.x) * alpha,
          previous.y + (unit.y - previous.y) * alpha,
        );
    ...
    if (this.lastCameraZoom !== camera.zoom) {
      this.callbacks.zoom(camera.zoom);

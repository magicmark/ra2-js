...
export function installLocalTools(
  ...
  pointer.className = 'local-spawn-pointer';
  pointer.hidden = true;
  document.body.append(panel, pointer);

  const element = <T extends HTMLElement>(name: string) =>
    panel.querySelector<T>(`[data-local="${name}"]`)!;

  const speed = element<HTMLSelectElement>('speed'),
    instant = element<HTMLInputElement>('instant'),
    free = element<HTMLInputElement>('free');

  const enemy = element<HTMLSelectElement>('enemy'),
    place = element<HTMLButtonElement>('place'),
    status = element('status');

  speed.replaceChildren(
    ...[...new Set([0.25, ...GAME_SPEED_STEPS])]
      .sort((a, b) => a - b)
      .map((value) => {
  ...
  const syncSpeed = () => {
    ...
    if (![...speed.options].some((option) => option.value === value)) {
      const option = document.createElement('option');
      option.value = value;
      option.textContent = `${game.state.speed}×`;
      speed.append(option);
    }
  ...
  for (const def of Object.values(game.defs).filter(
    (def) => !def.mapOnly && (def.category === 'infantry' || def.category === 'vehicles'),
  )) {
    const option = document.createElement('option');
    option.value = def.id;
  ...
  const report = (message?: string) => {
    ...
    status.textContent =
      message ??
      (armed
        ? `Click open ground to place ${game.defs[enemy.value].name}. Esc cancels.`
        : `Speed ${game.state.speed}× · instant ${instant.checked ? 'on' : 'off'} · free ${free.checked ? 'on' : 'off'}`);
  ...
  for (const input of [instant, free])
    input.addEventListener('change', () => {
      game.configureLocalTools({ instantBuild: instant.checked, free: free.checked });
      report();
    });
  enemy.addEventListener('change', () => report());
  place.addEventListener('click', () => {
    if (!enabled()) {
      report('Enter the battlefield before placing an enemy.');
  ...
  renderer.canvas.addEventListener(
    ...
    (event) => {
      ...
      pointer.style.left = `${event.clientX + 16}px`;
      pointer.style.top = `${event.clientY + 18}px`;
      event.stopImmediatePropagation();
    },
    true,
  );
  ...
  renderer.canvas.addEventListener(
    ...
    (event) => {
      ...
      const rect = renderer.canvas.getBoundingClientRect(),
        world = renderer.camera.world(event.clientX - rect.left, event.clientY - rect.top);

      const entity = game.placeLocalEnemy(enemy.value, world.x, world.y);
      report(
        entity
          ? `${game.defs[entity.type].name} placed. Click again, or Esc to finish.`
          : 'That cell is blocked. Choose open ground.',
      );
    },
    true,
  );

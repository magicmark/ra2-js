...
function manager() {
  ...
  palette.set([71, 82, 93], 30);
  Object.assign(assets, {
    files: new Map([
      ['pips.shp', testShp(19)],
      ['pips2.shp', testShp(18)],
      ['oregath.shp', testShp(120)],
    ]),
    pipPalette: palette,
...
describe('original unit indicator and harvesting artwork', () => {
  it('extracts the original pip palette, both pip shapes, and directional ore gathering effect', () => {
    ...
    for (const file of ['palette.pal', 'pips.shp', 'pips2.shp', 'oregath.shp'])
      expect(wanted.has(file)).toBe(true);
  });
  it('loops exactly fifteen ticks per facing and caches only authored frame combinations', () => {
    const assets = manager(),
      first = assets.getHarvestSprite(0, 0)!;

    expect(first).not.toBeNull();
    expect(assets.getHarvestSprite(0, 1 / 30)).not.toBe(first);
    expect(assets.getHarvestSprite(0, 0.5)).toBe(first);
    expect(assets.getHarvestSprite(1, 0)).not.toBe(first);
    const sources = new Set();

    for (let direction = 0; direction < 8; direction++)
      for (let tick = 0; tick < 120; tick++)
        sources.add(assets.getHarvestSprite(direction, tick / 30));
  ...
  it('keeps original palette colors without applying player remap to yellow pips', () => {
    ...
    for (const kind of ['veteran', 'elite', 'cargo-empty', 'cargo-ore'] as const) {
      ...
      expect(Array.from(testCanvas(sprite.source).pixels)).toEqual([71, 82, 93, 255]);
  ...
  it('uses the exact retail veterancy/cargo pixels and all 120 gathering frames', () => {
    ...
    const visit = (name: string, bytes: Uint8Array) => {
      ...
      for (const name of NESTED_MIXES) {
        ...
        if (nested) visit(name, nested);
      }
    };

    for (const file of ['ra2.mix', 'language.mix'])
      visit(file, readFileSync(join(process.env.RA2_ASSET_DIR!, file)));
    const get = (name: string) => archives.map((a) => a.get(name)).find(Boolean)!;

    const assets = manager(),
      pipPalette = decodePalette(get('palette.pal'));

    Object.assign(assets, {
      files: new Map(['pips.shp', 'pips2.shp', 'oregath.shp'].map((name) => [name, get(name)])),
      pipPalette,
      animationPalette: decodePalette(get('anim.pal')),
    });

    for (const [kind, name, index, width, height] of [
      ['veteran', 'pips.shp', 13, 8, 5],
      ['elite', 'pips.shp', 14, 8, 13],
      ['cargo-empty', 'pips2.shp', 0, 4, 4],
      ['cargo-ore', 'pips2.shp', 2, 4, 4],
    ] as const) {
      const sprite = assets.getPipSprite(kind)!,
        frame = new ShpFile(get(name)).frame(index);

      expect([sprite.width, sprite.height]).toEqual([width, height]);

      const expected = Array.from(frame.pixels).flatMap((i) =>
        i === 0 ? [0, 0, 0, 0] : [...pipPalette.slice(i * 3, i * 3 + 3), 255],
      );

      expect(Array.from(testCanvas(sprite.source).pixels)).toEqual(expected);
    }

    const shp = new ShpFile(get('oregath.shp'));
    expect(shp.frameCount).toBe(120);
    ...
    for (let facing = 0; facing < 8; facing++)
      for (let tick = 0; tick < 15; tick++) {
        ...
        hashes.add(Array.from(testCanvas(sprite.source).pixels).join(','));
      }

    expect(hashes.size).toBe(120);

    const selected = [...wantedFiles()].flatMap((name) => {
    ...
    Object.assign(assets, {
      files: new Map(selected.map((file) => [file.name, file.bytes])),
      art: readArtSections(get('art.ini')),
      buildingLoops: buildingLoops(get('art.ini')),
    });
    ...
    expect(Array.from(testCanvas(flag.source).pixels)).not.toEqual(
      Array.from(testCanvas(oil.source).pixels),
    );
    expect(Array.from(testCanvas(pump.source).pixels)).not.toEqual(
      Array.from(testCanvas(flag.source).pixels),
    );

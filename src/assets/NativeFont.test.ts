...
describe('original Unicode bitmap font', () => {
  it('decodes one-based character indices, MSB pixels, native spacing and transparent pixels', () => {
    const font = new NativeFont(testFont()),
      image = font.rasterize('AA', [17, 34, 51], false);

    expect([font.rows, font.lineHeight, image.width, image.height]).toEqual([2, 3, 7, 2]);

    const rows = Array.from({ length: 2 }, (_, y) =>
      Array.from({ length: 7 }, (_, x) => (image.pixels[(y * 7 + x) * 4 + 3] ? '#' : '.')).join(''),
    );

    expect(rows).toEqual(['#.#.#.#', '.#...#.']);
    expect(Array.from(image.pixels.slice(0, 4))).toEqual([17, 34, 51, 255]);
  });
  it('draws a one-pixel black shadow without replacing foreground glyph pixels', () => {
    ...
    const at = (x: number, y: number) =>
      Array.from(image.pixels.slice((y * 4 + x) * 4, (y * 4 + x + 1) * 4));
  ...
  it.skipIf(!existsSync('/tmp/ra2-references/game.fnt'))(
    'renders the actual original zero from its authored bitmap',
    () => {
      ...
      const rows = Array.from({ length: 16 }, (_, y) =>
        Array.from({ length: 5 }, (_, x) => (image.pixels[(y * 5 + x) * 4 + 3] ? '#' : '.')).join(
          '',
        ),
      );

...
export const ORIGINAL_ASSET_URL =
  'https://archive.org/cors/red-alert-2-multiplayer/Red-Alert-2-Multiplayer.exe';

export const LEGACY_ASSET_URL =
  ...
  if (typeof location !== 'undefined') {
    try {
      ...
      if (
        ...
        parsed.pathname === '/asset-source' &&
        !parsed.search &&
        !parsed.hash
      )
        return ORIGINAL_ASSET_URL;
    } catch {
      /* The explicit request reports malformed URLs. */
    }
  }
...
export function assetCacheKey(url: string): string {
  const source = assetSourceUrl(url);

  return source === ORIGINAL_ASSET_URL ? '/asset-source' : source;
}
...
export interface SavedArchive {
  ...
  saved: number;
  rejected?: string;
  rejectionKind?: 'container';
}

export interface ArchiveResume extends SavedArchive {
  stage: ArchiveStage;
}

export const archiveStageKey = (source: string, stage: ArchiveStage): IDBValidKey => [
  'archive-stage-v1',
  assetCacheKey(source),
  stage,
];

/** Keep the existing database/store and selected-art keys intact. */
export async function assetCache<T>(
  key: IDBValidKey,
  value?: T | null,
  alive = () => true,
): Promise<T | undefined> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const tx = db.transaction('assets', value === undefined ? 'readonly' : 'readwrite'),
      ...
      tx.onabort = () => {
        db.close();
        reject(tx.error ?? new Error('Asset cache transaction aborted'));
      };
    } catch (error) {
      db.close();
      reject(error);
...
const details = (cause: unknown) => (cause instanceof Error ? cause.message : String(cause));

const stageName = (stage: ArchiveStage) =>
  stage.startsWith('mix') ? 'extracted MIX files' : 'completed installer download';

function validArchiveInput(value: unknown): value is ArchiveInput {
  return (
    typeof value === 'object' &&
    value !== null &&
    'name' in value &&
...
function validStage(value: unknown): value is SavedArchive {
  return (
    ...
    (!('rejectionKind' in value) ||
      value.rejectionKind === undefined ||
      value.rejectionKind === 'container') &&
    'files' in value &&
    Array.isArray(value.files) &&
    value.files.length > 0 &&
    value.files.every(validArchiveInput)
  );
}
...
const legacyArtworkRejection = (reason: string | undefined): boolean =>
  !!reason &&
  /^(Missing original asset|Invalid original (asset|sprite)|Cannot render original)/.test(reason);

const legacyMixRejection = (reason: string | undefined): boolean =>
  !!reason &&
  /^(?:.+\.mix: (?:truncated (?:encrypted )?MIX (?:header|body)|invalid MIX index|file outside MIX body|encrypted index out of bounds)|Truncated MIX encryption key)$/i.test(
    reason,
  );

const legacyExtractorRejection = (reason: string | undefined): boolean =>
  !!reason &&
  (reason === '[object Object]' ||
    /^Archive extraction failed \(7-Zip code (?:7|8|255)\)/.test(reason));
...
export class AssetDownload {
  ...
  private memoryKey(stage: ArchiveStage): string {
    return JSON.stringify(archiveStageKey(this.source, stage));
  }
  private async read(stage: ArchiveStage): Promise<SavedArchive | undefined> {
    ...
    try {
      ...
      if (
        !validStage(value) ||
        (stage.startsWith('mix') && !value.files.every((file) => /\.mix$/i.test(file.name)))
      ) {
        this.warning(
          stage,
          `Saved ${stageName(stage)} are invalid; an explicit retry can replace them.`,
        );
      ...
      if (this.alive()) this.warning(stage, `Cannot read ${stageName(stage)}: ${details(error)}`);
    }
  }
  private async save(stage: ArchiveStage, entry: SavedArchive): Promise<boolean> {
    ...
    try {
      ...
      if (this.alive())
        this.warning(
          stage,
          `Browser storage could not save ${stageName(stage)}: ${details(error)}`,
        );
  ...
  private async remove(stage: ArchiveStage, id: string): Promise<void> {
    ...
    try {
      await assetCache(archiveStageKey(this.source, stage), null, this.alive);
    } catch (error) {
      if (this.alive())
        this.warning(
          `cleanup-${stage}`,
          `Could not remove temporary ${stageName(stage)}: ${details(error)}`,
        );
    }
  }
  async resume(options: { skipMix?: boolean } = {}): Promise<ArchiveResume | undefined> {
    ...
    for (const stage of ['mix-pending', 'mix', 'download', 'installer'] as const) {
      ...
      if (
        ...
        (!entry.rejected ||
          (!entry.rejectionKind &&
            (legacyArtworkRejection(entry.rejected) ||
              legacyExtractorRejection(entry.rejected) ||
              (stage === 'installer' && legacyMixRejection(entry.rejected)))))
      )
        return { ...entry, rejected: undefined, stage };
    }
  }
  async remember(files: ArchiveInput[]): Promise<ArchiveResume> {
    const entry: SavedArchive = {
      version: 1,
  ...
  async download(
    signal: AbortSignal,
    progress: (message: string, loaded: number, total?: number) => void,
  ): Promise<ArchiveResume> {
    ...
    if (!response.ok)
      throw new Error(
        `Asset server returned HTTP ${response.status}. Retry or import the installer below.`,
      );
    ...
    if (response.status === 206) {
      ...
      if (!range || Number(range[1]) !== 0 || Number(range[2]) + 1 !== Number(range[3])) {
        throw new Error(
          'The asset server returned only part of the archive. Press Enter to retry or choose a complete source.',
        );
      }
    }
    ...
    if (reader) {
      for (;;) {
        ...
        if (!this.alive()) {
          await reader.cancel();
          throw new Error('Asset loading cancelled');
        }

        if (next.done) break;
        // SAFETY: Fetch response byte streams yield ArrayBuffer-backed chunks, never shared memory.
        chunks.push(next.value as Uint8Array<ArrayBuffer>);
        loaded += next.value.byteLength;
        progress(
          `Downloading assets · ${(loaded / 1048576).toFixed(1)}${total ? ` / ${(total / 1048576).toFixed(1)}` : ''} MB`,
          loaded,
          total || undefined,
        );
      }
    ...
    if (total && !encoded && blob.size !== total)
      throw new Error(
        `Incomplete asset download: received ${blob.size} of ${total} bytes. Press Enter to retry.`,
      );
    ...
    const archive =
      ...
      (head[0] === 0x52 && head[1] === 0x61);

    if (!archive)
      throw new InvalidArchiveError(
        'The URL returned no supported game installer/archive (possibly an HTML error page). Correct the URL and press Enter to retry.',
      );
  ...
  async complete(input: ArchiveResume): Promise<void> {
    ...
    if (this.mix && input.stage !== 'mix' && (await this.save('mix', this.mix))) {
      await this.remove('mix-pending', this.mix.id);
      this.warning('mix-pending', null);
    }

    const installer =
      input.stage === 'download' || input.stage === 'installer'
        ? input
        : await this.read('download');

    if (
      installer?.id === input.id &&
      input.stage !== 'installer' &&
      !installer.files.every((file) => /\.mix$/i.test(file.name))
    ) {
      if (await this.save('installer', installer)) {
        await this.remove('download', installer.id);
        this.warning('download', null);
      }
  ...
  async reject(input: ArchiveResume, reason: string): Promise<void> {
    ...
    const stages: ArchiveStage[] = input.stage.startsWith('mix')
      ? ['mix-pending', 'mix']
      : ['download', 'installer'];
    ...
    for (const stage of stages) {
      ...
      if (entry?.id === input.id)
        await this.save(stage, { ...entry, rejected: reason, rejectionKind: 'container' });
    }
  }
  async clear(): Promise<void> {
    for (const stage of ['download', 'mix-pending', 'mix', 'installer'] as const) {
      this.memory.delete(this.memoryKey(stage));
      await assetCache(archiveStageKey(this.source, stage), null);
    }
  }
}

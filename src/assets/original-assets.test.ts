  ...
  it('resolves every shipped unit, cameo, terrain and scenery asset from real encrypted MIX files', () => {
    ...
    const visit = (name: string, bytes: Uint8Array) => {
      ...
      for (const nested of NESTED_MIXES) {
        const bytes = archive.get(nested);

        if (bytes) visit(nested, bytes);
      }
    };
    ...
    const get = (name: string): Uint8Array | undefined =>
      wanted.has(name) ? archives.map((a) => a.get(name)).find(Boolean) : undefined;

    const shp = (name: string) =>
      theaterNames(name)
        .flatMap((n) => [n + '.shp', n + '.tem'])
        .map(get)
        .find(Boolean);
    ...
    for (const [name, spec] of Object.entries(CATALOG)) {
      if (spec.kind === 'vehicle')
        expect(
          decodeVxl(get(spec.sprite + '.vxl')!).flatMap((l) => l.voxels).length,
          name,
        ).toBeGreaterThan(0);
      else
        expect(
          new ShpFile(shp(spec.sprite)!).frame().pixels.some((p) => p > 0),
          name,
        ).toBe(true);
      expect(
        new ShpFile(shp(spec.cameo)!).frame().pixels.some((p) => p > 0),
        `${name} cameo`,
      ).toBe(true);

      if (spec.bib)
        expect(
          new ShpFile(shp(spec.bib)!).frame().pixels.some((p) => p > 0),
          `${name} foundation`,
        ).toBe(true);

      if (spec.turret)
        expect(
          decodeVxl(get(spec.turret + '.vxl')!).flatMap((l) => l.voxels).length,
          `${name} turret`,
        ).toBeGreaterThan(0);

      for (const overlay of spec.overlays ?? [])
        expect(shp(overlay), `${name} animation ${overlay}`).toBeDefined();
    }

    for (const name of ['clear01.tem', 'water01.tem', 'rough01.tem', 'pave01.tem'])
      expect(
        decodeTmp(get(name)!).pixels.some((p) => p > 0),
        name,
      ).toBe(true);

    for (const name of ['proad01.tem', 'proad02.tem', 'proad03.tem', 'ruff01.tem', 'green01.tem'])
      expect(
        decodeTmp(get(name)!).pixels.some((p) => p > 0),
        name,
      ).toBe(true);
    ...
    for (const name of [
      ...
      expect(
        new ShpFile(shp(name)!).frame().pixels.some((p) => p > 0),
        name,
      ).toBe(true);
  });
});

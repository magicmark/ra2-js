...
export function alphaBounds(
  ...
  width: number,
  height: number,
  region: PixelBounds = { left: 0, top: 0, right: width, bottom: height },
  threshold = 32,
): PixelBounds {
  ...
  if (left >= right || top >= bottom)
    throw new Error('Bundled artwork contains an empty sprite frame.');

  return { left, top, right, bottom };
}

export function georgeFrameRegion(width: number, height: number, row: number, column: number) {
  const grid = {
    left: Math.round((column * width) / 8),
    right: Math.round(((column + 1) * width) / 8),
    top: Math.round((row * height) / 4),
    bottom: Math.round(((row + 1) * height) / 4),
  };
...
export function georgeAtlasFrame(action: string, facing: number, age: number) {
  ...
  const walkPhase = Math.floor(Math.max(0, age) / 0.15 + 1e-7) % 4;

  const row =
    action === 'Walk'
      ? [1, 0, 2, 0][walkPhase]
      : action === 'FireUp' && tick >= 1 && tick < 4
        ? 3
        : 0;
...
export class CustomArt {
  ...
  static async load(): Promise<CustomArt> {
    const entries = await Promise.all(
      Object.entries(CUSTOM_ART_URLS).map(async ([name, url]) => {
        ...
        try {
          ...
          throw new Error(`Unable to load bundled artwork ${url}. Reload the page to retry.`);
        }
    ...
    // SAFETY: Promise.all preserves every unique key from CUSTOM_ART_URLS and each value is a decoded Image.
    return new CustomArt(Object.fromEntries(entries) as ArtImages);
  }

  constructor(images: ArtImages) {
    const building = pixels(images.building),
      b = alphaBounds(building.data, building.source.width, building.source.height);

    const scale = BUTCHERS_PROJECTION.gameWidth / (b.right - b.left),
      scaleY = scale * BUTCHERS_PROJECTION_Y;

    const source = canvas(
      Math.ceil(building.source.width * scale),
      Math.ceil(building.source.height * scaleY),
    );
    ...
    source
      ...
      .drawImage(
        ...
        0,
        0,
        building.source.width * scale,
        building.source.height * scaleY,
      );
    // The 3x3 native foundation is 180x90. Its origin is 45px above the front corner.
    this.building = sprite(
      source,
      BUTCHERS_PROJECTION.frontCorner.x * scale,
      BUTCHERS_PROJECTION.frontCorner.y * scaleY - 45,
    );

    const sheet = pixels(images.infantry),
      width = sheet.source.width,
      height = sheet.source.height;
    ...
    for (let row = 0; row < 4; row++)
      for (let column = 0; column < 8; column++) {
        ...
        const frame = canvas(36, 36),
          factor = 34 / (grid.right - grid.left);
        ...
        frame
          ...
          .drawImage(
            ...
            1 + (crop.left - grid.left) * factor,
            1,
            (crop.right - crop.left) * factor,
            (crop.bottom - crop.top) * factor,
          );
    ...
    this.cameos = {
      butchers: cameo(images.buildingCameo, "Butcher's"),
      george: cameo(images.infantryCameo, 'George'),
    };
  }

  getCameo(name: string): Sprite | null {
    return name === 'butchers' || name === 'george' ? this.cameos[name] : null;
  }
  getSprite(name: string, frame = 0): Sprite | null {
    return name === 'butchers'
      ? this.building
      : name === 'george'
        ? this.getInfantryFrame(frame)
        : null;
  }
  getInfantryFrame(frame: number): Sprite | null {
    return Number.isInteger(frame) && frame >= 0 && frame < 32 ? this.infantry[frame] : null;
  }
  getInfantrySequence(action: string, facing: number, age: number): Sprite {
    const { column, row } = georgeAtlasFrame(action, facing, age);

    return this.infantry[row * 8 + column];
  }
}

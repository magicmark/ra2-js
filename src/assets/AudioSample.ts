 ...
 */
export interface AudioIndexEntry { name: string; offset: number; size: number; sampleRate: number; flags: number; chunkSize: number }
export interface DecodedAudioSample { sampleRate: number; channels: Float32Array[] }
...
export function readAudioIndex(bytes: Uint8Array): AudioIndexEntry[] {
  ...
  const data = view(bytes), count = data.getUint32(8, true);
  check(text(bytes.subarray(0, 4)) === 'GABA' && data.getUint32(4, true) === 2, 'Invalid audio.idx signature or version');
  check(count > 0 && count <= Math.floor((bytes.length - 12) / 36), 'Truncated audio.idx entries');
  const entries: AudioIndexEntry[] = [], names = new Set<string>();
  for (let i = 0; i < count; i++) {
    const start = 12 + i * 36, raw = bytes.subarray(start, start + 16), end = raw.indexOf(0);
    ...
    check(/^[a-z0-9_$-]+$/.test(name) && !names.has(name), `Invalid or duplicate audio.idx sample name: ${name}`);
    names.add(name);
    entries.push({ name, offset: data.getUint32(start + 16, true), size: data.getUint32(start + 20, true), sampleRate: data.getUint32(start + 24, true), flags: data.getUint32(start + 28, true), chunkSize: data.getUint32(start + 32, true) });
  }
  return entries;
}

function imaFrameCount(size: number, blockAlign: number, channels: number): number {
  check(Number.isInteger(blockAlign) && blockAlign >= channels * 4 && blockAlign <= 65535 && blockAlign % (channels * 4) === 0, 'Invalid IMA WAV block alignment');
  const full = Math.floor(size / blockAlign), partial = size % blockAlign;
  check(!partial || partial >= channels * 4, 'Truncated IMA WAV block header');
  check(channels === 1 || !partial || (partial - channels * 4) % 8 === 0, 'Truncated IMA WAV stereo channel group');
  return full * (1 + (blockAlign - channels * 4) * 2 / channels) + (partial ? 1 + (partial - channels * 4) * 2 / channels : 0);
}
...
export function audioBagWave(entry: AudioIndexEntry, bag: Uint8Array): Uint8Array {
  ...
  check(Number.isInteger(offset) && offset >= 0 && Number.isInteger(size) && size > 0 && offset + size <= bag.length, `${entry.name}: audio.bag sample outside archive`);
  check(Number.isInteger(sampleRate) && sampleRate > 0 && sampleRate <= 384000, `${entry.name}: invalid audio sample rate`);
  const channels = flags & 1 ? 2 : 1, pcm = !!(flags & 2);
  check(!(flags & ~15) && (pcm ? !(flags & 8) : !!(flags & 8)), `${entry.name}: unsupported audio.idx sample flags`);
  const blockAlign = pcm ? channels * 2 : chunkSize;
  check(!pcm || (chunkSize === 0 && size % blockAlign === 0), `${entry.name}: invalid PCM audio.bag sample`);
  ...
  check(pcm || samplesPerBlock <= 65535, `${entry.name}: IMA sample block exceeds WAV format limits`);
  const header = pcm ? 44 : 60, result = new Uint8Array(header + size + (size & 1)), data = view(result);
  const word = (at: number, value: string) => result.set(new TextEncoder().encode(value), at);
  word(0, 'RIFF'); data.setUint32(4, result.length - 8, true); word(8, 'WAVE'); word(12, 'fmt ');
  data.setUint32(16, pcm ? 16 : 20, true); data.setUint16(20, pcm ? 1 : 0x11, true); data.setUint16(22, channels, true);
  data.setUint32(24, sampleRate, true); data.setUint32(28, pcm ? sampleRate * blockAlign : Math.floor(sampleRate * blockAlign / samplesPerBlock), true);
  data.setUint16(32, blockAlign, true); data.setUint16(34, pcm ? 16 : 4, true);
  if (!pcm) {
    data.setUint16(36, 2, true); data.setUint16(38, samplesPerBlock, true);
    word(40, 'fact'); data.setUint32(44, 4, true); data.setUint32(48, frames, true);
  }
  word(header - 8, 'data'); data.setUint32(header - 4, size, true); result.set(bag.subarray(offset, offset + size), header);
  return result;
}
...
function readWave(bytes: Uint8Array) {
  ...
  for (let offset = 12; offset < end;) {
    ...
    const name = text(bytes.subarray(offset, offset + 4)), size = data.getUint32(offset + 4, true), start = offset + 8;
    check(start + size + (size & 1) <= end, `Truncated WAV ${name} chunk`);
    if (name === 'fmt ') { check(!format, 'Duplicate WAV format chunk'); format = bytes.subarray(start, start + size); }
    else if (name === 'data') { check(!body, 'Duplicate WAV data chunk'); body = bytes.subarray(start, start + size); }
    else if (name === 'fact') { check(fact === undefined && size >= 4, 'Invalid WAV fact chunk'); fact = data.getUint32(start, true); }
    offset = start + size + (size & 1);
  }
  check(format && format.length >= 16 && body && body.length > 0, 'Missing WAV format or sample data');
  const fmt = view(format), codec = fmt.getUint16(0, true), channelCount = fmt.getUint16(2, true), sampleRate = fmt.getUint32(4, true), blockAlign = fmt.getUint16(12, true), bits = fmt.getUint16(14, true);
...
export function decodeWave(bytes: Uint8Array): DecodedAudioSample {
  ...
  for (let block = 0; block < body.length; block += blockAlign) {
    ...
    while (cursor < blockEnd) {
      ...
      for (let c = 0; c < channelCount; c++) for (let i = 0; i < bytesPerChannel; i++) {
        const packed = body[cursor++];
        for (let half = 0; half < 2; half++) {
          const nibble = (packed >> (half * 4)) & 15, step = IMA_STEP[index[c]];
          const delta = (step >> 3) + (nibble & 1 ? step >> 2 : 0) + (nibble & 2 ? step >> 1 : 0) + (nibble & 4 ? step : 0);
          predictor[c] = Math.max(-32768, Math.min(32767, predictor[c] + (nibble & 8 ? -delta : delta)));

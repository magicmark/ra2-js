...
export const CURSOR_SEQUENCES = {
  ...
  attack: [58, 5],
  attackmove: [404, 9],
  'attackmove-blocked': [384, 1],
  guard: [68, 5],
  // Retail game.exe cursor table entry 25 (file 0x3e2d50).
  enter: [89, 10],
  deploy: [110, 9],
  'deploy-blocked': [119, 1],
  sell: [129, 10],
  'sell-blocked': [149, 1],
  repair: [170, 20],
  'repair-blocked': [190, 1],
  pan: [385, 1],
  'scroll-n': [2, 1],
  'scroll-ne': [3, 1],
  'scroll-e': [4, 1],
  ...
  'scroll-w': [8, 1],
  'scroll-nw': [9, 1],
  'scroll-n-blocked': [10, 1],
  'scroll-ne-blocked': [11, 1],
  'scroll-e-blocked': [12, 1],
  'scroll-se-blocked': [13, 1],
  'scroll-s-blocked': [14, 1],
  'scroll-sw-blocked': [15, 1],
  'scroll-w-blocked': [16, 1],
  'scroll-nw-blocked': [17, 1],
} as const;

export type NativeCursorName = keyof typeof CURSOR_SEQUENCES;

export class NativeCursors {
  private readonly shp: ShpFile;
  private readonly palette: Uint8Array;
  private readonly urls = new Map<number, string>();
  constructor(shp: Uint8Array, palette: Uint8Array) {
    this.shp = new ShpFile(shp);
    this.palette = decodePalette(palette);

    for (const [start, length] of Object.values(CURSOR_SEQUENCES))
      for (let frame = start; frame < start + length; frame++) {
        if (frame >= this.shp.frameCount) throw new Error(`Missing original cursor frame ${frame}`);
        const image = this.shp.frame(frame);

        if (image.canvasWidth !== 55 || image.canvasHeight !== 43)
          throw new Error('Invalid original cursor canvas: expected 55×43');
      }
  }
  rasterize(frame: number) {
    ...
    for (let y = 0; y < image.height; y++)
      for (let x = 0; x < image.width; x++) {
        ...
        const target = ((y + image.y) * width + image.x + x) * 4,
          source = index * 3;

        pixels[target] = this.palette[source];
        pixels[target + 1] = this.palette[source + 1];
        pixels[target + 2] = this.palette[source + 2];
        pixels[target + 3] = 255;
      }
  ...
  css(name: NativeCursorName, elapsed = 0) {
    ...
    if (!url) {
      const raster = this.rasterize(frame),
      ...
      const context = canvas.getContext('2d')!,
        image = context.createImageData(raster.width, raster.height);

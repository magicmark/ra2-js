...
function shp(format: number, payload: number[], width = 4, height = 2): Uint8Array {
  ...
  d.setUint16(2, 10, true);
  d.setUint16(4, 12, true);
  d.setUint16(6, 1, true);
  d.setUint16(8, 2, true);
  d.setUint16(10, 3, true);
  d.setUint16(12, width, true);
  d.setUint16(14, height, true);
  bytes[16] = format;
  d.setUint32(28, 32, true);
...
describe('Westwood asset formats', () => {
  it('decodes the authored VPL lighting lookup without treating its redundant palette as material data', () => {
    const bytes = new Uint8Array(784 + 32 * 256),
      data = new DataView(bytes.buffer);

    data.setUint32(0, 16, true);
    data.setUint32(4, 31, true);
    data.setUint32(8, 32, true);
    bytes.fill(77, 16, 784);
    bytes[784 + 25 * 256 + 88] = 15;
    const levels = decodeVpl(bytes);
    expect(levels).toHaveLength(8192);
    expect(levels[25 * 256 + 88]).toBe(15);
  ...
  it('reads both legacy and flagged MIX archives without copying file bodies', () => {
    for (const offset of [0, 4]) {
      const bytes = new Uint8Array(offset + 21),
        d = new DataView(bytes.buffer);

      d.setUint16(offset, 1, true);
      d.setUint32(offset + 2, 3, true);
      d.setUint32(offset + 6, mixHash('hello.dat'), true);
      d.setUint32(offset + 14, 3, true);
      bytes.set([1, 2, 3], offset + 18);
  ...
  it('decrypts RSA-wrapped Blowfish MIX indexes including trailing zero padding', () => {
    const wrapped = Uint8Array.from({ length: 80 }, (_, i) => i + 1),
      key = unwrapMixKey(wrapped);
    ...
    h.setUint16(0, 1, true);
    h.setUint32(2, 3, true);
    h.setUint32(6, mixHash('test.shp'), true);
    h.setUint32(14, 3, true);

    const fish = new Blowfish(key, Blowfish.MODE.ECB, Blowfish.PADDING.NULL),
      bytes = new Uint8Array(111);

    bytes.set([0, 0, 2, 0]);
    bytes.set(wrapped, 4);
    bytes.set(fish.encode(header).subarray(0, 24), 84);
    bytes.set([7, 8, 9], 108);
    const mix = new MixArchive(bytes);
    expect(mix.encrypted).toBe(true);
    expect([...mix.get('test.shp')!]).toEqual([7, 8, 9]);
  });
  it('decodes raw and RLE SHP frames with transparent scanline tails', () => {
    const raw = new ShpFile(shp(0, [1, 2, 3, 4, 5, 6, 7, 8]));
    expect(raw.frame()).toMatchObject({
      width: 4,
      height: 2,
      x: 2,
      y: 3,
  ...
  it('reads VXL column spans, skips empty columns, and associates HVA limb transforms', () => {
    const bytes = new Uint8Array(946),
      d = new DataView(bytes.buffer),
      body = 830,
      footer = 854;
    ...
    d.setUint32(28, 24, true);
    bytes.set(new TextEncoder().encode('hull'), 802);
    d.setInt32(body, 0, true);
    d.setInt32(body + 4, -1, true);
    bytes.set([0, 1, 7, 4, 1, 1, 0, 0], body + 16);
    d.setUint32(footer + 4, 8, true);
    d.setUint32(footer + 8, 16, true);
    ...
    bytes[footer + 91] = 2;
    expect(decodeVxl(bytes)[0].normalType).toBe(2);
    expect(limbs[0].name).toBe('hull');
    expect(limbs[0].voxels).toEqual([{ x: 0, y: 0, z: 0, color: 7, normal: 4 }]);

    const hva = new Uint8Array(88),
      h = new DataView(hva.buffer);

    h.setUint32(16, 1, true);
    h.setUint32(20, 1, true);
    hva.set(new TextEncoder().encode('hull'), 24);
  ...
  it('unpacks RA2 diamond terrain rather than treating it as rectangular pixels', () => {
    const bytes = new Uint8Array(20 + 52 + 900),
      d = new DataView(bytes.buffer);

    d.setUint32(0, 1, true);
    d.setUint32(4, 1, true);
    d.setUint32(8, 60, true);
    d.setUint32(12, 30, true);
    d.setUint32(16, 20, true);
    bytes.fill(10, 72);
    const tile = decodeTmp(bytes);
    expect(tile.pixels.filter((x) => x === 10)).toHaveLength(900);
    expect(tile.pixels[0]).toBe(0);
    expect(tile.pixels[28]).toBe(10);
    expect(tile.pixels[14 * 60]).toBe(10);
  });
});

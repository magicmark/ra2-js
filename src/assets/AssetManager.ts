...
export interface AssetProgress {
  phase: 'cache' | 'awaiting-source' | 'download' | 'extract' | 'decode' | 'ready' | 'error';
  loaded: number;
  total?: number;
  message: string;
}
...
interface InterfaceAssetFrames {
  readonly [asset: string]: readonly number[];
}

export const HUD_ASSET_FRAMES: InterfaceAssetFrames = {
  side1: [0],
  side2: [0],
  side2b: [0],
  side3: [0],
  ...
  'command-deploy': [0, 1],
  'command-guard': [0, 1],
  'command-planning': [0, 1],
  'command-background': [0],
  'command-left': [0, 1, 2],
  'command-right': [0],
};

export const REQUIRED_TERRAIN = [
  'clear01',
  'water01',
  'proad01',
  'proad02',
...
export const DIALOG_ASSET_FRAMES: InterfaceAssetFrames = {
  'options-small': [0],
  'options-medium': [0],
  'options-large': [0],
  'options-button': [0, 1, 2],
  'options-checkbox-on': [0],
  'options-checkbox-off': [0],
  'options-slider-thumb': [0],
};
...
interface CacheEntry {
  ...
  files: AssetFile[];
  saved: number;
  unavailableEnhancements?: string[];
}

interface PreparedVoxel {
  x: number;
  y: number;
  z: number;
  color: number;
...
const SIDE_REMAPS = SIDE_COLORS.map(([h, s, v]) =>
  Array.from({ length: 16 }, (_, index) => {
    const hue = (h / 255) * 6,
      saturation = (s / 255) * Math.sin((index * Math.PI) / 67.5 + Math.PI / 3.6);

    const value = v * Math.cos((index * 7 * Math.PI) / 270 + Math.PI / 9),
...
interface InfantryIdleFrames {
  [sprite: string]: [number, number];
}

const idleFrames: InfantryIdleFrames = {
  gi: [0, 1],
  cons: [0, 1],
  engineer: [0, 1],
  rock: [292, 6],
...
function trimSprite(value: Sprite): Sprite {
  const { source, width, height, anchorX, anchorY } = value,
  ...
  if (right < left || (left === 0 && right === width - 1 && top === 0 && bottom === height - 1))
    return value;

  const result = canvas(right - left + 1, bottom - top + 1),
    target = result.getContext('2d')!,
    pixels = target.createImageData(result.width, result.height);

  for (let y = 0; y < result.height; y++)
    pixels.data.set(
      image.data.subarray(((y + top) * width + left) * 4, ((y + top) * width + right + 1) * 4),
      y * result.width * 4,
    );
...
function paint(frame: IndexedFrame, palette: Uint8Array, side = -1): HTMLCanvasElement {
  const result = canvas(frame.width, frame.height),
  ...
  for (let i = 0; i < frame.pixels.length; i++) {
    ...
    if (side >= 0 && index >= 16 && index <= 31) {
      ...
      image.data[p] = palette[c];
      image.data[p + 1] = palette[c + 1];
      image.data[p + 2] = palette[c + 2];
    }
...
export class AssetManager {
  constructor(
    private readonly catalog: AssetCatalog = CATALOG,
    private readonly resolveTheaterNames: typeof theaterNames = theaterNames,
  ) {}
  ...
  getSounds(): OriginalSounds | undefined {
    return this.ready ? this.sounds : undefined;
  }
  private music: OriginalMusic = [];
  getMusic(): OriginalMusic | undefined {
    return this.ready ? this.music : undefined;
  }
  private font: NativeFont | null = null;
  private cursors: NativeCursors | null = null;
  private shpFiles = new Map<string, ShpFile>();
  ...
  private start(options: AssetOptions, downloading: boolean): Promise<AssetRestoreResult> {
    ...
    const source = assetSourceUrl(options.url || DEFAULT_ASSET_URL),
      key = assetCacheKey(source);

    if (this.active?.key === key && (this.active.downloading || !downloading))
  ...
  private async restore(run: number, source: string): Promise<AssetRestoreResult> {
    ...
    for (const candidate of keys) {
      ...
      try {
        entry = await assetCache<CacheEntry>(candidate);
      } catch (error) {
        if (run !== this.run) return 'cancelled';
        this.storageWarning(
          'selected-read',
          `Browser asset cache unavailable: ${error instanceof Error ? error.message : String(error)}`,
        );
      ...
      if (!entry) continue;
      this.cachedArtworkFiles = Math.max(
        this.cachedArtworkFiles,
      ...
      if (!Number.isInteger(entry.version) || entry.version < 5 || entry.version > CACHE_VERSION) {
        ...
        this.diagnostics.push(
          `Saved asset cache ${candidate} has unsupported version ${entry.version}`,
        );
        result = 'invalid';
        continue;
      }
      ...
      try {
        ...
        this.diagnostics.push(
          `Saved game assets at ${candidate} are invalid: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  ...
  private async runLoad(
    ...
    try {
      ...
      const usableFiles =
        restored === 'ready'
          ? [...this.files].map(([name, bytes]) => ({ name, bytes }))
          : undefined;
      ...
      if (local)
        for (let attempt = 0; attempt < 2; attempt++) {
          this.report(
            'cache',
            `Preparing saved game files from ${local.stage.startsWith('mix') ? 'extracted MIX archives' : 'the completed installer'}…`,
          );
          ...
          try {
            ...
            if (
              ...
              (error instanceof InvalidArchiveError ||
                (error instanceof MissingMusicError && local.stage.startsWith('mix')))
            ) {
              const alternative = await archives.resume({
                skipMix: error instanceof MissingMusicError,
              });
              ...
              if (
                alternative &&
                (alternative.stage !== local.stage || alternative.id !== local.id)
              ) {
                local = alternative;
                continue;
              }
            ...
            this.report(
              'awaiting-source',
              `${this.cacheProblem} Press Enter to retry using saved work, or choose another source.`,
            );
      ...
      if (!downloading) {
        ...
        const message =
          restored === 'invalid' && this.cachedArtworkFiles > 0
            ...
            : restored === 'invalid'
              ? `${this.cacheProblem} Confirm the archive URL and press Enter to download complete game assets.`
              : restored === 'unavailable'
                ? 'Browser storage is unavailable. Confirm the archive URL and press Enter to download for this session.'
      ...
      const downloaded = await archives.download(
      ...
      return run === this.run ? 'invalid' : 'cancelled';
    }
  }
  private storageWarning(stage: string, message: string | null): void {
    if (message) this.storageWarnings.set(stage, message);
    else this.storageWarnings.delete(stage);
  ...
  async importFiles(files: FileList | File[], options: AssetOptions = {}): Promise<void> {
    ...
    this.cacheWarning = null;
    this.ready = false;
    this.source = assetSourceUrl(options.url || this.source || DEFAULT_ASSET_URL);
    this.progress = options.onProgress ?? this.progress;
    ...
    try {
      ...
      if (!inputs.length)
        throw new Error(
          'Choose the Red Alert 2 installer, or ra2.mix, language.mix and theme.mix.',
        );
  ...
  cancel(): void {
    ...
    this.pendingReject?.(new Error('Asset loading cancelled'));
  ...
  async clearCache(): Promise<void> {
    ...
    for (const alias of selectedAssetCacheKeys(this.source))
      if (alias !== key) await assetCache(alias, null);
  }
  private fail(cause: unknown): void {
    this.ready = false;
    this.error = cause instanceof Error ? cause.message : String(cause);

    if (/Failed to fetch|NetworkError|Load failed/i.test(this.error))
      this.error =
        'Asset download could not reach the server. Retry, use a CORS-enabled asset URL, or import the installer / MIX files.';
    this.report('error', this.error);
  }
  private async load(
    files: ArchiveInput[],
    run: number,
    archives: AssetDownload,
    input: ArchiveResume,
  ): Promise<void> {
    if (!files.length)
      throw new Error('Choose the Red Alert 2 installer, or ra2.mix, language.mix and theme.mix.');
    const phase = input.stage.startsWith('mix') ? 'cache' : 'extract';
    this.report(
      phase,
      input.stage.startsWith('mix')
        ? 'Preparing saved game files from MIX archives…'
        : 'Opening the saved installer in the browser…',
    );

    if (run !== this.run) return;

    let staged = input.stage.startsWith('mix')
      ? archives.extracted(input, files)
      : Promise.resolve();
    ...
    try {
      const selected = await new Promise<AssetFile[]>((resolve, reject) => {
        ...
        worker.onmessage = ({ data }) => {
          ...
          if (data.kind === 'mix')
            staged = staged.then(() => archives.extracted(input, data.files));
          ...
          if (data.kind === 'error') {
            ...
            reject(
              data.invalidArchive
                ? new InvalidArchiveError(data.message)
                : data.missingMusic
                  ? new MissingMusicError(data.message)
          ...
          if (data.kind === 'complete') {
            this.diagnostics.splice(
              ...
              ...data.archives.map(
                (a: { name: string; entries: number; encrypted: boolean }) =>
                  `${a.name}: ${a.entries} files${a.encrypted ? ' (encrypted index)' : ''}`,
              ),
            );
            worker.terminate();
            this.worker = undefined;
        ...
        worker.onerror = (event) => {
          ...
          this.worker = undefined;
          this.pendingReject = undefined;
          reject(new Error(event.message || 'Browser archive extractor failed'));
        };
      ...
      try {
        const entry: CacheEntry = { version: CACHE_VERSION, files: selected, saved: Date.now() };

        if (this.missingEnhancements.length)
          entry.unavailableEnhancements = this.missingEnhancements;
        await assetCache(assetCacheKey(this.source), entry, () => run === this.run);

        if (run === this.run) {
          this.storageWarning('selected-read', null);
          this.storageWarning('selected', null);
        }
      } catch (error) {
        if (run === this.run)
          this.storageWarning(
            'selected',
            `Assets loaded; browser storage could not save selected artwork: ${error instanceof Error ? error.message : String(error)}`,
          );
      }
      ...
      if (run === this.run)
        this.report(
          'ready',
          `Original game assets ready · ${selected.length} files${this.cacheWarning ? ' · cache unavailable for some stages' : ' saved in this browser'}`,
          1,
          1,
        );
    } catch (error) {
  ...
  private requiredFile(name: string): Uint8Array {
    ...
    if (!bytes?.length)
      throw new Error(
        `Missing original asset: ${name}. Download or import a complete copy of the game.`,
      );
  ...
  private decodeFile<T>(name: string, decode: (bytes: Uint8Array) => T): T {
    ...
    try {
      ...
      throw new Error(
        `Invalid original asset ${name}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  private validateShp(name: string, frames?: readonly number[]): ShpFile {
    try {
      const shp = this.shp(name);

      if (!shp) throw new Error(`Missing ${name}.shp (or theater alias)`);

      for (const frame of frames ?? Array.from({ length: shp.frameCount }, (_, i) => i)) {
        if (frame >= shp.frameCount) throw new Error(`Missing frame ${frame}`);
        shp.frame(frame);
      }
      ...
      throw new Error(
        `Invalid original sprite ${name}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  private async prepare(files: AssetFile[], run = this.run): Promise<void> {
    ...
    try {
      ...
      this.files = new Map(files.map((f) => [f.name.toLowerCase(), f.bytes]));
      this.shpFiles.clear();
      this.sprites.clear();
      this.voxelModels.clear();
      this.buildingAnchors.clear();
      this.nativePalettes.clear();
      this.theater = 'TEMPERATE';
      this.missingEnhancements = [
        'game.fnt',
        'mouse.shp',
        'mousepal.pal',
        'palette.pal',
        ...
        'side0/sidebar.pal',
        'side0/uibkgd.pal',
        ...DIALOG_SPRITE_FILES.map((name) => `side0/${name}.shp`),
        ...Object.values(DIALOG_PCX_FILES),
        ...EFFECT_ANIMATIONS.map((name) => `${name}.shp`),
        ...PROJECTILE_SPRITES.map((name) => `${name}.shp`),
        ...IFV_TURRET_FILES,
      ].filter((name) => !this.files.has(name));
      ...
      for (const spec of Object.values(this.catalog))
        if (spec.kind === 'building' && !spec.noBuildup) {
          const name = this.resolveTheaterNames(spec.sprite + 'mk', 'URBAN')[0] + '.shp';
      ...
      if (this.requiresNativeMaps)
        for (const theater of NATIVE_THEATERS) {
          for (const kind of ['iso', 'unit', 'overlay'] as const) this.nativePalette(theater, kind);
          const tile = nativeTileSpec(theater, 0);

          if (tile) this.requiredFile(tile.fileName);

          for (const type of ['caoild', 'caairp'])
            if (!this.nativeShp(theater, type))
              throw new Error(
                `Missing original native structure: ${theater} ${type}. Import complete game MIX files.`,
              );
        }
      ...
      this.infantry.clear();
      this.font = this.files.has('game.fnt')
        ? this.decodeFile('game.fnt', (bytes) => new NativeFont(bytes))
        : null;
      this.cursors =
        this.files.has('mouse.shp') && this.files.has('mousepal.pal')
          ? this.decodeFile(
              'mouse.shp',
              (bytes) => new NativeCursors(bytes, this.requiredFile('mousepal.pal')),
            )
          : null;
      this.pipPalette = this.files.has('palette.pal')
        ? this.decodeFile('palette.pal', decodePalette)
        : new Uint8Array();

      if (this.shp('pips')) this.validateShp('pips', [0, 1, 2, 4, 13, 14]);

      if (this.shp('pips2')) this.validateShp('pips2', [0, 2]);
      ...
      this.animationPalette = this.files.has('anim.pal')
        ? this.decodeFile('anim.pal', decodePalette)
        : new Uint8Array();
      this.sidebarPalettes = this.files.has('side0/sidebar.pal')
        ? [this.decodeFile('side0/sidebar.pal', decodePalette)]
        : [];
      this.voxelLighting = this.decodeFile('voxels.vpl', (bytes) => {
        const levels = decodeVpl(bytes);

        if (levels.length !== 32 * 256)
          throw new Error('All 32 material lighting levels are required');

        return levels;
      });

      for (const [name, frames] of Object.entries(HUD_ASSET_FRAMES)) {
        if (!this.files.has(`side0/${name}.shp`)) {
          this.missingEnhancements.push(`side0/${name}.shp`);
          continue;
        }

        this.decodeFile(`side0/${name}.shp`, (bytes) => {
          const shp = new ShpFile(bytes);
      ...
      for (const name of DIALOG_SPRITE_FILES)
        if (this.files.has(`side0/${name}.shp`))
          this.decodeFile(`side0/${name}.shp`, (bytes) => {
            const shp = new ShpFile(bytes);
      ...
      for (const name of Object.values(DIALOG_PCX_FILES))
        if (this.files.has(name)) this.decodeFile(name, decodePcx);

      for (const name of PROJECTILE_SPRITES)
        if (this.shp(name))
          this.validateShp(
            name,
      ...
      for (const name of EFFECT_ANIMATIONS) {
        if (!this.shp(name) || !this.animationPalette.length) continue;

        const shp = this.validateShp(name),
          art = this.art.get(name),
          ticksPerFrame = nativeAnimationInterval(Number(art?.rate ?? 900));

        if (!ticksPerFrame) throw new Error(`Invalid original animation Rate: ${name}`);
        this.effectDefinitions[name] = {
          frames: shp.frameCount,
          ticksPerFrame,
          normalized: /^(yes|true|1)$/i.test(art?.normalized ?? ''),
        };
      }
      ...
      for (const name of REQUIRED_TERRAIN) {
        if (!this.files.has(`${name}.tem`) && !['clear01', 'water01'].includes(name)) {
          this.missingEnhancements.push(`${name}.tem`);
          continue;
        }
      ...
      if (this.files.has('water02.tem'))
        this.decodeFile('water02.tem', (bytes) => {
          for (let frame = 0; frame < 6; frame++) decodeTmp(bytes, frame);
        });

      for (const name of IFV_TURRET_FILES)
        if (this.files.has(name))
          this.decodeFile(name, (bytes) =>
            name.endsWith('.vxl') ? decodeVxl(bytes) : decodeHva(bytes),
          );
      let done = 0;

      for (const [name, spec] of Object.entries(this.catalog)) {
        if (spec.kind === 'vehicle') {
          this.decodeFile(`${spec.sprite}.vxl`, decodeVxl);
          this.decodeFile(`${spec.sprite}.hva`, decodeHva);
        } else this.validateShp(spec.sprite);

        if (spec.kind === 'infantry') {
          const definition = infantryArt(this.art, spec.sprite),
            shp = this.shp(spec.sprite)!;

          const bodyFrames =
            shp.frameCount > 1 && shp.frameCount % 2 === 0 ? shp.frameCount / 2 : shp.frameCount;

          for (const [action, sequence] of Object.entries(definition.sequences)) {
            if (sequence.start + sequence.stride * 7 + sequence.frames > bodyFrames)
              throw new Error(`Invalid original infantry frames: ${spec.sprite}.${action}`);
          }
        ...
        if (name === 'gi')
          this.validateShp(
            spec.sprite,
            Array.from({ length: 71 }, (_, i) => 292 + i),
          );
        this.validateShp(spec.cameo, [0]);

        if (spec.bib) this.validateShp(spec.bib);

        for (const overlay of spec.overlays ?? []) this.validateShp(overlay);

        if (spec.turret) {
          this.decodeFile(`${spec.turret}.vxl`, decodeVxl);
          this.decodeFile(`${spec.turret}.hva`, decodeHva);
        }

        if (!this.getSprite(name, 0, spec.sprite.startsWith('n') ? 1 : 0))
          throw new Error(`Cannot render original sprite: ${spec.sprite}`);

        if (!this.getCameo(name)) throw new Error(`Cannot render original cameo: ${spec.cameo}`);
        this.report(
          'decode',
          `Decoding ${name.replaceAll('_', ' ')}…`,
          ++done,
      ...
      for (let variant = 0; variant < 6; variant++) {
        this.validateShp(`tib${String(variant + 1).padStart(2, '0')}`, [8]);

        if (!this.getOverlay('ore', variant))
          throw new Error(`Cannot render original ore variant ${variant}`);
      }

      for (let variant = 0; variant < 8; variant++) {
        this.validateShp(`tree${String(variant + 1).padStart(2, '0')}`);

        if (!this.getOverlay('tree', variant))
          throw new Error(`Cannot render original tree variant ${variant}`);
      }

      this.missingEnhancements.sort();
      this.enhancementUpgrade = this.missingEnhancements.length > 0;
      validated = true;
    } finally {
  ...
  private spec(name: string): AssetSpec | undefined {
    ...
    return (
      this.catalog[name] ??
      Object.values(this.catalog).find(
        (s) =>
          s.sprite === name ||
          s.cameo === name ||
          NATIVE_THEATERS.some((theater) =>
            this.resolveTheaterNames(s.sprite, theater).includes(name),
          ),
      )
    );
  }
  ...
  private shp(name: string): ShpFile | undefined {
    ...
    const candidate = this.resolveTheaterNames(name, this.theater)
      .flatMap((n) => [n + '.shp', n + '.' + THEATER_EXTENSION[this.theater]])
      .find((n) => this.files.has(n));

    if (!candidate) return;
    let shp = this.shpFiles.get(candidate);

    if (!shp) {
      shp = new ShpFile(this.files.get(candidate)!);
      this.shpFiles.set(candidate, shp);
    }
  ...
  getCameo(name: string): Sprite | null {
    ...
    const spec = this.spec(name),
      file = spec?.cameo ?? name.toLowerCase().replace(/\.shp$/, ''),
      key = `cameo:${file}`;

    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const shp = this.shp(file);
    ...
    const frame = shp.frame(0),
      image = paint(frame, this.cameoPalette),
      result = sprite(image, 0, 0);

    this.sprites.set(key, result);
  ...
  getUIAsset(name: string, frame = 0, side = 0): Sprite | null {
    ...
    const filename = name.toLowerCase().replace(/\.shp$/, ''),
      key = `ui:${side}:${filename}:${frame}`;

    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const bytes = this.files.get(`side${side}/${filename}.shp`);

    if (!bytes || !this.files.has(`side${side}/sidebar.pal`)) return null;

    const shp = new ShpFile(bytes),
      image = shp.frame(frame),
      result = canvas(shp.width, shp.height);
    ...
    result
      ...
      .drawImage(
        paint(
          image,
          this.sidebarPalettes[side] ?? this.decodeFile(`side${side}/sidebar.pal`, decodePalette),
        ),
        image.x,
        image.y,
      );
    const value = sprite(result, 0, 0);
  ...
  getDialogAsset(name: string, frame = 0, side = 0): Sprite | null {
    ...
    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const pcxFile = Object.entries(DIALOG_PCX_FILES).find(([asset]) => asset === name)?.[1];
    ...
    if (pcxFile) {
      ...
      result = paint(
        { ...image, x: 0, y: 0, canvasWidth: image.width, canvasHeight: image.height },
        image.palette,
      );
    } else {
      const filename = new Map([
        ['options-small', 'bkgdsm'],
        ['options-medium', 'bkgdmd'],
        ['options-large', 'bkgdlg'],
        ['options-button', 'sidebttn'],
      ]).get(name);

      if (
        !this.files.has(`side${side}/${filename}.shp`) ||
        !this.files.has(`side${side}/${name === 'options-button' ? 'sidebar' : 'uibkgd'}.pal`)
      )
        return null;

      const shp = this.decodeFile(`side${side}/${filename}.shp`, (bytes) => new ShpFile(bytes)),
        image = shp.frame(frame);

      const palette =
        name === 'options-button'
          ? this.decodeFile(`side${side}/sidebar.pal`, decodePalette)
          : this.decodeFile(`side${side}/uibkgd.pal`, decodePalette);
  ...
  getHarvestSprite(facing: number, time: number): Sprite | null {
    ...
    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const shp = this.shp('oregath');

    if (!shp) return null;
    const frame = shp.frame(index);

    const result = sprite(
      paint(frame, this.animationPalette),
      shp.width / 2 - frame.x,
      shp.height / 2 - frame.y,
    );
  ...
  getPipSprite(
    ...
    const frame = shp.frame(index),
      result = sprite(paint(frame, this.pipPalette), 0, 0);
  ...
  getAnimationDefinitions(): Record<string, NativeAnimationDefinition> {
    return Object.fromEntries(
      Object.entries(this.effectDefinitions).map(([name, definition]) => [name, { ...definition }]),
    );
  }
  getInfantryAnimationDefinitions(): Record<
    string,
    {
      sequences: Record<string, NativeAnimationDefinition & { facing?: number }>;
      fireFrame: number;
      idleFrequency: number;
    }
  > {
    const idleFrequency = Number(
      readArtSections(this.files.get('rules.ini')).get('general')?.idleactionfrequency ?? 0.15,
    );

    return Object.fromEntries(
      [...this.infantry].map(([name, definition]) => [
        name,
        {
          fireFrame: definition.fireFrame,
          idleFrequency,
          sequences: Object.fromEntries(
            Object.entries(definition.sequences).map(([action, sequence]) => {
              const animation: NativeAnimationDefinition & { facing?: number } = {
                frames: sequence.frames,
                ticksPerFrame: sequence.ticksPerFrame,
                normalized: sequence.normalized,
              };

              if (sequence.facing !== undefined) animation.facing = sequence.facing;
  ...
  getInfantrySequence(
    ...
    if (name === 'george')
      return this.ready
        ? (this.customArt?.getInfantrySequence(action, facing, ageSeconds) ?? null)
        : null;
    const spec = this.spec(name);

    if (!spec) return null;
    const sequence = this.infantry.get(spec.sprite)?.sequences[action];

    if (!sequence) return null;

    return this.getInfantryFrame(
      name,
      infantrySequenceFrame(sequence, action, facing, ageSeconds, speedIndex),
      side,
    );
  }
  ...
  getAnimationSprite(name: string, ageSeconds: number, ticksPerFrame?: number): Sprite | null {
    ...
    const definition = this.effectDefinitions[name];
    ...
    const timing =
      ticksPerFrame === undefined
        ? definition
        : { ...definition, ticksPerFrame, normalized: false };

    const index = nativeAnimationFrame(timing, ageSeconds),
    ...
    const frame = shp.frame(index),
      result = sprite(
        paint(frame, this.animationPalette),
        shp.width / 2 - frame.x,
        (name.startsWith('wclbolt') ? shp.height : shp.height / 2) - frame.y,
      );
  ...
  getProjectileSprite(name: string, heading: number): Sprite | null {
    ...
    if (!PROJECTILE_SPRITES.includes(name)) return null;
    const shp = this.shp(name);
    ...
    const result = sprite(
      paint(frame, this.nativePalette(this.theater, 'unit')),
      shp.width / 2 - frame.x,
      shp.height / 2 - frame.y,
    );
  ...
  getSprite(name: string, frame = 0, side = 0): Sprite | null {
    ...
    if (name === 'butchers' || name === 'george')
      return this.customArt?.getSprite(name, frame) ?? null;
    name = name.toLowerCase().replace(/\.(shp|vxl)$/, '');
    const spec = this.spec(name);

    if (!spec) return this.getDecoration(name, frame);
    frame =
      spec.kind === 'infantry'
        ? Math.max(0, Math.floor(frame))
        : ((Math.floor(frame) % 32) + 32) % 32;
    ...
    try {
      if (spec.kind === 'vehicle') result = this.renderVehicle(spec.sprite, frame % 32, side);
        ...
        const shp = this.shp(spec.sprite);

        if (shp) {
          if (spec.kind === 'building') result = this.renderBuilding(shp, spec, side, frame);
          else if (spec.kind === 'spriteVehicle')
            return this.getSpriteVehicle(name, Math.round(frame / 4), 0, side);
          else {
            const ready = this.infantry.get(spec.sprite)?.sequences.Ready;

            const [start, stride] = ready
              ? [ready.start, ready.stride]
              : (idleFrames[spec.sprite] ?? [0, 1]);

            const index =
              frame < 8
                ? start + frame * stride
                : (spec.sprite === 'rock' ? 292 : 8) +
                  (frame % 8) * 6 +
                  (Math.floor((frame - 8) / 8) % 6);
      ...
      this.diagnostics.push(`${name}: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  getInfantryFrame(name: string, frame: number, side = 0): Sprite | null {
    ...
    try {
      ...
      if (!shp) return null;
      const hasShadow = shp.frameCount > 1 && shp.frameCount % 2 === 0;

      if (frame >= shp.frameCount / (hasShadow ? 2 : 1)) return null;

      const image = shp.frame(frame),
        lift = spec.sprite === 'rock' ? 26 : 0;

      const source = canvas(shp.width, shp.height + lift),
        ctx = source.getContext('2d')!;

      if (hasShadow) {
        const shade = shp.frame(frame + shp.frameCount / 2);
        ctx.drawImage(shadow(shade), shade.x, shade.y + lift);
      }

      ctx.drawImage(paint(image, this.unitPalette, side), image.x, image.y);
      result = trimSprite(sprite(source, shp.width / 2, shp.height / 2 + lift));
    } catch (error) {
      this.diagnostics.push(
        `${name} frame ${frame}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  getBuildingSprite(
    ...
    const frames = (spec.overlays ?? []).map((name) => {
      const overlay = this.shp(name);

      return overlay
        ? buildingLoopFrame(
            this.buildingLoops.get(name),
            time,
            overlay.frameCount > 1 && overlay.frameCount % 2 === 0
              ? overlay.frameCount / 2
              : overlay.frameCount,
            speedIndex,
          )
        : 0;
    ...
    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const result = trimSprite(this.renderBuilding(shp, spec, side, 0, frames));
    this.sprites.set(key, result);
  ...
  getFactoryExitSprite(
    ...
    const overlays = (spec.overlays ?? []).filter((name) =>
      layer === 'back' ? name.endsWith('_1') : !name.endsWith('_1'),
    );
    ...
    const frames = overlays.map((name) => {
      ...
      return shp
        ? buildingLoopFrame(this.buildingLoops.get(name), time, shp.frameCount / 2, speedIndex)
        : 0;
    });
    ...
    const result = trimSprite(
      this.renderBuilding(
        shp,
        { ...spec, bib: layer === 'back' ? spec.bib : undefined, overlays },
        side,
        0,
        frames,
        false,
  ...
  private getBuildingTransitionSprite(
    name: string,
    progress: number,
    side: number,
    reverse: boolean,
    ...
    if (!this.ready || !spec || spec.kind !== 'building')
      return this.getBuildingSprite(name, 0, side);
    const buildup = this.art.get(spec.sprite)?.buildup?.toLowerCase() ?? spec.sprite + 'mk';

    const shp = this.shp(buildup),
      base = this.shp(spec.sprite);
    ...
    if (!shp || !base) return this.getBuildingSprite(name, 0, side);

    const frames =
      shp.frameCount > 1 && shp.frameCount % 2 === 0 ? shp.frameCount / 2 : shp.frameCount;

    const frame = reverse
      ? buildingSaleFrame(progress, frames)
      : Math.min(frames - 1, Math.floor(Math.max(0, progress) * frames));
    ...
    const source = canvas(shp.width, shp.height),
      ctx = source.getContext('2d')!;

    if (shp.frameCount === frames * 2) {
      const shade = shp.frame(frame + frames);
      ctx.drawImage(shadow(shade), shade.x, shade.y);
    }
    ...
    const anchorY =
      (this.buildingAnchors.get(spec.sprite) ?? base.height / 2) + (shp.height - base.height) / 2;

    const result = trimSprite(sprite(source, shp.width / 2, anchorY));
    this.sprites.set(key, result);
  ...
  getSpriteVehicle(
    ...
    if (spec?.kind !== 'spriteVehicle') return null;
    const shp = this.shp(spec.sprite);
    ...
    const frame =
      (firing ? 8 * Number(this.art.get(spec.sprite)?.walkframes ?? 1) : 0) +
      direction * frames +
      (Math.floor(time * 10) % frames);
    ...
    const image = shp.frame(frame),
      shade = shp.frame(frame + shp.frameCount / 2);
    ...
    ctx.drawImage(shadow(shade), shade.x, shade.y);
    ctx.drawImage(paint(image, this.unitPalette, side), image.x, image.y);
    const result = trimSprite(sprite(source, shp.width / 2, shp.height / 2));
    this.sprites.set(key, result);
  ...
  getVehicleSprite(
    ...
    if (spec?.kind !== 'vehicle') return null;
    hull = ((Math.round(hull) % 32) + 32) % 32;
    turret = ((Math.round(turret) % 32) + 32) % 32;

    // Older artwork-only caches stay usable until their optional art upgrade.
    const variant =
      spec.sprite === 'fv' &&
      [1, 2, 3].includes(turretVariant) &&
      this.files.has(`fvtur${turretVariant}.vxl`) &&
      this.files.has(`fvtur${turretVariant}.hva`)
        ? turretVariant
        : 0;
    ...
    const result = rendered ? trimSprite(rendered) : null;
    this.sprites.set(key, result);
  ...
  private renderBuilding(
    ...
    const result = canvas(shp.width, shp.height),
      ctx = result.getContext('2d')!;

    const layers = [
      { shp: spec.bib ? this.shp(spec.bib) : undefined, frame: 0 },
      ...(includeBody ? [{ shp, frame: 0 }] : []),
      ...(spec.overlays ?? []).map((name, index) => ({
        shp: this.shp(name),
        frame: overlayFrames[index] ?? 0,
      })),
    ].filter((layer): layer is { shp: ShpFile; frame: number } => !!layer.shp);

    const draw = ({ shp: s, frame: index }: { shp: ShpFile; frame: number }, isShadow = false) => {
      const frame = s.frame(index + (isShadow ? s.frameCount / 2 : 0));
      ctx.drawImage(
        isShadow ? shadow(frame) : paint(frame, this.unitPalette, side),
        frame.x + (shp.width - s.width) / 2,
        frame.y + (shp.height - s.height) / 2,
      );
    };

    for (const layer of layers)
      if (layer.shp.frameCount > 1 && layer.shp.frameCount % 2 === 0) draw(layer, true);

    for (const layer of layers) draw(layer);

    // Ground origin is the centre of the isometric foundation diamond.
    const foundation = spec.footprint ?? [2, 2],
      pixels = ctx.getImageData(0, 0, result.width, result.height).data;
    ...
    while (bottom > 0) {
      ...
      for (let x = 0; x < result.width; x++)
        if (pixels[(bottom * result.width + x) * 4 + 3] > 128) {
          painted = true;
          break;
        }
    ...
    const anchorY =
      this.buildingAnchors.get(spec.sprite) ?? bottom + 5 - (foundation[0] + foundation[1]) * 7.5;
    ...
    if (spec.turret) {
      ...
      if (turret)
        ctx.drawImage(turret.source, result.width / 2 - turret.anchorX, anchorY - turret.anchorY);
    }
  ...
  getTerrain(terrain: string, variant = 0): Sprite | null {
    ...
    const names =
      /^(proad|p_end|green|ruff|sandy|glat|clat|shore|dlat|plat|water|pvclr|clear)\d+$/.test(
        terrain,
      )
        ? [terrain]
        : terrain === 'water'
          ...
          : terrain === 'road'
            ? ['pave01']
            : terrain === 'sand'
              ? ['green01', 'sand01', 'rough01']
              : terrain === 'rock'
                ? ['ruff01', 'rough01', 'rough02']
                : ['clear01'];

    const available = names.filter((name) => this.files.has(name + '.tem'));

    const name =
        available[terrain === 'sand' || terrain === 'rock' ? 0 : variant % available.length],
      key = `terrain:${name}:${variant % 16}`;

    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const bytes = this.files.get(name + '.tem');

    if (!bytes) return null;

    try {
      const frame = decodeTmp(bytes, variant),
        result = sprite(paint(frame, this.terrainPalette), 30, 15);
  ...
  private nativePalette(theater: string, kind: 'iso' | 'unit' | 'overlay'): Uint8Array {
    ...
    if (!palette) {
      palette = decodePalette(this.requiredFile(name));
      this.nativePalettes.set(name, palette);
    }
  ...
  getNativeTerrain(theater: NativeTheater, tileIndex: number, subTile: number): Sprite | null {
    ...
    if (spec.fileName.startsWith('plat') && !this.files.has(spec.fileName))
      return this.getNativeTerrain(theater, theater === 'SNOW' ? 734 : 534, 0);
    const key = `native-tile:${spec.fileName}:${subTile}`;

    if (this.sprites.has(key)) return this.sprites.get(key)!;

    const bytes = this.requiredFile(spec.fileName),
      data = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);

    const count = data.getUint32(0, true) * data.getUint32(4, true);

    if (subTile < 0 || subTile >= count)
      throw new Error(`Invalid native subtile ${spec.fileName}:${subTile}`);

    const frame = decodeTmp(bytes, subTile),
      result = sprite(paint(frame, this.nativePalette(theater, 'iso')), 30, 15);

    this.sprites.set(key, result);

    return result;
  }
  private nativeShp(theater: NativeTheater, name: string): ShpFile | undefined {
    const extension = THEATER_EXTENSION[theater];
    name = name.toLowerCase();
    const variants = this.resolveTheaterNames(name, theater);

    const filename = variants
      .flatMap((value) => [`${value}.${extension}`, `${value}.shp`])
      .find((value) => this.files.has(value));

    if (!filename) return;
    let shp = this.shpFiles.get(filename);

    if (!shp) {
      shp = new ShpFile(this.requiredFile(filename));
      this.shpFiles.set(filename, shp);
    }
  ...
  getNativeDecoration(
    ...
    if (this.sprites.has(key)) return this.sprites.get(key)!;
    const shp = this.nativeShp(theater, name);

    if (!shp) return null;

    if (frame >= shp.frameCount) throw new Error(`Invalid native overlay frame ${name}:${frame}`);

    const image = shp.frame(frame),
      source = canvas(shp.width, shp.height),
      ctx = source.getContext('2d')!;

    if (!overlay && shp.frameCount > 1 && shp.frameCount % 2 === 0) {
      const shade = shp.frame(frame + shp.frameCount / 2);
      ctx.drawImage(shadow(shade), shade.x, shade.y);
    }
    ...
    ctx.drawImage(
      paint(
        ...
        this.nativePalette(
          theater,
          overlay ? 'overlay' : /^tibtre\d+$/i.test(name) ? 'unit' : 'iso',
        ),
      ),
      image.x,
      image.y,
    );
    const result = trimSprite(sprite(source, shp.width / 2, shp.height / 2 + (overlay ? 15 : 0)));
    this.sprites.set(key, result);
  ...
  getNativeFoundation(type: string): [number, number] {
    const art = this.art.get(type.toLowerCase()),
      match = /^(\d+)x(\d+)$/i.exec(art?.foundation ?? '');

    return match
      ? [Number(match[1]), Number(match[2])]
      : type.toUpperCase() === 'CAAIRP'
        ? [3, 3]
        : [2, 2];
  }
  getNativeStructure(
    ...
    const art = this.art.get(type),
      shp = this.nativeShp(theater, art?.image ?? type);

    if (!shp) return null;

    const overlayNames = Object.entries(art ?? {})
      .filter(([name]) => /^(activeanim(?:two|three|four)?|idleanim\d*|bibshape)$/i.test(name))
      .map(([, name]) => name.toLowerCase());

    const layers = [
      { shp, frame: 0, side: -1 },
      ...overlayNames.flatMap((name) => {
        const overlay = this.nativeShp(theater, this.art.get(name)?.image ?? name);
        ...
        return [
          {
            shp: overlay,
            frame: buildingLoopFrame(this.buildingLoops.get(name), time, frames, speedIndex),
            side: name.endsWith('_f') ? side : -1,
          },
        ];
      }),
    ];
    ...
    const draw = (layer: (typeof layers)[number], isShadow: boolean) => {
      const frame = layer.shp.frame(layer.frame + (isShadow ? layer.shp.frameCount / 2 : 0));
      ctx.drawImage(
        isShadow ? shadow(frame) : paint(frame, this.nativePalette(theater, 'unit'), layer.side),
        frame.x + (shp.width - layer.shp.width) / 2,
        frame.y + (shp.height - layer.shp.height) / 2,
      );
    };
    ...
    const base = shp.frame(0),
      foundation = this.getNativeFoundation(type);
    ...
    while (
      bottom > 0 &&
      !base.pixels
        .subarray(bottom * base.width, (bottom + 1) * base.width)
        .some((index) => index > 1)
    )
      bottom--;

    const result = trimSprite(
      sprite(source, source.width / 2, base.y + bottom + 5 - (foundation[0] + foundation[1]) * 7.5),
    );
  ...
  getDecoration(name: string, frame = 0): Sprite | null {
    if (!this.ready && this.preparingRun !== this.run) return null;
    const key = `decoration:${name}:${frame}`;
    ...
    const image = shp.frame(frame),
      groundY = shp.height / 2 + (isOre ? 15 : 0);
    ...
    if (/^tree\d/.test(name) && shp.frameCount > 1 && shp.frameCount % 2 === 0) {
      const source = canvas(shp.width, shp.height),
        ctx = source.getContext('2d')!,
        shade = shp.frame(frame + shp.frameCount / 2);

      ctx.drawImage(shadow(shade), shade.x, shade.y);
      ctx.drawImage(paint(image, palette), image.x, image.y);
      result = sprite(source, shp.width / 2, groundY);
    } else result = sprite(paint(image, palette), shp.width / 2 - image.x, groundY - image.y);
    this.sprites.set(key, result);
  ...
  getOverlay(kind: 'ore' | 'tree', variant = 0): Sprite | null {
    return this.getDecoration(
      kind === 'ore'
        ? `tib${String((variant % 6) + 1).padStart(2, '0')}`
        : `tree${String((variant % 8) + 1).padStart(2, '0')}`,
      kind === 'ore' ? 8 : 0,
    );
  }
  private vehicleModel(name: string, turretVariant = 0): PreparedVoxel[] {
    ...
    for (const part of [
      name,
      name + 'tur' + (turretVariant || ''),
      name + 'barl' + (turretVariant || ''),
    ]) {
      const bytes = this.files.get(part + '.vxl');

      if (!bytes) continue;
      const transforms = this.decodeFile(part + '.hva', decodeHva);

      for (const limb of decodeVxl(bytes))
        this.prepareLimb(limb, transforms.get(limb.name), result, part !== name);
    }

    this.voxelModels.set(key, result);
  ...
  private prepareLimb(
    ...
    result: PreparedVoxel[],
    turret = false,
  ): void {
    const [sx, sy, sz] = limb.size,
      b = limb.bounds,
      scale = [(b[3] - b[0]) / sx, (b[4] - b[1]) / sy, (b[5] - b[2]) / sz];
    ...
    for (const v of limb.voxels) {
      let x = b[0] + (v.x + 0.5) * scale[0],
        y = b[1] + (v.y + 0.5) * scale[1],
        z = b[2] + (v.z + 0.5) * scale[2];
      ...
      if (transform) {
        const t = transform;
        [x, y, z] = [
          t[0] * x + t[1] * y + t[2] * z + t[3] * scale[0] * limb.scale,
          t[4] * x + t[5] * y + t[6] * z + t[7] * scale[1] * limb.scale,
          t[8] * x + t[9] * y + t[10] * z + t[11] * scale[2] * limb.scale,
        ];
        [nx, ny, nz] = [
  ...
  private renderVehicle(
    ...
    const model = this.vehicleModel(name, turretVariant);
    ...
    const radius = model.reduce(
      (max, voxel) => Math.max(max, Math.hypot(voxel.x, voxel.y) + Math.abs(voxel.z) + 16),
      0,
    );

    const size = Math.max(144, Math.ceil(radius) * 2),
      result = canvas(size, size),
      ctx = result.getContext('2d')!,
      output = ctx.createImageData(size, size),
      depth = new Float32Array(size * size).fill(-Infinity);

    const angle = (facing / 32) * Math.PI * 2,
      cs = Math.cos(angle),
      sn = Math.sin(angle),
      center = size / 2,
      ground = size / 2 + 12;

    const turretAngle = (turretFacing / 32) * Math.PI * 2,
      tcs = Math.cos(turretAngle),
      tsn = Math.sin(turretAngle);

    if (castShadow)
      for (const voxel of model) {
        const cosine = voxel.turret ? tcs : cs,
          sine = voxel.turret ? tsn : sn;

        const x = voxel.x * cosine - voxel.y * sine,
          y = voxel.x * sine + voxel.y * cosine;

        const px = Math.round(center + (x - y) * Math.SQRT1_2 + voxel.z * 0.55),
          py = Math.round(ground + (x + y) * Math.SQRT1_2 * 0.5 + voxel.z * 0.15);

        if (px >= 0 && py >= 0 && px < size && py < size)
          output.data[(py * size + px) * 4 + 3] = 105;
      }
    ...
    for (const voxel of model) {
      ...
      const px = Math.round(center + (x - y) * 0.7071),
        py = Math.round(ground + (x + y) * 0.35355 - z * 0.866),
        distance = x + y + z * 0.8165;

      const nx = voxel.nx * cosine - voxel.ny * sine,
        ny = voxel.nx * sine + voxel.ny * cosine;
      ...
      const diffuse = Math.max(0, nx * 0.2013022 + ny * 0.9101138 - voxel.nz * 0.3621709);
      const halfDot = nx * 0.178224 + ny * 0.805757 + voxel.nz * 0.564725;
      const specular = Math.max(0, halfDot / (3 - 2 * halfDot));
      const level = Math.min(31, Math.floor(16 * (diffuse + specular)));

      const c = this.voxelLighting[level * 256 + voxel.color],
        remap = c >= 16 && c <= 31,
        p = c * 3;

      const color = remap
        ? SIDE_REMAPS[side % SIDE_REMAPS.length][c - 16]
        : [this.unitPalette[p], this.unitPalette[p + 1], this.unitPalette[p + 2]];
      ...
      if (depth[index] > distance) continue;
    ...
    return sprite(result, center, ground);
  }
}

...
function shp(color: number, column = 0): Uint8Array {
  const bytes = testShp(4),
...
function manager(files: [string, Uint8Array][]): AssetManager {
  ...
  Object.assign(assets, {
    files: new Map([
      ...files,
      ...['unittem.pal', 'unitsno.pal', 'uniturb.pal'].map((name) => [name, palette] as const),
    ]),
  });
...
function colors(sprite: ReturnType<AssetManager['getSprite']>): number[] {
  ...
  const pixels = testCanvas(sprite!.source).pixels;
...
describe('map theater artwork', () => {
    ...
    for (const id of ['warfactory', 'warfactory_soviet']) {
      ...
      for (const [letter, color] of [
        ...
        for (const [column, name] of [spec.sprite, spec.bib!, ...spec.overlays!].entries()) {
          const bytes = shp(color + column, column);
      ...
      for (const [theater, color] of [
        ...
        expect(colors(front)).toEqual(
          Array.from({ length: spec.overlays!.length - 1 }, (_, n) => (color + 3 + n) * 4),
        );
        expect(back.anchorY).toBe(complete.anchorY);
        expect(front.anchorY).toBe(complete.anchorY);
  ...
  it('extracts every supported theater for building bodies, bibs, animations and buildup', () => {
    ...
    for (const spec of Object.values(CATALOG).filter((spec) => spec.kind === 'building')) {
      for (const name of [
        ...
        for (const letter of ['t', 'a', 'u', 'g'])
          expect(wanted.has(name[0] + letter + name.slice(2) + '.shp')).toBe(true);
      }
    }
  ...
  for (const [id, spec] of Object.entries(CATALOG).filter(([, spec]) => spec.kind === 'building')) {
    it(`${id}: uses generic dry art and Arctic snow art without leaking cached sprites`, () => {
      const assets = manager([
        [spec.sprite + '.shp', shp(30)],
        [spec.sprite[0] + 'g' + spec.sprite.slice(2) + '.shp', shp(10)],
      ]);
  ...
  it('prefers explicit temperate/urban art and keeps bibs, animated layers and sales in the same theater', () => {
    ...
    for (const [letter, color] of [
      ...
      for (const [column, name] of [spec.sprite, spec.bib!, ...spec.overlays!].entries())
        files.push([name[0] + letter + name.slice(2) + '.shp', shp(color + column, column)]);
      files.push([`g${letter}refnmk.shp`, shp(color + 5)]);
    }
    ...
    for (const [theater, color] of [
      ...
      assets.setTheater(theater);
      expect(colors(assets.getBuildingSprite('refinery', 0, -1))).toEqual(
        [color, color + 1, color + 2, color + 3, color + 4].map((value) => value * 4),
      );
      expect(colors(assets.getBuildingSprite('refinery', 1 / 30, -1))).toEqual(
        [color, color + 1, color + 3, color + 4, color + 5].map((value) => value * 4),
      );
      ...
      expect(colors(assets.getBuildingBuildSprite('refinery', 0.5, -1))).toEqual([(color + 6) * 4]);
  ...
  it('does not mistake the Gap Generator cameo for Arctic building artwork', () => {
    for (const theater of NATIVE_THEATERS)
      expect(theaterNames('GAPICON', theater)).toEqual(['gapicon']);
  });
  ...
  it('switches unit palettes along with the theater and invalidates cached pixels', () => {
    ...
    for (const [index, theater] of NATIVE_THEATERS.entries()) {
      const palette = new Uint8Array(768);
      palette.set([index + 1, 0, 0], 30);
      assets['nativePalettes'].set(
        { TEMPERATE: 'unittem.pal', SNOW: 'unitsno.pal', URBAN: 'uniturb.pal' }[theater],
        palette,
      );
    }

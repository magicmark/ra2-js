...
it('uses authored playlist order and Sound aliases, excluding menu music and disabled declarations', () => {
  expect(musicDefinitions(theme)).toEqual([
    { name: 'First', sample: 'song-one' },
    { name: 'Second', sample: 'song-two' },
  ]);
  expect(() => musicDefinitions(encode('[Themes]\n1=Broken\n[Broken]\nSound=../outside'))).toThrow(
    'Sound: Broken',
  );
...
it('selects available original standalone WAV overrides and requires every declared gameplay track', () => {
  ...
  expect(selected.map((file) => file.name)).toEqual([
    'theme.ini',
    'music/song-one.wav',
    'music/song-two.wav',
  ]);
  expect(selected[2].bytes).toEqual(last);
  expect(selected[2].bytes).not.toBe(last);
...
it('retains encoded bytes for lazy playback and rejects missing or malformed selected audio', () => {
  ...
  expect(decodeWave(music[0].bytes).channels[0][1]).toBe(0.5);
  files.set('music/fixture.wav', new Uint8Array(16));
  expect(() => prepareMusic(files)).toThrow('music/fixture.wav: Invalid WAV header');
  files.delete('music/fixture.wav');
  expect(() => prepareMusic(files)).toThrow(MissingMusicError);
});
...
it('validates every IMA block state before music playback, including later corrupt blocks', () => {
  ...
  const wave = audioBagWave(
    { name: 'fixture', offset: 0, size: body.length, sampleRate: 22050, flags: 12, chunkSize: 8 },
    body,
  );
...
it.skipIf(!process.env.RA2_ASSET_DIR)(
  'validates all 13 original gameplay tracks and decodes their actual stereo music',
  () => {
    ...
    for (const track of tracks) {
      ...
      expect(
        decoded.channels[0].some((value) => Math.abs(value) > 0.1),
        track.name,
      ).toBe(true);
    }
  },

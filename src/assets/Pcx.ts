...
export interface PcxImage { width: number; height: number; pixels: Uint8Array; palette: Uint8Array }
...
export function decodePcx(bytes: Uint8Array): PcxImage {
  ...
  const stride = data.getUint16(66, true), paletteStart = bytes.length - 769;
  if (width <= 0 || height <= 0 || width > 4096 || height > 4096 || stride < width || stride > 8192) throw new Error('Invalid PCX dimensions');
  ...
  for (let y = 0; y < height; y++) {
    ...
    while (x < stride) {
      ...
      const code = bytes[offset++], count = (code & 0xc0) === 0xc0 ? code & 0x3f : 1;

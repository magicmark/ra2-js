...
export function infantryArt(art: ArtSections, sprite: string): InfantryArt {
  const type = art.get(sprite), sequence = art.get(type?.sequence?.toLowerCase() ?? '');
...
export function infantrySequenceFrame(sequence: InfantrySequence, action: string, facing: number, ageSeconds: number, speedIndex = NATIVE_SPEED_INDEX): number {
  const tick = Math.max(0, Math.floor(ageSeconds * NATIVE_LOGIC_HZ + 1e-7));
  const interval = sequence.normalized ? nativeNormalizedInterval(sequence.ticksPerFrame, speedIndex) : sequence.ticksPerFrame;

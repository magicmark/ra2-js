...
export function mixHash(name: string, classic = false): number {
  ...
  const padded = new Uint8Array((text.length + 3) & ~3); padded.set(text);
  if (classic) {
    const data = view(padded); let hash = 0;
    for (let i = 0; i < padded.length; i += 4) hash = (((hash << 1) | (hash >>> 31)) + data.getUint32(i, true)) >>> 0;
    return hash;
  }
  if (text.length % 4) {
...
function powerMod(base: bigint, exponent: bigint, modulus: bigint): bigint {
  let result = 1n;
  while (exponent) { if (exponent & 1n) result = result * base % modulus; base = base * base % modulus; exponent >>= 1n; }
  return result;
}
export function unwrapMixKey(source: Uint8Array): Uint8Array {
  ...
  let modulus = 0n;
  for (let i = 2; i < encoded.length; i++) modulus = (modulus << 8n) | BigInt(encoded.charCodeAt(i));
  ...
  for (let block = 0; block < 2; block++) {
    ...
    for (let i = 0; i < 39; i++) { result[block * 39 + i] = Number(value & 255n); value >>= 8n; }
  }
...
export class MixArchive {
  ...
  constructor(readonly bytes: Uint8Array, readonly name = 'archive.mix') {
    check(bytes.length >= 6, `${name}: truncated MIX header`);
    let data = view(bytes), start = data.getUint16(0, true) ? 0 : 4;
    this.encrypted = start === 4 && !!(data.getUint16(2, true) & 2);
    let body: number;
    if (this.encrypted) {
      check(bytes.length >= 92, `${name}: truncated encrypted MIX header`);
      const cipher = new Blowfish(unwrapMixKey(bytes.subarray(4, 84)), Blowfish.MODE.ECB, Blowfish.PADDING.NULL);
      const decrypt = (size: number) => { const out = new Uint8Array(size); out.set(cipher.decode(bytes.subarray(84, 84 + size), Blowfish.TYPE.UINT8_ARRAY)); return out; };
      ...
      data = view(decrypt(size)); start = 0; body = 84 + size;
    } else { body = start + 6 + data.getUint16(start, true) * 12; }
    const count = data.getUint16(start, true);
    check(count > 0 && start + 6 + count * 12 <= data.byteLength && body <= bytes.length, `${name}: invalid MIX index`);
    const bodySize = data.getUint32(start + 2, true);
    check(body + bodySize <= bytes.length, `${name}: truncated MIX body`);
    for (let i = 0; i < count; i++) {
      const p = start + 6 + i * 12, offset = data.getUint32(p + 4, true), size = data.getUint32(p + 8, true);
...
export interface IndexedFrame { width: number; height: number; x: number; y: number; canvasWidth: number; canvasHeight: number; pixels: Uint8Array }
export class ShpFile {
  ...
  constructor(readonly bytes: Uint8Array) {
    ...
    this.width = d.getUint16(2, true); this.height = d.getUint16(4, true); this.frameCount = d.getUint16(6, true);
    check(this.width <= 4096 && this.height <= 4096 && this.frameCount > 0 && 8 + this.frameCount * 24 <= bytes.length, 'Invalid SHP dimensions or frame table');
  }
  frame(index = 0): IndexedFrame {
    const d = view(this.bytes), h = 8 + ((index % this.frameCount + this.frameCount) % this.frameCount) * 24;
    const x = d.getUint16(h, true), y = d.getUint16(h + 2, true), width = d.getUint16(h + 4, true), height = d.getUint16(h + 6, true), format = this.bytes[h + 8];
    check(width <= 4096 && height <= 4096 && format < 4, 'Invalid SHP frame');
    const pixels = new Uint8Array(width * height); let p = d.getUint32(h + 20, true);
    if (p && width && height) {
      check(p < this.bytes.length, 'SHP frame offset out of bounds');
      if (format === 3) {
        for (let row = 0; row < height; row++) {
          check(p + 2 <= this.bytes.length, 'Truncated SHP scanline');
          const end = p + d.getUint16(p, true); p += 2;
          check(end >= p && end <= this.bytes.length, 'Invalid SHP scanline length');
          let col = 0;
          while (p < end) {
            const value = this.bytes[p++];
            if (value) { check(col < width, 'SHP row overflow'); pixels[row * width + col++] = value; }
            else { check(p < end, 'Truncated SHP zero run'); col += this.bytes[p++]; check(col <= width || p === end, 'SHP zero run overflow'); }
          }
        }
      } else {
        let stride = width;
        if (format === 2) { stride = d.getUint16(p, true) - 2; p += 2; }
        check(stride >= 0 && stride <= width && p + stride * height <= this.bytes.length, 'Truncated SHP raw frame');
        for (let row = 0; row < height; row++) { pixels.set(this.bytes.subarray(p, p + stride), row * width); p += stride; }
      }
    }
...
export function decodeVpl(bytes: Uint8Array): Uint8Array {
  ...
  const d = view(bytes), sections = d.getUint32(8, true);
  check(sections > 0 && sections <= 32 && bytes.length >= 784 + sections * 256, 'Invalid VPL lighting table');
...
export function decodeVxl(bytes: Uint8Array): VoxelLimb[] {
  ...
  const count = d.getUint32(20, true), bodySize = d.getUint32(28, true), body = 802 + count * 28, tail = body + bodySize;
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    for (let c = 0; c < size[0] * size[1]; c++) {
      ...
      let p = spanData + offset, z = 0;
      while (z < size[2]) {
        check(p + 2 <= tail, 'Truncated VXL span');
        z += bytes[p++]; const length = bytes[p++];
        check(z + length <= size[2] && p + length * 2 + 1 <= tail, 'Invalid VXL span length');
        for (let k = 0; k < length; k++) limb.voxels.push({ x: c % size[0], y: Math.floor(c / size[0]), z: z++, color: bytes[p++], normal: bytes[p++] });
        p++; check(length > 0 || z >= size[2], 'Empty VXL span');
      }
    }
    return limb;
  });
...
export function decodeHva(bytes: Uint8Array): Map<string, number[]> {
  ...
  const d = view(bytes), count = d.getUint32(20, true), frames = d.getUint32(16, true), result = new Map<string, number[]>();
  check(count > 0 && frames > 0 && 24 + count * 16 + frames * count * 48 <= bytes.length, 'Invalid HVA frame table');
  for (let i = 0; i < count; i++) {
    const name = new TextDecoder().decode(bytes.subarray(24 + i * 16, 40 + i * 16)).replace(/\0.*$/, '');
    result.set(name, Array.from({ length: 12 }, (_, k) => d.getFloat32(24 + count * 16 + i * 48 + k * 4, true)));
  }
  return result;
}
...
export function decodeTmp(bytes: Uint8Array, frame = 0): IndexedFrame {
  ...
  const d = view(bytes), columns = d.getUint32(0, true), rows = d.getUint32(4, true), width = d.getUint32(8, true), height = d.getUint32(12, true);
  check(columns * rows > 0 && columns * rows <= 4096 && width === 60 && height === 30 && 16 + columns * rows * 4 <= bytes.length, 'Invalid RA2 TMP dimensions');
  const offset = d.getUint32(16 + (frame % (columns * rows)) * 4, true), pixels = new Uint8Array(width * height);
  check(offset && offset + 52 + width * height / 2 <= bytes.length, 'Missing TMP tile');
  let p = offset + 52, span = 4;
  for (let y = 0; y < height; y++) {
    const start = y * width + (width - span) / 2;
    pixels.set(bytes.subarray(p, p + span), start); p += span;

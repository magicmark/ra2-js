...
export class NativeFont {
  ...
  constructor(bytes: Uint8Array) {
    ...
    if (this.view.getUint32(0, false) !== 0x666f6e54)
      throw new Error('Invalid Unicode font signature');
    ...
    if (
      ...
      !count ||
      count > 65535 ||
      this.glyphSize !== 1 + this.stride * this.rows
    )
      throw new Error('Invalid Unicode font geometry');

    if (NativeFont.glyphStart + count * this.glyphSize > bytes.length)
      throw new Error('Truncated Unicode font glyphs');

    for (let code = 0; code < 65536; code++)
      if (this.view.getUint16(28 + code * 2, true) > count)
        throw new Error('Unicode font index exceeds glyph count');

    for (let index = 0; index < count; index++)
      if (bytes[NativeFont.glyphStart + index * this.glyphSize] > this.stride * 8)
        throw new Error('Unicode font glyph exceeds row width');
  }
  ...
  rasterize(text: string, color: readonly number[] = [255, 255, 0], shadow = true) {
    const glyphs = [...text].map((character) => {
      ...
      if (!index)
        throw new Error(`Original game font is missing character ${JSON.stringify(character)}`);
    ...
    const width = Math.max(
      1,
      glyphs.reduce((sum, glyph) => sum + glyph.width + 1, 0) - 1 + Number(shadow),
    );

    const height = this.rows + Number(shadow),
    ...
    for (const glyph of glyphs) {
      for (let y = 0; y < this.rows; y++)
        for (let x = 0; x < glyph.width; x++) {
          if (!(this.bytes[glyph.offset + 1 + y * this.stride + (x >> 3)] & (0x80 >> (x & 7))))
            continue;

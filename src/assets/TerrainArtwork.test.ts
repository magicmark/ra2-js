...
it('paints ore drill foreground in unit colors at full opacity', () => {
  ...
  const unit = new Uint8Array(768),
    iso = new Uint8Array(768);

  unit.set([12, 21, 31], 30);
  iso.set([60, 2, 61], 30);
  Object.assign(assets, {
    files: new Map([
      ['tibtre01.tem', testShp(22)],
      ['unittem.pal', unit],
      ['isotem.pal', iso],
    ]),
  });
  const art = assets.getNativeDecoration('TEMPERATE', 'TIBTRE01')!;
  expect(Array.from(testCanvas(art.source).pixels)).toEqual([48, 84, 124, 255]);
});
...
describe.skipIf(!process.env.RA2_ASSET_DIR)('original ore drill foreground and shadows', () => {
  it('retains each opaque original unit-palette pixel throughout the eleven-frame cycle', () => {
    ...
    assets.ready = true;
    Object.assign(assets, {
      files: new Map(['tibtre01.tem', 'unittem.pal'].map((name) => [name, get(name)])),
    });

    const shp = new ShpFile(get('tibtre01.tem')),
      palette = decodePalette(get('unittem.pal'));

    expect(shp.frameCount).toBe(22);

    for (let n = 0; n < 11; n++) {
      const art = assets.getNativeDecoration('TEMPERATE', 'TIBTRE01', n)!,
        pixels = testCanvas(art.source).pixels,
        frame = shp.frame(n);

      const left = shp.width / 2 - art.anchorX,
        top = shp.height / 2 - art.anchorY;
      ...
      for (let y = 0; y < frame.height; y++)
        for (let x = 0; x < frame.width; x++) {
          ...
          expect(Array.from(pixels.slice(p, p + 4))).toEqual([
            ...palette.slice(color * 3, color * 3 + 3),
            255,
          ]);
        }

      expect(
        pixels.some((v, i) => i % 4 === 3 && v === 105),
        'separate translucent ground shadow',
      ).toBe(true);
    }
  });
});

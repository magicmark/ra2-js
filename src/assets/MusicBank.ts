...
export interface MusicTrack { name: string; bytes: Uint8Array }
...
function required(files: { get(name: string): Uint8Array | undefined }, name: string): Uint8Array {
  ...
  if (!bytes) throw new MissingMusicError(`Missing original music asset: ${name}. Import the complete installer, or ra2.mix, language.mix and theme.mix.`);
  return bytes;
}
...
export function musicDefinitions(theme: Uint8Array): { name: string; sample: string }[] {
  const sections = readArtSections(theme), themes = sections.get('themes');
  ...
  for (const name of Object.values(themes)) {
    ...
    if (!sample || !/^[a-z0-9_-]+$/.test(sample)) throw new Error(`Invalid original music Sound: ${name}`);
...
export function selectMusicFiles(archives: readonly Archive[]): { name: string; bytes: Uint8Array }[] {
  const ordered = [...archives].reverse();
  const files = { get: (name: string) => ordered.map(archive => archive.get(name)).find(Boolean) };
  const theme = required(files, 'theme.ini');
  return [{ name: 'theme.ini', bytes: theme.slice() }, ...musicDefinitions(theme).map(({ sample }) => ({
    name: `music/${sample}.wav`, bytes: required(files, `${sample}.wav`).slice(),
  }))];
}
...
export function prepareMusic(files: ReadonlyMap<string, Uint8Array>): OriginalMusic {
  return musicDefinitions(required(files, 'theme.ini')).map(({ name, sample }) => {
    const file = `music/${sample}.wav`, bytes = required(files, file);
    try { validateWave(bytes); }

...
export function buildingLoops(bytes: Uint8Array | undefined): Map<string, BuildingLoop> {
  ...
  for (const [name, art] of sections) {
    ...
    const ticksPerFrame = nativeAnimationInterval(Number(art.rate ?? 900)),
      normalized = /^(yes|true|1)$/i.test(art.normalized ?? '');

    if (Number.isInteger(start) && Number.isInteger(end) && end > start && ticksPerFrame > 0)
      loops.set(name, { start, end, ticksPerFrame, normalized });
  }

  return loops;
}

export function buildingLoopFrame(
  loop: BuildingLoop | undefined,
  time: number,
  frameCount: number,
  ...
  const end = Math.min(loop.end, frameCount),
    start = Math.min(loop.start, Math.max(0, end - 1));
  ...
  const interval = loop.normalized
    ? nativeNormalizedInterval(loop.ticksPerFrame, speedIndex)
    : loop.ticksPerFrame;

...
import { AssetDownload, assetCache, assetCacheKey, assetSourceUrl, archiveStageKey, ORIGINAL_ASSET_URL, LEGACY_ASSET_URL, requestPersistentAssetStorage, type SavedArchive } from './AssetDownload';

let fetchArchive: ReturnType<typeof vi.fn>;
beforeEach(() => {
  vi.stubGlobal('indexedDB', new IDBFactory());
  fetchArchive = vi.fn(async () => new Response(new Uint8Array([77, 90, 0, 0]), { headers: { 'content-length': '4' } }));
  vi.stubGlobal('fetch', fetchArchive);
});
afterEach(() => { vi.restoreAllMocks(); vi.unstubAllGlobals(); });
const loader = (source = ORIGINAL_ASSET_URL, alive = () => true) => new AssetDownload(source, alive, () => {}, new Map());

it.each([true, false])('preserves the browser persistence decision %s without changing saved assets', async decision => {
...
it('tolerates absent or unavailable persistent-storage support', async () => {
  ...
  expect(await requestPersistentAssetStorage()).toBeUndefined();
  vi.stubGlobal('navigator', { storage: { persist: () => Promise.reject(new TypeError('Unavailable storage')) } });
...
it('fetches the public URL directly, commits before extraction, and restores exact bytes under the legacy cache alias', async () => {
  ...
  expect(reopened?.id).toBe(saved.id); expect(reopened?.stage).toBe('download');
...
it.each(['/asset-source', LEGACY_ASSET_URL, ORIGINAL_ASSET_URL])('reuses completed archives for built-in source alias %s', async source => {
  const saved = await loader('/asset-source').remember([{ name: 'original.exe', blob: new Blob(['MZoriginal']) }]);
...
it.each([
  ...
  ['truncated response', () => new Response(new Uint8Array([77, 90]), { headers: { 'content-length': '100' } }), /Incomplete asset download/],
  ['partial range', () => new Response(new Uint8Array([77, 90, 0, 0]), { status: 206, headers: { 'content-range': 'bytes 0-3/100' } }), /only part of the archive/],
  ['HTTP error', () => new Response(null, { status: 503 }), /HTTP 503/],
] as const)('never persists an invalid %s as a completed archive', async (_name, response, message) => {
  fetchArchive.mockResolvedValueOnce(response());
  await expect(loader().download(new AbortController().signal, () => {})).rejects.toThrow(message);
  expect(await loader().resume()).toBeUndefined();
});

it('does not persist a stream that disconnects before completion', async () => {
  let reads = 0;
  fetchArchive.mockResolvedValueOnce(new Response(new ReadableStream({ pull(controller) {
    if (reads++ === 0) controller.enqueue(new Uint8Array([77, 90])); else controller.error(new Error('Connection interrupted'));
  } })));
  await expect(loader().download(new AbortController().signal, () => {})).rejects.toThrow(/Connection interrupted/);
  expect(await loader().resume()).toBeUndefined();
});
...
it('cancels a late stage write before commit without overwriting an earlier completed download', async () => {
  ...
  vi.spyOn(IDBObjectStore.prototype, 'put').mockImplementation(function (this: IDBObjectStore, ...args: Parameters<typeof original>) {
    const request = original.apply(this, args); request.addEventListener('success', () => { alive = false; }); return request;
  });
  await loader(ORIGINAL_ASSET_URL, () => alive).remember([{ name: 'replacement.exe', blob: new Blob(['MZreplacement']) }]);
...
it('rejects malformed persisted stage shapes without touching selected artwork', async () => {
  ...
  await assetCache(archiveStageKey('/asset-source', 'mix'), { version: 1, id: 'broken', files: [{ name: 'ra2.mix', blob: 'not a Blob' }], saved: 1 });
  expect(await loader().resume()).toBeUndefined();
  expect(await assetCache('/asset-source')).toEqual({ version: 8, files: ['selected artwork'], saved: 1 });
});

it('does not mistake a rejected downloaded candidate for a verified installer on a new page', async () => {
  const downloader = loader(), input = await downloader.download(new AbortController().signal, () => {});
...
it('keeps the installer eligible when only its saved MIX representation is structurally damaged', async () => {
  ...
  await assetCache(archiveStageKey('/asset-source', 'mix'), { ...input, files: [{ name: 'ra2.mix', blob: new Blob(['damaged MIX']) }] });
  const archives = loader(), mix = (await archives.resume())!;
...
it.each(['[object Object]', 'Archive extraction failed (7-Zip code 8). Choose a complete game installer.'])('retries an old ambiguous/runtime rejection locally, but honors a new confirmed container rejection: %s', async rejected => {
  const input = await loader().download(new AbortController().signal, () => {}), key=archiveStageKey('/asset-source','download');

...
import { DIALOG_PCX_FILES, DIALOG_SPRITE_FILES, EFFECT_ANIMATIONS } from './catalog';

// Exercise the real decoder with a focused roster and literal fixture filenames.
class FixtureAssetManager extends AssetManager {
  constructor() {
    super({ gi: { sprite: 'gi', cameo: 'giicon', kind: 'infantry' } }, (name) => [name]);
  }
}
...
interface FixtureWorker {
  ...
  terminated: boolean;
  terminate(): void;
  postMessage(input: { files: { name: string; blob: Blob }[]; includeMixStage?: boolean }): void;
}
...
function expectFiles(actual: AssetFile[] | undefined, expected: AssetFile[]) {
  ...
  expected.forEach((file, index) =>
    expect(Buffer.from(actual![index].bytes).equals(Buffer.from(file.bytes)), file.name).toBe(true),
  );
}
...
function expectEntry(actual: Entry | undefined, expected: Entry | undefined) {
  ...
  expect({ version: actual!.version, saved: actual!.saved }).toEqual({
...
function files(): AssetFile[] {
  const tile = new Uint8Array(972),
    tmp = new DataView(tile.buffer);

  [1, 1, 60, 30, 20].forEach((n, i) => tmp.setUint32(i * 4, n, true));
  tile.fill(10, 72);
  const palette = new Uint8Array(768);
  palette.set([63, 31, 7], 30);
  const vpl = new Uint8Array(784 + 8192);
  new DataView(vpl.buffer).setUint32(8, 32, true);
  ...
  return [
    ...
    ...[
      ...
      'side0/sidebar.pal',
      'side0/uibkgd.pal',
    ].map((name) => ({ name, bytes: palette.slice() })),
    {
      name: 'art.ini',
    ...
    { name: 'oregath.shp', bytes: testShp(120) },
    { name: 'pips.shp', bytes: testShp(19) },
    { name: 'pips2.shp', bytes: testShp(18) },
    { name: 'tibtre01.tem', bytes: testShp(60) },
    { name: 'tree20.tem', bytes: testShp(2) },
    { name: 'plat02.tem', bytes: tile.slice() },
    { name: 'gapowrmk.shp', bytes: testShp(20) },
    { name: 'mouse.shp', bytes: testCursorShp() },
    { name: 'mousepal.pal', bytes: palette.slice() },
    { name: 'voxels.vpl', bytes: vpl },
    { name: 'gi.shp', bytes: testShp(756) },
    { name: 'giicon.shp', bytes: testShp() },
    ...Array.from({ length: 6 }, (_, i) => ({
      name: `tib${String(i + 1).padStart(2, '0')}.tem`,
      bytes: testShp(9),
    })),
    ...Array.from({ length: 8 }, (_, i) => ({
      name: `tree${String(i + 1).padStart(2, '0')}.tem`,
      bytes: testShp(2),
    })),
    ...REQUIRED_TERRAIN.map((name) => ({ name: `${name}.tem`, bytes: tile.slice() })),
    ...Object.entries(HUD_ASSET_FRAMES).map(([name, frames]) => ({
      name: `side0/${name}.shp`,
      bytes: testShp(Math.max(...frames) + 1),
    })),
    ...DIALOG_SPRITE_FILES.map((name) => ({
      name: `side0/${name}.shp`,
      bytes: testShp(name === 'sidebttn' ? 3 : 1),
    })),
    ...Object.values(DIALOG_PCX_FILES).map((name) => ({ name, bytes: testPcx() })),
    ...EFFECT_ANIMATIONS.map((name) => ({ name: `${name}.shp`, bytes: testShp(12) })),
    { name: 'dragon.shp', bytes: testShp(32) },
  ];
}
...
async function entry(
  key = assetCacheKey(DEFAULT_ASSET_URL),
  value?: Entry,
  ...
  return new Promise((resolve, reject) => {
    ...
    transaction.oncomplete = () => {
      db.close();
      resolve(value ? value : request.result);
    };
...
function pixels(manager: AssetManager): number[] {
  ...
  return Array.from(testCanvas(sprite!.source).pixels);
}
...
let makeWorker: ReturnType<typeof vi.fn<() => FixtureWorker>>;

beforeEach(() => {
  extracted = files();
  workerFailure = undefined;
  workerInvalid = false;
  workerMixThenFailure = false;
  workerMissingMusic = false;
  workerInputs = [];
    ...
    const worker: FixtureWorker = {
      ...
      postMessage(input: { files: { name: string; blob: Blob }[]; includeMixStage?: boolean }) {
        ...
        queueMicrotask(() => {
          ...
          if (workerMissingMusic && input.files.every((file) => /\.mix$/i.test(file.name))) {
            this.onmessage?.({
              data: {
                ...
                message:
                  'Missing original music asset: grinder.wav. Import the complete installer, or ra2.mix, language.mix and theme.mix.',
              },
            });
          ...
          if ((!workerFailure || workerMixThenFailure) && input.includeMixStage !== false)
            this.onmessage?.({
              data: {
                kind: 'mix',
                files: [{ name: 'ra2.mix', blob: new Blob(['validated MIX fixture']) }],
              },
            });
          this.onmessage?.({
            data: workerFailure
...
describe('durable extracted asset cache', () => {
  it('recovers omitted music from the saved installer when old MIX caches lack theme.mix, then reopens without extraction', async () => {
    await new FixtureAssetManager().download();

    const saved = (await entry())!,
      key = archiveStageKey('/asset-source', 'mix');

    const archive = await assetCache<SavedArchive>(key);
    await entry('/asset-source', {
      ...saved,
      files: saved.files.filter(
        (file) => file.name !== 'theme.ini' && !file.name.startsWith('music/'),
      ),
    });
    workerMissingMusic = true;
    download.mockClear();
    makeWorker.mockClear();
    workerInputs = [];
    const manager = new FixtureAssetManager();
    expect(await manager.initialize()).toBe('ready');
    expect(manager.getMusic()?.map((track) => track.name)).toEqual(['Fixture']);
    expect(workerInputs.map((input) => input.files[0].name)).toEqual([
      'ra2.mix',
      'Red-Alert-2-Multiplayer.exe',
    ]);
    ...
    expect(await new FixtureAssetManager().initialize()).toBe('ready');
    expect(makeWorker).toHaveBeenCalledTimes(2);
    expect(download).not.toHaveBeenCalled();
  });
  ...
  it('keeps missing music at the explicit source gate without an installer, preserving valid partial MIXes', async () => {
    ...
    makeWorker.mockClear();
    const manager = new FixtureAssetManager();
    expect(await manager.initialize()).toBe('invalid');
    expect(manager.getMusic()).toBeUndefined();
    expect(manager.status.phase).toBe('awaiting-source');
  ...
  it('repairs corrupt selected music from saved MIXes without fetching or losing the archive', async () => {
    ...
    await entry('/asset-source', {
      ...
      files: saved.files.map((file) =>
        file.name === 'music/fixture.wav' ? { ...file, bytes: new Uint8Array(5) } : file,
      ),
    });
    download.mockClear();
    makeWorker.mockClear();
  ...
  it('reselects missing original sounds from saved MIX files without downloading or rejecting archives', async () => {
    ...
    await entry('/asset-source', {
      ...saved,
      files: saved.files.filter((file) => !file.name.startsWith('audio/')),
    });
    download.mockClear();
    makeWorker.mockClear();
    const manager = new FixtureAssetManager();
  ...
  it('asks for an explicit source when an old artwork-only cache has no sound or reusable archive', async () => {
    const old = {
      version: 8,
      files: files().filter((file) => !file.name.startsWith('audio/')),
      saved: 1,
    };
    ...
    expect(manager.getSounds()).toBeUndefined();
    expect(manager.status.phase).toBe('awaiting-source');
    expect(manager.status.message).toContain('sound');
    expect(download).not.toHaveBeenCalled();
  ...
  it('continues immediately with already validated same-source artwork without re-reading storage or decoding', async () => {
    ...
    const open = vi.spyOn(indexedDB, 'open').mockImplementation(() => {
      throw new Error('Storage must not be read by cached Continue');
    });

    await manager.download({ url: ORIGINAL_ASSET_URL, onProgress: (p) => phases.push(p.phase) });
    expect(phases).toEqual(['ready']);
    expect(manager.ready).toBe(true);
    expect(manager.getSprite('gi')).toBe(sprite);
    expect(manager.getFont()).toBe(font);
  ...
  it('starts legacy artwork with its sound bank without fonts, optional art, archives, or a download', async () => {
    ...
    while (savedFiles.length < 126)
      savedFiles.push({
        name: `legacy-extra-${savedFiles.length}.bin`,
    ...
    expect(manager.ready).toBe(true);
    expect(manager.getFont()).toBeNull();
    expect(manager.getCursors()).toBeNull();
    expect(manager.getPipSprite('veteran')).toBeNull();
    expect(manager.getHarvestSprite(0, 0)).toBeNull();
    expect(manager.getDialogAsset('options-small')).toBeNull();
    expect(manager.getAnimationSprite('piffpiff', 0)).toBeNull();
    expect(manager.getSprite('gi')).not.toBeNull();
    expect(manager.getTerrain('grass')).not.toBeNull();
  ...
  it('upgrades the pre-rocket cache from saved MIXes once without a download', async () => {
    ...
    await entry('/asset-source', {
      ...saved,
      files: saved.files.filter((file) => file.name !== 'dragon.shp'),
    });
    download.mockClear();
    makeWorker.mockClear();
    const recovered = new FixtureAssetManager();
    expect(await recovered.initialize()).toBe('ready');
  ...
  it('keeps usable legacy artwork when a cached-archive enhancement fails', async () => {
    ...
    const saved = (await entry())!,
      legacy = {
        ...saved,
        files: saved.files.filter((file) => !['game.fnt', 'oregath.shp'].includes(file.name)),
      };

    await entry('/asset-source', legacy);
    workerFailure = 'Transient extraction failure';
    download.mockClear();
    makeWorker.mockClear();
    const reopened = new FixtureAssetManager();
    expect(await reopened.initialize()).toBe('ready');
    expect(reopened.ready).toBe(true);
    expect(reopened.getFont()).toBeNull();
    expect(reopened.getSprite('gi')).not.toBeNull();
    expectEntry(await entry(), legacy);
    expect(download).not.toHaveBeenCalled();
    expect(makeWorker).toHaveBeenCalledOnce();
  });
  ...
  it('does not repeatedly extract an archive that lacks the same optional artwork, but retries a newly missing enhancement', async () => {
    ...
    await entry('/asset-source', {
      ...saved,
      files: saved.files.filter((file) => file.name !== 'game.fnt'),
    });
    const repaired = new FixtureAssetManager();
    expect(await repaired.initialize()).toBe('ready');
    expect(repaired.getFont()).not.toBeNull();
    expect(await new FixtureAssetManager().initialize()).toBe('ready');
    expect(makeWorker).toHaveBeenCalledTimes(2);
    expect(download).toHaveBeenCalledOnce();
  });
  ...
  it('resumes a completed download after a transient worker failure and a fresh page without another request', async () => {
    ...
    expect(candidate?.files[0].blob.size).toBe(4);
    ...
    expect(workerInputs[1].files[0].name).toMatch(/\.exe$/);
  ...
  it('resumes saved MIX inputs after interrupted selection without rerunning installer extraction', async () => {
    workerFailure = 'Worker stopped while selecting files';
    workerMixThenFailure = true;
    await new FixtureAssetManager().download();
    ...
    expect(await new FixtureAssetManager().initialize()).toBe('ready');
    expect(download).toHaveBeenCalledTimes(1);
    expect(workerInputs[1].files[0].name).toBe('ra2.mix');
  });

  it.each(['missing', 'corrupt'])(
    'does not poison verified archives after a %s artwork selection failure',
    async (failure) => {
      await new FixtureAssetManager().download();

      const saved = (await entry())!,
        mix = await assetCache<SavedArchive>(archiveStageKey('/asset-source', 'mix'));

      const installer = await assetCache<SavedArchive>(
        archiveStageKey('/asset-source', 'installer'),
      );

      await entry('/asset-source', {
        ...saved,
        files: saved.files.filter((file) => file.name !== 'gi.shp'),
      });
      extracted =
        failure === 'missing'
          ? files().filter((file) => file.name !== 'gi.shp')
          : files().map((file) =>
              file.name === 'gi.shp' ? { ...file, bytes: new Uint8Array(3) } : file,
            );
      const failed = new FixtureAssetManager();
      expect(await failed.initialize()).toBe('invalid');
      expect(failed.ready).toBe(false);
      expect(failed.status.phase).toBe('awaiting-source');
      expect(
        (await assetCache<SavedArchive>(archiveStageKey('/asset-source', 'mix')))?.rejected,
      ).toBeUndefined();
      expect(
      ...
      expect(
        workerInputs
          .slice(1)
          .every((input) => input.files.every((file) => /\.mix$/i.test(file.name))),
      ).toBe(true);
      expect((await assetCache<SavedArchive>(archiveStageKey('/asset-source', 'mix')))?.id).toBe(
        mix?.id,
      );
      expect(
        (await assetCache<SavedArchive>(archiveStageKey('/asset-source', 'installer')))?.id,
      ).toBe(installer?.id);
    },
  );

  it.each([
    'Missing original asset: game.fnt.',
    'Invalid original sprite gi: Invalid TS SHP header',
    'Invalid original asset voxels.vpl: invalid table',
    'Cannot render original sprite: gi',
  ])(
    'recovers pre-existing rejected archive stages after %s without fetching again',
    async (rejected) => {
      await new FixtureAssetManager().download();
      const saved = (await entry())!;
      await entry('/asset-source', {
      ...
      for (const stage of ['mix', 'installer'] as const) {
        const key = archiveStageKey('/asset-source', stage),
          archive = (await assetCache<SavedArchive>(key))!;
      ...
      const reopened = new ReopenedManager(
  ...
  it('repairs a structurally damaged MIX stage from its saved installer in one bounded cache-only initialization', async () => {
    ...
    const ordinary = makeWorker.getMockImplementation()!;
    makeWorker.mockImplementation(function () {
      ...
      worker.postMessage = function (input: any) {
        if (input.files.every((file: any) => /\.mix$/i.test(file.name))) {
          ...
          queueMicrotask(() =>
            this.onmessage?.({
              data: { kind: 'error', invalidArchive: true, message: 'ra2.mix: truncated MIX body' },
            }),
          );
    ...
    expect(await new FixtureAssetManager().initialize()).toBe('ready');
    expect(download).not.toHaveBeenCalled();
    expect(workerInputs.slice(1).map((input) => input.files[0].name)).toEqual([
      'ra2.mix',
      'Red-Alert-2-Multiplayer.exe',
    ]);
    expectFiles((await entry())?.files, saved.files);
  ...
  it('keeps completed extraction across decode cancellation and resumes on a new page', async () => {
    ...
    await manager.download({
      onProgress: (progress) => {
        if (progress.phase === 'decode') manager.cancel();
      },
    });
    expect(manager.ready).toBe(false);
    expect(await entry()).toBeUndefined();
  ...
  it('rejects a no-MIX candidate without automatic retry, then lets another explicit submission download corrected bytes', async () => {
    workerFailure = 'No game MIX files found';
    workerInvalid = true;
    await new FixtureAssetManager().download();
    expect(
    ...
    expect(await new FixtureAssetManager().initialize()).toBe('missing');
    expect(download).toHaveBeenCalledTimes(1);
    expect(makeWorker).toHaveBeenCalledTimes(1);
    workerFailure = undefined;
    workerInvalid = false;
    const fixed = new FixtureAssetManager();
    await fixed.download();
  ...
  it('retains archive-stage quota warnings when the smaller selected-art cache saves successfully', async () => {
    ...
    vi.spyOn(IDBObjectStore.prototype, 'put').mockImplementation(function (
      ...
      if (Array.isArray(args[1]))
        throw new DOMException('Large stages exceed quota', 'QuotaExceededError');
    ...
    expect(manager.cacheWarning).toMatch(/completed installer download/);
    expect(manager.cacheWarning).toMatch(/extracted MIX files/);
  ...
  it('never reports partially decoded originals as ready', async () => {
    const manager = new FixtureAssetManager(),
      samples: { phase: string; ready: boolean }[] = [];

    await manager.download({
      onProgress: (progress) => samples.push({ phase: progress.phase, ready: manager.ready }),
    });
    expect(
      samples.filter((sample) => sample.phase !== 'ready').every((sample) => !sample.ready),
    ).toBe(true);
  ...
  it.each([
    ...
    'repairs selected artwork missing %s from durable MIX inputs without downloading or unpacking the EXE',
    async (name) => {
      await new FixtureAssetManager().download();
      const saved = (await entry())!;
      await entry('/asset-source', {
        ...saved,
        files: saved.files.filter((file) => file.name !== name),
        version: 5,
      });
      const manager = new FixtureAssetManager();
      const phases: string[] = [];
      ...
      expectFiles((await entry())?.files, saved.files);
      await new FixtureAssetManager().initialize();
      expect(download).toHaveBeenCalledTimes(1);
      expect(makeWorker).toHaveBeenCalledTimes(2);
    },
  );
  ...
  it.each([
    ...
    'tree08.tem',
    'giicon.shp',
  ])('rejects corrupt consumed payload %s instead of loading substitute artwork', async (name) => {
    extracted = files().map((file) =>
      file.name === name ? { ...file, bytes: new Uint8Array(3) } : file,
    );
    const manager = new FixtureAssetManager();
    await manager.download();
    expect(manager.ready).toBe(false);
  ...
  it('rejects a short radar frame table and incomplete authored material levels', async () => {
    extracted = files().map((file) =>
      file.name === 'side0/radar.shp' ? { ...file, bytes: testShp(1) } : file,
    );
    const manager = new FixtureAssetManager();
    await manager.download();
    expect(manager.error).toMatch(/radar.*frame 32/);
    expect(manager.ready).toBe(false);
    extracted = files();
    new DataView(extracted.find((file) => file.name === 'voxels.vpl')!.bytes.buffer).setUint32(
      8,
      1,
      true,
    );
    await manager.download();
    expect(manager.error).toMatch(/voxels.vpl.*32/);
    expect(manager.ready).toBe(false);
    expect(await entry()).toBeUndefined();
  });
  ...
  it('refreshes usable artwork only on an explicit forceRefresh and preserves it if the update fails', async () => {
    ...
    await manager.download({ forceRefresh: true });
    expect(download).toHaveBeenCalledTimes(2);
    expect(manager.error).toMatch(/503/);
  ...
  it('commits extracted files once; a fresh module and manager render identical pixels without download or extraction', async () => {
    ...
    makeWorker.mockClear().mockImplementation(() => {
      throw new Error('Extraction must not start');
    });

    const reopened = new FreshAssetManager(
        { gi: { sprite: 'gi', cameo: 'giicon', kind: 'infantry' } },
        (name) => [name],
      ),
      reopenPhases: string[] = [];
    ...
    expect(reopenPhases).not.toContain('download');
    expect(reopenPhases).not.toContain('extract');
    expect(download).not.toHaveBeenCalled();
    expect(makeWorker).not.toHaveBeenCalled();
  ...
  it('reuses locally imported installer extraction on the next automatic launch', async () => {
    await new FixtureAssetManager().importFiles([
      new File(['fixture'], 'Red-Alert-2-Multiplayer.exe'),
    ]);
    const reopened = new FixtureAssetManager();
    ...
    expect(download).not.toHaveBeenCalled();
    expect(makeWorker).toHaveBeenCalledTimes(1);
    expect(pixels(reopened)).toEqual([252, 124, 28, 255]);
  });
  ...
  it('fetches the public URL directly while preserving one cache for public and historical proxy source aliases', async () => {
    ...
    const manager = new FixtureAssetManager();
    await manager.download({ url: ORIGINAL_ASSET_URL });
    expect(download.mock.calls[0][0]).toBe(ORIGINAL_ASSET_URL);
    expect(manager.source).toBe(ORIGINAL_ASSET_URL);

    for (const source of [
      '/asset-source',
      'http://omarky:5173/asset-source',
      LEGACY_ASSET_URL,
      ORIGINAL_ASSET_URL,
    ]) {
      expect(assetSourceUrl(source)).toBe(ORIGINAL_ASSET_URL);
      expect(await new FixtureAssetManager().initialize({ url: source })).toBe('ready');
    }

    expect(await entry('/asset-source')).toBeDefined();
    expect(await entry(ORIGINAL_ASSET_URL)).toBeUndefined();
    expect(download).toHaveBeenCalledTimes(1);
    expect(makeWorker).toHaveBeenCalledTimes(1);
    expect(assetSourceUrl('https://other.example/asset-source')).toBe(
      'https://other.example/asset-source',
    );
    expect(assetCacheKey('http://omarky:5173/asset-source?custom=1')).toBe(
      'http://omarky:5173/asset-source?custom=1',
    );
  });

  it.each([LEGACY_ASSET_URL, ORIGINAL_ASSET_URL])(
    'can read public-URL cache alias %s even if the proxy entry is invalid',
    async (alias) => {
      await new FixtureAssetManager().download();
      const saved = (await entry())!;
      ...
      const manager = new FixtureAssetManager();
      expect(await manager.initialize()).toBe('ready');
      expect(manager.cacheWarning).toBeNull();
      expect(download).toHaveBeenCalledTimes(1);
      expect(makeWorker).toHaveBeenCalledTimes(1);
  ...
  it('clears every built-in selected cache alias while preserving unrelated sources', async () => {
    ...
    await manager.download();
    const saved = (await entry())!;
    await entry(LEGACY_ASSET_URL, saved);
    await entry(ORIGINAL_ASSET_URL, saved);
    await entry('/own-copy.zip', saved);
    await manager.clearCache();

    for (const alias of ['/asset-source', LEGACY_ASSET_URL, ORIGINAL_ASSET_URL])
      expect(await entry(alias)).toBeUndefined();
  ...
  it('preserves complete original artwork across compatible cache versions without fetching again', async () => {
    ...
    for (const version of [5, 6, 7]) {
      ...
      expect(manager.getTerrain('rock')).not.toBeNull();
      expect(manager.getTerrain('sand')).not.toBeNull();
  ...
  it('coalesces rapid explicit submissions and does not cancel/restart the first download', async () => {
    ...
    const manager = new FixtureAssetManager(),
      ...
      duplicate = manager.download({ url: '/asset-source' });

    await vi.waitFor(() => expect(download).toHaveBeenCalledTimes(1));
    const signal = download.mock.calls[0][1].signal;
  ...
  it('allows explicit cancellation before any fetch without starting background retries', async () => {
    ...
    await manager.download({
      onProgress: (progress) => {
        if (progress.phase === 'download') manager.cancel();
      },
    });
    expect(download).not.toHaveBeenCalled();
    expect(makeWorker).not.toHaveBeenCalled();
  ...
  it('ignores cancelled worker events during a replacement import', async () => {
    ...
    const replacement = manager.importFiles([new File(['second'], 'second.exe')], {
      url: '/second.exe',
    });

    await vi.waitFor(() => expect(workers).toHaveLength(2));
    workers[0].onmessage?.({
      data: { kind: 'error', message: 'Late failure from cancelled extraction' },
    });
    ...
    expect(await entry('/first.exe')).toBeUndefined();
    expect(await entry('/second.exe')).toBeDefined();
    expect(download).not.toHaveBeenCalled();
  });

  it('keeps configured source caches independent and preserves a source when another is cleared', async () => {
    const a = new FixtureAssetManager(),
      b = new FixtureAssetManager();

    await a.download({ url: '/source-a.exe' });
    await b.download({ url: '/source-b.exe' });
    expect(download.mock.calls.map((call) => call[0])).toEqual(['/source-a.exe', '/source-b.exe']);
    const savedA = await entry('/source-a.exe');
    await a.initialize({ url: '/source-a.exe' });
    await b.clearCache();
    expect(download).toHaveBeenCalledTimes(2);
    expect(makeWorker).toHaveBeenCalledTimes(2);
    expectEntry(await entry('/source-a.exe'), savedA);
    expect(await entry('/source-b.exe')).toBeUndefined();
  });
  ...
  it('keeps a corrupt cache at the source gate until explicitly replaced, then reuses the repair', async () => {
    await new FixtureAssetManager().download();
    const saved = (await entry())!;
    await entry(assetCacheKey(DEFAULT_ASSET_URL), {
      ...saved,
      files: [{ name: 'unittem.pal', bytes: new Uint8Array(3) }],
    });

    for (const stage of ['download', 'installer', 'mix-pending', 'mix'] as const)
      await assetCache(archiveStageKey('/asset-source', stage), null);
    const recovered = new FixtureAssetManager();
    expect(await recovered.initialize()).toBe('invalid');
    expect(await recovered.initialize()).toBe('invalid');
    expect(download).toHaveBeenCalledTimes(1);
    expect(makeWorker).toHaveBeenCalledTimes(1);
  ...
  it('surfaces a genuine download failure after a corrupt cache instead of reporting ready', async () => {
    ...
    const saved = (await entry())!,
      corrupt = saved.files.filter((file) => file.name !== 'gi.shp');
    ...
    expect(reopened.status.phase).toBe('error');
    expect(reopened.error).toMatch(/HTTP 503/);
    expect(makeWorker).toHaveBeenCalledTimes(1);
  ...
  it('surfaces invalid extracted artwork and keeps the previously valid cache intact', async () => {
    ...
    const saved = await entry();
    extracted = files().map((file) =>
      file.name === 'giicon.shp' ? { ...file, bytes: new Uint8Array(3) } : file,
    );
  ...
  it('keeps artwork usable when IndexedDB cannot open and reports that it cannot save', async () => {
    ...
    expect(manager.status.message).toMatch(/cache unavailable/);
    expect(pixels(manager)).toEqual([252, 124, 28, 255]);
  });
  ...
  it('clears a transient read warning if the replacement saves successfully', async () => {
    ...
    expect(manager.status.message).toMatch(/saved in this browser/);
    expect(await entry()).toBeDefined();
  });
  ...
  it('closes the database on a synchronous quota error and clears warnings after a later successful import', async () => {
    ...
    expect(manager.ready).toBe(true);
    expect(manager.cacheWarning).toMatch(/could not save.*Storage full/);
    expect(await entry()).toBeUndefined();
    put.mockRestore();
    await manager.importFiles([new File(['fixture'], 'installer.exe')]);
    expect(manager.cacheWarning).toBeNull();
    expect(manager.status.message).toMatch(/saved in this browser/);
    await new FixtureAssetManager().initialize();
    expect(download).toHaveBeenCalledTimes(1);
  ...
  it('waits for transaction commit: an aborted successful put is not reported as saved', async () => {
    ...
    await manager.download();
    expect(manager.ready).toBe(true);
    expect(manager.cacheWarning).toMatch(/could not save.*aborted/);
    expect(manager.status.message).toMatch(/cache unavailable/);
    expect(await entry()).toBeUndefined();
  });

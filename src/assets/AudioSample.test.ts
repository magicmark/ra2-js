...
const entry = (overrides: Partial<AudioIndexEntry> = {}): AudioIndexEntry => ({ name: 'sample', offset: 0, size: 8, sampleRate: 22050, flags: 12, chunkSize: 8, ...overrides });
const intSamples = (wave: Uint8Array) => decodeWave(wave).channels.map(channel => Array.from(channel, value => Math.round(value * 32768)));
function idx(names = ['SAMPLE']): Uint8Array {
  const bytes = new Uint8Array(12 + names.length * 36), data = new DataView(bytes.buffer);
  bytes.set(new TextEncoder().encode('GABA')); data.setUint32(4, 2, true); data.setUint32(8, names.length, true);
  for (const [i, name] of names.entries()) {
    const at = 12 + 36 * i; bytes.set(new TextEncoder().encode(name), at);
    bytes.fill(0xef, at + name.length + 1, at + 16);
    [5, 8, 22050, 12, 512].forEach((value, field) => data.setUint32(at + 16 + field * 4, value, true));
  }
  return bytes;
}
function wave(format: number[], body: number[], extra: [string, number[]][] = []): Uint8Array {
  ...
  const bytes = new Uint8Array(12 + chunks.reduce((size, [, data]) => size + 8 + data.length + data.length % 2, 0)), data = new DataView(bytes.buffer);
  bytes.set(new TextEncoder().encode('RIFF')); data.setUint32(4, bytes.length - 8, true); bytes.set(new TextEncoder().encode('WAVE'), 8);
  let at = 12;
  for (const [name, body] of chunks) { bytes.set(new TextEncoder().encode(name), at); data.setUint32(at + 4, body.length, true); bytes.set(body, at + 8); at += 8 + body.length + body.length % 2; }
  return bytes;
}

it('reads native index fields, normalizes case and ignores garbage after the name terminator', () => {
  const bytes = idx();
  expect(readAudioIndex(bytes)).toEqual([{ name: 'sample', offset: 5, size: 8, sampleRate: 22050, flags: 12, chunkSize: 512 }]);
  // DataView must honor offsets when called with archive subarrays.
  const padded = new Uint8Array(bytes.length + 6); padded.set(bytes, 3);
...
it('rejects corrupt index signatures, counts, names and duplicate entries', () => {
  ...
  const signature = idx(); signature[0] = 0; expect(() => readAudioIndex(signature)).toThrow(/signature/);
  const version = idx(); version[4] = 3; expect(() => readAudioIndex(version)).toThrow(/version/);
...
it('wraps native PCM16 samples and deinterleaves stereo without changing amplitude', () => {
  ...
  expect(intSamples(wav)).toEqual([[-32768, 0], [32767, 16384]]);
...
it('decodes unsigned PCM8 and skips padded unknown RIFF chunks', () => {
  const wav = wave([1, 0, 1, 0, 0x22, 0x56, 0, 0, 0x22, 0x56, 0, 0, 1, 0, 8, 0], [0, 128, 255], [['JUNK', [9]], ['LIST', [1, 2, 3]]]);
...
it('keeps stereo IMA channel state independent across four-byte channel groups', () => {
  const wav = audioBagWave(entry({ size: 16, chunkSize: 16, flags: 13 }), Uint8Array.from([0, 0, 0, 0, 0, 0, 0, 0, 0x10, 0x32, 0x54, 0x76, 0x98, 0xba, 0xdc, 0xfe]));
  expect(intSamples(wav)).toEqual([[0, 0, 1, 4, 8, 15, 27, 47, 88], [0, 0, -1, -4, -8, -15, -27, -47, -88]]);
});

it('resets IMA state for each block and preserves a trailing partial mono block', () => {
  const wav = audioBagWave(entry({ size: 14 }), Uint8Array.from([0, 0, 0, 0, 0x10, 0x32, 0x54, 0x76, 100, 0, 0, 0, 0, 0]));
...
it('preserves valid trailing partial stereo blocks and honors fact padding trims', () => {
  const bag = new Uint8Array(40); bag.set([10, 0, 0, 0, 20, 0, 0, 0], 24);
  const wav = audioBagWave(entry({ flags: 13, size: 40, chunkSize: 24 }), bag);
  expect(intSamples(wav)).toEqual([Array(17).fill(0).concat(Array(9).fill(10)), Array(17).fill(0).concat(Array(9).fill(20))]);
  new DataView(wav.buffer).setUint32(48, 20, true);
  expect(intSamples(wav)).toEqual([Array(17).fill(0).concat(Array(3).fill(10)), Array(17).fill(0).concat(Array(3).fill(20))]);
});
...
it('rejects outside BAG slices, unsupported flags, rates, and malformed sample alignment', () => {
  ...
  for (const fields of [{ offset: -1 }, { offset: 30 }, { size: 0 }, { sampleRate: 0 }, { flags: 4 }, { flags: 14 }, { flags: 0x100c }, { flags: 6 }, { flags: 6, chunkSize: 0, size: 7 }, { chunkSize: 0 }, { chunkSize: 7 }, { size: 9 }, { flags: 13, chunkSize: 16, size: 12 }])
...
it('rejects truncated WAV chunks, unsupported formats, and invalid IMA state', () => {
  ...
  const mutate = (at: number, value: number) => { const wav = original.slice(); wav[at] = value; return wav; };
  ...
  const truncated = original.slice(); new DataView(truncated.buffer).setUint32(56, 9, true);
...
describe.skipIf(!process.env.RA2_ASSET_DIR)('original RA2 audio archive', () => {
  it('decodes every original sample and matches independent PCM goldens for all four native formats', () => {
    const language = new MixArchive(readFileSync(join(process.env.RA2_ASSET_DIR!, 'language.mix'))), audio = new MixArchive(language.get('audio.mix')!);
    const entries = readAudioIndex(audio.get('audio.idx')!), bag = audio.get('audio.bag')!, hashes: Record<string, string> = {
    ...
    for (const entry of entries) {
      ...
      for (let i = 0; i < sample.channels[0].length; i++) for (let c = 0; c < sample.channels.length; c++)
        pcm.writeInt16LE(Math.round(sample.channels[c][i] * 32768), (i * sample.channels.length + c) * 2);
      expect(createHash('sha256').update(pcm).digest('hex'), entry.name).toBe(hashes[entry.name]); verified.push(entry.name);
    }

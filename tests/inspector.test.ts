...
function setup() {
  ...
  game.state.entities = game.state.entities.filter(
    (e) => e.type === 'conyard' || e === george || e === target || e === enemy,
  );
...
describe('George noncombat inspector', () => {
  it('pauses for three seconds, leaves without upgrading, and resumes wandering', () => {
    ...
    expect(inspectionStatus(george, game.state, game.defs)).toContain(
      `Inspecting ${game.defs[target.type].name}`,
    );
    advance(game, 2.8);
    expect(george.x).toBe(stopped.x);
    expect(george.y).toBe(stopped.y);
    expect(target.inspectionProgress).toBeCloseTo(2.9 / 3);
    advance(game, 0.1);
    expect(george.inspection).toBeUndefined();
    expect(target.inspectedBy).toBeUndefined();
    expect(george.order).toBe('move');
    expect(george.path.length).toBeGreaterThan(0);
    advance(game, 2);
    expect(Math.hypot(george.x - stopped.x, george.y - stopped.y)).toBeGreaterThan(0.5);
    expect(george.inspection).toBeUndefined();
    advance(game, 30);
    expect(target.rank ?? 0).toBe(0);
    expect(target.promotedAt).toBeUndefined();
    expect(target.hp).toBe(60);
    expect(target.maxHp).toBe(game.defs.gi.hp);
    expect(rankMultiplier(target)).toBe(1);
    expect(game.state.events.filter((e) => e.text.includes('promoted'))).toEqual([]);
  });

  for (const reason of ['range', 'ownership', 'death', 'transport'] as const)
    it(`ends an inspection on ${reason} loss and walks away`, () => {
      const { game, george, target } = setup();
      advance(game, 1);

      if (reason === 'range') target.x = george.x + INSPECTION_RULES.radius + 0.1;
  ...
  it('excludes enemies, neutral units, buildings, embarked units and other Georges; Elite units remain inspectable', () => {
    ...
    target.rank = 2;
    advance(game, 0.1);
    expect(george.inspection?.targetId).toBe(target.id);
  ...
  it('rejects selection and all player orders while preserving autonomous movement', () => {
    ...
    game.select([target.id], true);
    expect(george.selected).toBe(false);
    const before = structuredClone(george);
    game.orderMove([george.id], 1, 1);
    game.orderMove([george.id], 1, 1, true);
    game.orderForceMove([george.id], 1, 1);
    game.orderAttack([george.id], enemy.id, true);
    game.orderForceFire([george.id], 1, 1);
    game.orderWaypoints([george.id], [{ x: 1, y: 1 }]);
    game.orderGuard([george.id], 1, 1, enemy.id);
    game.stop([george.id]);
    game.guard([george.id]);
    game.scatter([george.id]);
    game.deploy([george.id]);
    expect(george).toEqual(before);
    advance(game, 2);
    expect(Math.hypot(george.x - before.x, george.y - before.y)).toBeGreaterThan(0.5);
  });
  ...
  it('never attacks or produces firing effects under attack, force-fire, retaliation or attack-move commands', () => {
    ...
    for (let frame = 0; frame < 60; frame++) {
      game.tick(1 / 30);
      fired ||= game.state.effects.some(
        (e) => e.side === 0 && ['shot', 'impact', 'explosion'].includes(e.kind),
      );
    ...
    game.orderMove([george.id], george.x - 2, george.y, true);
    advance(game, 1);
  ...
  it('preserves independent veteran damage and durability bonuses', () => {
    ...
    game.tick(1 / 30);
    game.orderAttack([target.id], enemy.id);

...
describe('original building sale lifecycle', () => {
  it('refunds once, disables power/actions, and holds the footprint through reverse buildup', () => {
    const game = new Game({ ai: false }),
      power = game.state.entities.find((e) => e.side === 0 && e.type === 'power')!;
    ...
    expect(power.selling?.duration).toBe(BUILDING_SALE_SECONDS);
    expect(game.state.sides[0].money).toBe(before + game.defs.power.cost / 2);
    expect(game.state.sides[0].power).toBe(0);
    expect(game.sell(power.id)).toBe(false);
    expect(game.repair(power.id)).toBe(false);
    game.select([power.id]);
    game.orderMove([power.id], 4, 4);
    expect(power.selected).toBe(false);
    expect(power.rally).toBeUndefined();
    expect(game['isPassable'](power.x, power.y)).toBe(false);
    ...
    expect(game['isPassable'](power.x, power.y)).toBe(true);
    expect(game.state.sides[0].money).toBe(before + game.defs.power.cost / 2);
  ...
  it('holds production immediately and cannot capture or attack a selling structure', () => {
    const game = new Game({ ai: false }),
      barracks = game.state.entities.find((e) => e.side === 0 && e.type === 'barracks')!;

    game.build('gi');
    game.sell(barracks.id);
    expect(game.canBuild('gi').ok).toBe(false);
    advance(game, 0.5);
    expect(game.state.sides[0].queues.infantry[0].progress).toBe(0);
    const engineer = game['spawn']('engineer', 0, barracks.x + 1, barracks.y - 1);
  ...
  it('pauses and resumes sale on the simulation clock', () => {
    ...
    game.setGameSpeed(2);
    advance(game, BUILDING_SALE_SECONDS / 2);
    expect(game.state.entities).not.toContain(power);
  });
  it('walks every authored buildup frame in reverse order', () => {
    expect(Array.from({ length: 25 }, (_, tick) => buildingSaleFrame(tick / 25, 25))).toEqual(
      Array.from({ length: 25 }, (_, i) => 24 - i),
    );

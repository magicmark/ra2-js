...
function advance(game: Game, seconds: number) { for (let n = 0; n < Math.ceil(seconds * 30); n++) game.tick(1 / 30); }
function setup(native = false) {
  const map = native ? parseNativeMap(readFileSync('public/maps/frostline-basin.map', 'utf8')) : undefined;
...
function placeReady(game: Game, type: string) {
  for (let y = 0; y < game.state.height; y++) for (let x = 0; x < game.state.width; x++) {
...
describe('The Butcher’s and George', () => {
  it('has explicit Allied technology and noncombat inspector rules', () => {
    const { defs } = setup();
    expect(defs.butchers).toMatchObject({ name: "The Butcher's", category: 'structures', faction: 'allied', cost: 1000, hp: 650, armor: 'steel', power: -30, footprint: [3, 3], requires: ['barracks', 'refinery'] });
    expect(defs.butchers.producer).toBeUndefined();
    expect(defs.george).toMatchObject({ name: 'George', category: 'infantry', faction: 'allied', cost: 650, hp: 225, armor: 'flak', damage: 0, range: 0, requires: ['barracks', 'butchers'] });
  ...
  for (const native of [false, true]) it(`builds, places, unlocks and trains through the real ${native ? 'native' : 'procedural'} map queues`, () => {
    const game = setup(native), side = game.state.sides[0];
  ...
  it('rejects Soviet production even with captured Allied prerequisites, and ignores enemy/dead prerequisites', () => {
    const game = setup(), shop = buildShop(game);
    for (const id of ['butchers', 'george']) expect(game.canBuild(id, 1)).toEqual({ ok: false, reason: 'Unavailable to this faction' });
    shop.side = 1;
    ...
    shop.side = 0; shop.hp = 0;
    const barracks = game.state.entities.find(e => e.type === 'barracks')!; barracks.side = 0;
    expect(game.canBuild('george').reason).toBe("Requires The Butcher's");
  });

  for (const method of ['destroy', 'sell'] as const) it(`holds paid George progress when the shop is ${method === 'destroy' ? 'destroyed' : 'sold'}, then resumes after rebuilding`, () => {
    const game = setup(), shop = buildShop(game), side = game.state.sides[0];
    game.build('george'); advance(game, 3);
    const item = side.queues.infantry[0]; expect(item.progress).toBeGreaterThan(0);
    if (method === 'destroy') shop.hp = 0; else expect(game.sell(shop.id)).toBe(true);
    game.tick(1 / 30);
    const progress = item.progress, spent = item.spent, money = side.money;
    expect(item.blockedPrerequisite).toBe("Requires The Butcher's");
    expect(game.canBuild('george').ok).toBe(false);
    advance(game, 15);
    expect(item.progress).toBe(progress); expect(item.spent).toBe(spent); expect(side.money).toBe(money);
    expect(game.state.entities.some(e => e.type === 'george')).toBe(false);
    buildShop(game); advance(game, game.defs.george.buildTime);
  ...
  it('holds shop construction when Refinery is lost and can refund its actual spending', () => {
    const game = setup(), side = game.state.sides[0]; game.build('butchers'); advance(game, 4);
    const item = side.queues.structures[0], paid = item.spent;
    game.state.entities.find(e => e.type === 'refinery')!.hp = 0;
    const cash = side.money; advance(game, 25);
    ...
    expect(item.spent).toBe(paid); expect(item.ready).toBe(false);
    game.cancelBuild('structures'); expect(side.money).toBeCloseTo(cash + paid);
  });

  it('moves without attacking after original animation metadata is installed', () => {
    const game = setup(), george = game.state.entities.find(e => e.type === 'gi')!, target = game.state.entities.find(e => e.type === 'conscript')!;
    game.state.entities = game.state.entities.filter(e => e.type === 'conyard' || e === george || e === target);

...
function arena(targetX: number, navigable = false, targetType = 'rhino') {
  ...
  for (let y = 29; y <= 31; y++) for (let x = 8; x <= (navigable ? 45 : 8); x++)
  ...
  const planes = () => game.state.entities.filter(e => e.type === 'hornet' && e.homeId === carrier.id);
  const advance = (seconds: number) => { for (let i = 0; i < Math.ceil(seconds * 30); i++) game.tick(1 / 30); };
...
it('moves into carrier range before launching, then its aircraft damage the target', () => {
  ...
  expect(Math.hypot(target.x - carrier.x, target.y - carrier.y)).toBeLessThanOrEqual(game.defs.carrier.range);
  advance(15);
  expect(target.hp).toBeLessThan(target.maxHp);
});

it.each(['rhino', 'power_soviet'])('launches at a %s exactly at the carrier range boundary', targetType => {

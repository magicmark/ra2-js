...
it('retains explored radar terrain at full brightness without exposing unseen enemies', () => {
  ...
  const context = {
    ...
    fillStyle: '',
    fillRect() {
      paints.push({ alpha: this.globalAlpha, color: this.fillStyle });
    },
  };
  ...
  const state = {
    ...
    fog: new Uint8Array([0, 1]),
    explored: new Uint8Array([1, 1]),
    entities: [
      { type: 'gi', side: 1, hp: 100, x: 0, y: 0, revealed: true },
      { type: 'gi', side: 1, hp: 100, x: 1, y: 0 },
    ],
  ...
  Object.assign(ui, {
    game: { state, defs: { gi: { footprint: [1, 1] } } },
    actions: {},
    el: vi.fn(() => canvas),
  });
  ui.drawMinimap();
  expect(paints.filter((paint) => paint.color === '#747b37')).toEqual([
    { alpha: 1, color: '#747b37' },
  ]);

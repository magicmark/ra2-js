...
function inspect(name:string,cells:any[],mines:readonly any[],cx:number,cy:number,local=(c:any)=>true,structures:any[]=[]) {
  const idx = new Map(cells.map(c=>[key(c.x,c.y),c]));
  const roads = cells.filter(c=>road(c)&&local(c));
  const ranges = (axis:'x'|'y',fixed:number) => {
    const axisCells=roads.filter(c=>axis==='x'?c.y===fixed:c.x===fixed).map(c=>c[axis]).sort((a,b)=>a-b);
    const runs:number[][]=[];
    for(const n of axisCells) { const last=runs.at(-1); if(last&&n===last[1]+1)last[1]=n; else runs.push([n,n]); }
...
function visit(bytes:Uint8Array,name:string) {
  ...
  for(const child of ['local.mix','cache.mix','generic.mix','conquer.mix','isogen.mix','isotemp.mix','isosnow.mix','isourb.mix','temperat.mix','snow.mix','urban.mix','neutral.mix']) {const b=a.get(child);if(b)visit(b,child);}
}
visit(readFileSync(`${process.env.RA2_ASSET_DIR}/ra2.mix`),'ra2.mix');
const get=(name:string)=>{const b=archives.map(a=>a.get(name)).find(Boolean);if(!b)throw Error(`Missing ${name}`);return b;};
const ini=(name:string)=>{const sections:Record<string,any>={};let current:any={};for(const raw of new TextDecoder().decode(get(name)).split(/\r?\n/)){const line=raw.replace(/;.*/,'').trim();const h=/^\[(.+)\]$/.exec(line);if(h)current=sections[h[1].toLowerCase()]??={};else{const eq=line.indexOf('=');if(eq>=0)current[line.slice(0,eq).trim().toLowerCase()]=line.slice(eq+1).trim();}}return sections;};

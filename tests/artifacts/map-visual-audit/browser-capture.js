...
export async function install(options = {}) {
  ...
  const makeRenderer = source => {
    ...
    const r = new Renderer(c, source); r.assets = assets;
 ...
 return r;
  ...
  input.onchange = async () => {
    ...
    const visit = a => { archives.push(a);

 for (const name of ['local.mix','cache.mix','generic.mix','conquer.mix','isogen.mix','isotemp.mix','isosnow.mix','isourb.mix','temperat.mix','snow.mix','urban.mix','neutral.mix']) { const bytes = a.get(name);
 ...
 if (bytes) visit(new MixArchive(bytes,name)); } };
  ...
  return {
    ...
    ends(extension = 'tem') {
      ...
      const get = name => { const bytes = archives.map(a=>a.get(name)).find(Boolean);

 if (!bytes) throw Error(name);
 ...
 if(!v.getUint32(16+sub*4,true))continue;

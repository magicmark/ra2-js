...
function rgbaPng(path: string) {
  ...
  const data = inflateSync(Buffer.concat(chunks)),
    stride = width * 4,
    pixels = new Uint8ClampedArray(stride * height);
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < stride; x++) {
      const a = x >= 4 ? pixels[y * stride + x - 4] : 0,
        ...
        c = y && x >= 4 ? pixels[(y - 1) * stride + x - 4] : 0;
...
function buildingSprite(building: ReturnType<typeof rgbaPng>, name = 'butchers') {
  ...
  const image = (decoded: ReturnType<typeof rgbaPng>) => {
    // SAFETY: this metadata fixture is consumed only by CustomArt dimensions and the pixel-map-backed drawImage below.
    const value = {
      naturalWidth: decoded.width,
      naturalHeight: decoded.height,
    } as HTMLImageElement;

    imagePixels.set(value, decoded.pixels);

    return value;
  };

  const buildingImage = image(building),
    infantryImage = image(rgbaPng(`public${CUSTOM_ART_URLS.infantry}`));
    ...
    createElement: () => {
      ...
      const context = {
        drawImage: (source: CanvasImageSource) => {
          drawnPixels = imagePixels.get(source)!;
        },
  ...
  try {
    return new CustomArt({
      building: buildingImage,
      infantry: infantryImage,
      buildingCameo: buildingImage,
      infantryCameo: buildingImage,
    }).getSprite(name)!;
  } finally {
    vi.unstubAllGlobals();
  }
...
describe('bundled imagegen artwork', () => {
  it('ships four real PNGs and preserves actual alpha around the painted building foundation', () => {
    for (const url of Object.values(CUSTOM_ART_URLS))
      expect(readFileSync(`public${url}`).toString('ascii', 1, 4)).toBe('PNG');

    const { width, height, pixels } = rgbaPng(`public${CUSTOM_ART_URLS.building}`),
      b = alphaBounds(pixels, width, height);
    ...
    expect(paintedHeight).toBeGreaterThan(115);
    expect(paintedHeight).toBeLessThan(122);
    ...
    const edges = [
      ...
      const points = Array.from(
        { length: Math.floor((end - start) * (b.right - b.left)) },
        (_, i) => {
          const x = Math.round(b.left + start * (b.right - b.left)) + i;

          return { x, y: bottomAt(x) };
        },
      );
      ...
      const slope =
        points.reduce((sum, p) => sum + (p.x - mx) * (p.y - my), 0) /
        points.reduce((sum, p) => sum + (p.x - mx) ** 2, 0);

      const intercept = my - slope * mx;
      expect(Math.abs(slope * BUTCHERS_PROJECTION_Y)).toBeCloseTo(0.5, 2);
      expect(
        Math.sqrt(
    ...
    expect(
      (Math.abs(corner - BUTCHERS_PROJECTION.frontCorner.x) * 180) /
        BUTCHERS_PROJECTION.paintedWidth,
    ).toBeLessThan(0.1);
    expect(
      (Math.abs(corner - (b.left + b.right) / 2) * 180) / BUTCHERS_PROJECTION.paintedWidth,
    ).toBeLessThan(0.5);

    const rendered = buildingSprite({ width, height, pixels }),
      camera = new Camera();

    const sourceFrontY = edges[0].slope * corner + edges[0].intercept;

    const actualFrontOffset =
      ((sourceFrontY * 180) / BUTCHERS_PROJECTION.paintedWidth) * BUTCHERS_PROJECTION_Y -
      rendered.anchorY;

    const engineFrontOffset = camera.project(3, 3).y - camera.project(1.5, 1.5).y;
    expect(engineFrontOffset).toBe(45);
    expect(Math.abs(actualFrontOffset - engineFrontOffset)).toBeLessThan(0.1);
  });
  ...
  it('decodes all32 directional frames, separates the east muzzle, and keeps north flash and boot registration', () => {
    ...
    expect([width, height]).toEqual([1774, 887]);
    expect(pixels[3]).toBe(0);

    for (let row = 0; row < 4; row++)
      for (let column = 0; column < 8; column++) {
        const { grid, crop } = georgeFrameRegion(width, height, row, column),
          b = alphaBounds(pixels, width, height, crop, 128);

        expect(b.bottom - b.top).toBeGreaterThan(150);
        expect(b.right - b.left).toBeGreaterThan(75);

        const factor = 34 / (grid.right - grid.left),
          anchorY = (b.bottom - grid.top) * factor;

        expect(anchorY).toBeGreaterThan(28);
        expect(anchorY).toBeLessThan(35);

        if (row === 0) {
          const paintedHeight = (b.bottom - b.top) * factor;
          expect(paintedHeight).toBeGreaterThanOrEqual(26);
          expect(paintedHeight).toBeLessThanOrEqual(31);
        }
      }

    const rendered = buildingSprite(rgbaPng(`public${CUSTOM_ART_URLS.building}`), 'george');
    expect(rendered.width).toBe(36);
    expect(rendered.height).toBe(36);
    expect(rendered.anchorX).toBe(18);
    expect(rendered.anchorY).toBeGreaterThan(28);
    expect(rendered.anchorY).toBeLessThan(35);

    const east = georgeFrameRegion(width, height, 3, 6),
      northeast = georgeFrameRegion(width, height, 3, 7);

    expect(east.crop.right).toBe(1556);
    expect(northeast.crop.left).toBe(1556);
    expect(alphaBounds(pixels, width, height, east.crop).right).toBeGreaterThan(east.grid.right);
    expect(alphaBounds(pixels, width, height, northeast.crop).left).toBeGreaterThan(
      northeast.crop.left,
    );
  ...
  it('keeps eight facings and inserts a passing pose between walking contacts over a 0.6-second cycle', () => {
    for (let facing = 0; facing < 8; facing++) {
      ...
      expect(georgeAtlasFrame('Walk', facing, 0.15 - 1e-6).row).toBe(1);
      expect(georgeAtlasFrame('Walk', facing, 0.3 - 1e-6).row).toBe(0);

...
describe('temporary automatic Soviet wave setting', () => {
  for (const native of [false, true])
    it(`keeps the ${native ? 'native' : 'training'} map economy active without scheduled attacks`, () => {
      ...
      const playerBuildings = game.state.entities.filter(
        (e) => e.side === 0 && isBuilding(game.defs[e.type]),
      );
      ...
      expect(
        game.state.entities.filter((e) => e.side === 1 && e.type === 'rhino').length,
      ).toBeGreaterThanOrEqual(2);

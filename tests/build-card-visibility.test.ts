...
const spawn = (game: Game, type: string) => game['spawn'](type, 0, 25, 35);
...
function cards(game: Game, types: string[]) {
  const elements = new Map(types.map((type) => [type, { hidden: false }])),
    count = { textContent: '' };
  ...
  const visible = () => [...elements].flatMap(([type, card]) => (card.hidden ? [] : [type]));
  update();
...
describe('building cards follow prerequisite visibility', () => {
  it('reveals buildings only when prerequisite construction finishes and updates the count', () => {
    ...
    expect(ui.visible()).toEqual(['power']);
    expect(ui.count.textContent).toBe('01 AVAILABLE');
    ...
    ui.update();
    expect(ui.visible()).toEqual(['power', 'refinery', 'barracks']);
    expect(ui.count.textContent).toBe('03 AVAILABLE');
    spawn(game, 'refinery');
    ui.update();
    expect(ui.visible()).not.toContain('warfactory');
    spawn(game, 'barracks');
    ui.update();
    expect(ui.visible()).toContain('warfactory');
  });

  for (const loss of ['destroyed', 'selling', 'captured'] as const)
    it(`hides unqueued technology when its last prerequisite is ${loss}`, () => {
      const game = new Game({ ai: false }),
        ui = cards(game, ['warfactory', 'radar', 'shipyard']);
      ...
      expect(ui.count.textContent).toBe('00 AVAILABLE');
      const replacement = spawn(game, 'refinery');
      ui.update();
      expect(ui.visible()).toHaveLength(3);
      spawn(game, 'refinery');
      replacement.hp = 0;
  ...
  it('requires a completed construction producer even for buildings without explicit prerequisites', () => {
    const game = new Game({ ai: false }),
      ui = cards(game, ['power', 'pillbox']);

    expect(ui.visible()).toEqual(['power', 'pillbox']);
    game.state.entities.find((e) => e.side === 0 && e.type === 'conyard')!.selling = {
      startedAt: 0,
      duration: 2,
    };
    ui.update();
    expect(ui.visible()).toEqual([]);
    const yard = spawn(game, 'conyard');
    yard.constructing = { startedAt: 0, duration: 2 };
    ui.update();
    expect(ui.visible()).toEqual([]);
    delete yard.constructing;
    ui.update();
    expect(ui.visible()).toEqual(['power', 'pillbox']);
  });
  ...
  it('keeps non-head queued buildings visible after prerequisite loss', () => {
    const game = new Game({ ai: false }),
      ui = cards(game, ['power', 'butchers']);

    expect(game.build('power')).toBe(true);
    expect(game.build('butchers')).toBe(true);
    game.state.entities.find((e) => e.side === 0 && e.type === 'refinery')!.hp = 0;
    ui.update();
    expect(ui.visible()).toEqual(['power', 'butchers']);
    const item = game.state.sides[0].queues.structures[1];
    game.cancelBuild('structures', 0, item.id);
    ui.update();
    expect(ui.visible()).toEqual(['power']);
  });
  ...
  it('preserves ready unique defenses after their prerequisite and producer are lost', () => {
    ...
    ui.update();
    expect(ui.visible()).toEqual(['chronosphere']);
    game.cancelBuild('defenses');
    ui.update();
    expect(ui.visible()).toEqual([]);
  });

...
function arena(targetType: string, rank: Entity['rank'] = 0) {
  ...
  Object.assign(target, { type: targetType, x: 25.5, y: 30.5, hp: game.defs[targetType].hp, maxHp: game.defs[targetType].hp, rank, cooldown: 999 });
...
it.each(['gi', 'conscript', 'engineer', 'attack_dog', 'spy', 'tanya', 'sniper', 'chrono_legionnaire'])(
...
it('uses finite infantry damage: the tougher custom inspector survives and nearby infantry are unharmed', () => {
  ...
  const bystander = { ...target, id: 9000, type: 'conscript', hp: 125, maxHp: 125, y: target.y + 1, path: [] };
...
it.each([
  ['ifv', 100], ['miner', 100], ['rhino', 100],
  ['power_soviet', 30], ['barracks_soviet', 20], ['wall', 20],

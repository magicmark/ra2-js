...
import { openEntry } from '../src/entryRoute';

const harness = { viewer: vi.fn(async () => {}), game: vi.fn(async () => {}) };
...
async function open(path: string) {
  ...
  await openEntry(location, history, harness);
  await vi.waitFor(() =>
    expect(harness.viewer.mock.calls.length + harness.game.mock.calls.length).toBe(1),
  );
}
...
it.each([
  ...
  '/maps/',
  '/maps/emerald-divide',
  '/maps/frostline-basin/preview',
  '/maps/unknown',
])('opens %s in the isolated Map Viewer', async (path) => {
  await open(path);
...
it.each(['/admin', '/admin/', '/admin/emerald-divide', '/admin/frostline-basin/preview'])(
  'migrates bookmarked %s to /maps while preserving query and fragment',
  async (path) => {
...
it.each(['/', '/?map=emerald-divide', '/maps-extra', '/administrator'])(
  'keeps game startup separate at %s',
  async (path) => {
    await open(path);

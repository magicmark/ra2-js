...
const spawn = (game: Game, type: string, x: number, y: number) => game['spawn'](type, 0, x, y);

describe('bounded shipyard placement', () => {
  it('loads original shipyard adjacency and rejects malformed placement metadata', () => {
    expect(definitions.shipyard).toMatchObject({
      adjacent: 12,
      baseNormal: false,
      footprint: [4, 4],
      movement: 'water',
    ...
    for (const value of ['-1', '12.5', 'inf', '"12"'])
      expect(() => parseDefinitions(`${rule}adjacent=${value}`)).toThrow(
        'Invalid placement adjacency',
      );
  ...
  it.each([
    ...
    'accepts a 12-cell foundation gap at %i,%i, but rejects one cell farther',
    (x, y, farX, farY) => {
      const { game } = sea();
  ...
  it('does not extend ordinary land-building reach or chain reach through shipyards', () => {
    ...
    for (let y = 20; y < 28; y++)
      for (let x = 26; x < 42; x++) game.state.tiles[y * 64 + x].terrain = 'grass';
  ...
  it('keeps per-cell water, exploration, ore, occupancy and map bounds enforced', () => {
    ...
    const invalidCells = () =>
      placementCells(game.state, game.defs, 'shipyard', 34, 20).filter((cell) => !cell.valid);
  ...
  it('rejects native border/padding water even when explored and in reach', () => {
    ...
    game.state.entities = [];
    spawn(game, 'power', 55, 55);
    game.state.explored.fill(1);

    for (let dy = 0; dy < 4; dy++)
      for (let dx = 0; dx < 4; dx++) {
        const tile = game.state.tiles[(y + dy) * game.state.width + x + dx];
        tile.terrain = 'water';
        tile.ore = 0;
      }
  ...
  it('uses the longer reach for normal paid-ready placement and cannot consume invalid plots', () => {
    const { game, base } = sea();
    spawn(game, 'conyard', 16, 16);
    spawn(game, 'refinery', 16, 22);
    game['spawn']('conyard', 1, 52, 52); // Keep the staged match active.
    ...
    expect(item.spent).toBe(game.defs.shipyard.cost);
    expect(game.place('shipyard', 35, 20)).toBe(false);
    expect(queue[0]).toBe(item);
    expect(game.place('shipyard', 34, 20)).toBe(true);
    expect(queue).toHaveLength(0);
    const yard = game.state.entities.find((e) => e.type === 'shipyard')!;
    expect(yard.constructing).toBeDefined();
    expect([yard.x, yard.y]).toEqual([34, 20]);

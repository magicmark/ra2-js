...
const load = (id = MAP_CATALOG[0].id) =>
...
describe('native map codec robustness', () => {
  it('rejects truncated chunks, output overflows and invalid dictionary references', () => {
    ...
    expect(() => decodeLzo1x(Uint8Array.of(18, 42, 64, 255, 17, 0, 0), 4)).toThrow(
      /back-reference/,
    );
    ...
    expect(() => unpackSection(Buffer.from([10, 0, 1, 0, 17]).toString('base64'), 'lzo')).toThrow(
      /chunk size/,
    );
  });
  ...
  it('handles literal lengths, dictionary overlap and chunk-size boundaries', () => {
    for (const length of [1, 3, 4, 18, 19, 238, 239, 255, 8192]) {
      for (const bytes of [
        Uint8Array.from({ length }, (_, i) => (i * 103 + i * i) % 251),
        new Uint8Array(length).fill(73),
        Uint8Array.from({ length }, (_, i) => i % 3),
      ]) {
  ...
  for (const [seed, entry] of MAP_CATALOG.entries())
    it(`regenerates ${entry.name} deterministically`, () => {
      expect(writeNativeMap(authorMap(entry, seed), entry.description).replace(/\n/g, '\r\n')).toBe(
        readFileSync(`public/maps/${entry.id}.map`, 'utf8'),
      );
    });
});
...
describe('native maps in the actual simulation', () => {
  for (const entry of MAP_CATALOG)
    it(`${entry.name}: native terrain, resources, starts and neutral foundations drive game state`, () => {
      const map = load(entry.id),
        game = new Game({ map, ai: false }),
        state = game.state;

      expect(state.nativeMap).toBe(map);
      expect(state.width).toBe(192);
      expect(state.height).toBe(192);
      ...
      for (const [side, waypoint] of [
        [0, 0],
        [1, 3],
      ]) {
        const start = map.starts.find((s) => s.index === waypoint)!,
          yard = state.entities.find((e) => e.side === side && e.type === 'conyard')!;
      ...
      for (const e of state.entities.filter((e) => isBuilding(game.defs[e.type]))) {
        ...
        for (let y = e.y; y < e.y + height; y++)
          for (let x = e.x; x < e.x + width; x++) {
            ...
            expect(tile.ore).toBe(0);
          }
      }

      const oil = state.entities.find((e) => e.type === 'tech_oil')!;
      expect(game.canBuild('tech_oil').ok).toBe(false);
      ...
      game.restart();
      expect(game.state.nativeMap).toBe(map);
  ...
  it('captures native neutral oil with an Engineer, awards its income and preserves normal victory conditions', () => {
    const game = new Game({ map: load(), ai: false }),
      oil = game.state.entities.find((e) => e.type === 'tech_oil')!;

    const engineer = game.state.entities.find((e) => e.type === 'gi')!;
    engineer.type = 'engineer';
    engineer.x = oil.x - 0.5;
    engineer.y = oil.y + 0.5;
    game.tick(0.25);
    const initial = game.state.sides[0].money;
    game.orderAttack([engineer.id], oil.id);

    for (let i = 0; i < 20; i++) game.tick(0.25);
    expect(oil.side).toBe(0);
    expect(game.state.sides[0].money).toBeGreaterThanOrEqual(initial + 1020);
    expect(game.sell(oil.id)).toBe(false);

    // Captured tech cannot keep a defeated command alive after its base is destroyed.
    for (const e of game.state.entities)
      if (e.side === 0 && isBuilding(game.defs[e.type]) && !game.defs[e.type].mapOnly) e.hp = 0;
    game.tick(0.25);
  ...
  it('loads a selected map after construction and mines its native ore', () => {
    ...
    game.loadNativeMap(load('saffron-wash'));

    const state = game.state,
      initialOre = state.tiles.reduce((sum, tile) => sum + tile.ore, 0);

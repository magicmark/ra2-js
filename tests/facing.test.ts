...
describe('authored integer ROT facing', () => {
  it('takes twelve frames for a quarter-turn at ROT5, following the shortest direction without overshooting', () => {
    let facing = 0, turn: FacingTurn | undefined;
    for (let frame = 1; frame <= 12; frame++) {
      const result = turnFacing(facing, Math.PI / 2, 5, turn); facing = result.facing; turn = result.turn;
      expect(facing).toBeGreaterThan(0); expect(facing).toBeLessThanOrEqual(Math.PI / 2);
  ...
  it('holds a fresh 90 or 180 degree order in place until the authored turn completes, then moves at full speed', () => {
    for (const [heading, target, frames] of [[0, Math.PI / 2, 12], [-Math.PI / 2, Math.PI / 2, 25]] as const) {
      ...
      game.tick(1 / 30); expect(tank.y - 35.5).toBeCloseTo(game.defs[tank.type].speed / 30, 8);
    }
  });
  it('continues translating through a bend on an already moving route', () => {
    ...
    for (let frame = 0; frame < 30; frame++) {
      const before = { x: tank.x, y: tank.y }; game.tick(1 / 30); const distance = Math.hypot(tank.x - before.x, tank.y - before.y);
      if (distance > 0) inMotion = true;
      if (inMotion) expect(distance).toBeGreaterThan(game.defs[tank.type].speed / 30 * .7);
    }
  });
  it('wraps through zero and retargets from the current pose without snapping', () => {
    const initial = Math.PI * 2 - .2, first = turnFacing(initial, .2, 5);
    expect(difference(first.facing, initial)).toBeGreaterThan(0);
    expect(difference(first.facing, initial)).toBeLessThan(.2);
    const retargeted = turnFacing(first.facing, Math.PI, 5, first.turn);
    expect(Math.abs(difference(retargeted.facing, first.facing))).toBeLessThan(.15);
  ...
  it('turns the moving hull at its native rate and lets a stationary turret aim independently', () => {
    ...
    game.stop([tank.id]);
    const friendly = game.state.entities.find(e => e.type === 'gi')!; friendly.x = 22.5; friendly.y = tank.y;
    game.orderAttack([tank.id], friendly.id, true);
    for (let i = 0; i < 15; i++) game.tick(1 / 30);
    expect(tank.facing).toBeCloseTo(Math.PI / 2, 8); expect(difference(tank.turretFacing!, 0)).toBeCloseTo(0, 4);
    expect(friendly.hp).toBeLessThan(friendly.maxHp);
  });
  it('does not rotate a miners hull while digging an ore cell', () => {
    const game = new Game({ ai: false }), miner = game.state.entities.find(e => e.type === 'miner')!;
    const ore = game.state.tiles.findIndex(t => t.ore > 0 && t.terrain === 'grass');
    miner.x = ore % game.state.width + .5; miner.y = Math.floor(ore / game.state.width) + .5; miner.facing = .7;
    game.state.explored.fill(1); game.orderHarvest([miner.id], miner.x, miner.y);
    for (let i = 0; i < 30; i++) game.tick(1 / 30);
    expect(miner.cargo).toBeGreaterThan(0); expect(miner.facing).toBeCloseTo(.7, 3);
  });
});

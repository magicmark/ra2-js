...
function fixture() {
  ...
  const element = (id: string) => {
    if (!elements.has(id)) elements.set(id, {
      value: ORIGINAL, textContent: '', style: {}, readOnly: false, disabled: false,
      classList: { toggle: vi.fn() }, setAttribute: vi.fn(), removeAttribute: vi.fn(), focus: vi.fn(),
    });
  ...
  Object.assign(ui, {
    root: { querySelector: (selector: string) => element(selector.slice(1)), querySelectorAll: (selector: string) => selector === '.asset-submit-button' ? submits : [] },
    loading: true, assetSubmitting: false, assetSourceEdited: false,
    assetStatus: { phase: 'cache' }, actions: { onAssetRetry: vi.fn() },
  });
...
describe('cache restoration presentation', () => {
  it('shows local preparation without telling the user to download again, then offers cached Continue', () => {
    ...
    for (const phase of ['cache', 'extract', 'decode']) {
      ui.setAssetStatus({ phase, detail: 'Preparing saved game files from MIX archives.', source: ORIGINAL });
  ...
  it('does not inherit cached Continue when a different source is being edited', () => {
    ...
    ui.setAssetStatus({ phase: 'download', source: 'https://example.test/my-game.exe', detail: 'Receiving archive bytes.' });
  ...
  it('retains storage warnings alongside ready Continue and truthful retry errors', () => {
    ...
    ui.setAssetStatus({ phase: 'error', error: 'Saved MIX artwork selection failed.', source: ORIGINAL, cacheWarning });
  ...
  it('preserves the actionable saved-work reason and coalesces explicit retry requests', () => {
    ...
    const detail = 'Saved files need a newer artwork decoder. Press Enter to retry using saved work.';
    ...
    ui.setAssetStatus({ phase: 'error', error: 'Saved archive still needs repair.', source: ORIGINAL });
    expect(submits.every(button => !button.disabled)).toBe(true);
  });
});

...
function arena(type: 'miner' | 'warminer' | 'grizzly' = 'miner') {
  ...
  game.state.entities = game.state.entities.filter(
    (e) => e === unit || e.type === 'conyard' || e.type === 'refinery',
  );
...
function stepWithoutSwooping(game: Game, unit: Entity) {
  ...
  if (moved > 1e-6) {
    ...
    expect(
      Math.abs(difference(Math.atan2(unit.y - before.y, unit.x - before.x), unit.facing)),
    ).toBeLessThan(1e-4);
  }
...
describe('native harvester movement and gathering state', () => {
  ...
  it('turns in place at a route corner, including a partial-frame boundary, while straight sections keep full speed', () => {
    ...
    for (let i = 0; i < 180; i++) {
      const oldFacing = unit.facing,
        distance = stepWithoutSwooping(game, unit);

      moved += distance;

      if (Math.abs(difference(oldFacing, unit.facing)) > 1e-4) {
        turning++;
        expect(distance).toBe(0);
      }

      if (unit.x < 21.4 && distance > 0)
        expect(distance).toBeCloseTo(game.defs.grizzly.speed / 30, 8);
    }

    expect(turning).toBeGreaterThan(0);
    expect(moved).toBeCloseTo(4, 8);
  ...
  it('keeps automatic refinery returns and repeat harvesting free of simultaneous turning and translation', () => {
    ...
    game.orderMove([unit.id], 26.5, 35.5);
    game.tick(1 / 30);
  ...
  it('only marks active gathering, freezes it on pause, and clears it after stop', () => {
    ...
    game.orderHarvest([unit.id], unit.x, unit.y);
    game.tick(1 / 30);
    expect(unit.harvesting).toBe(true);
    const anim = unit.anim;
    game.state.paused = true;
    game.tick(1);
    expect(unit.anim).toBe(anim);
...
describe('full Chrono Miner refinery teleport', () => {
  it('teleports over impassable terrain with original endpoint effects, unloads once, then drives back to harvest', () => {
    ...
    expect(unit.order).toBe('harvest');
  ...
  it('waits for a free dock, avoids other miners, and can use a captured Soviet refinery', () => {
    const { game, unit } = arena();
    const refinery = game.state.entities.find((e) => e.type === 'refinery')!;
    refinery.type = 'refinery_soviet';
    refinery.x = 10;
    refinery.y = 35;
    unit.cargo = 500;

    for (let y = 34; y <= 38; y++)
      for (let x = 9; x <= 14; x++) game.state.tiles[y * game.state.width + x].terrain = 'water';
    game.tick(1 / 30);
    expect(unit.x).toBe(20.5);
    expect(unit.cargo).toBe(500);
    expect(unit.path).toHaveLength(0);
    ...
    blocker.x = 25.5;
    game.tick(1 / 30);
    expect(unit.x).toBe(9.5);
    expect(unit.y).toBe(35.5);
  });

  it.each(['absent', 'enemy', 'selling', 'constructing', 'destroyed'])(
    'does not teleport to an %s refinery',
    (state) => {
      const { game, unit } = arena();
      const refinery = game.state.entities.find((e) => e.type === 'refinery')!;

      if (state === 'absent')
        game.state.entities = game.state.entities.filter((e) => e !== refinery);

      if (state === 'enemy') refinery.side = 1;

      if (state === 'selling') refinery.selling = { startedAt: 0, duration: 10 };
  ...
  it.each(['miner', 'warminer'] as const)(
    'keeps manual moves and non-full %s returns on land',
    (type) => {
      const { game, unit } = arena(type);
      unit.cargo = 100;
      unit.order = 'return';
      game.tick(1 / 30);
      expect(unit.x).toBeGreaterThan(19);

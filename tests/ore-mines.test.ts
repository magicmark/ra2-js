...
// SAFETY: the checked-in audit inventory is generated from original INI sections with numeric first/count and string file fields.
const { originalRoadSets } = JSON.parse(
...
describe('original ore mines', () => {
  for (const id of ['training', ...MAP_CATALOG.map((entry) => entry.id)])
    it(`${id}: every drill avoids original road art before and after initialization`, () => {
      const map =
        id === 'training'
          ? undefined
          : parseNativeMap(readFileSync(`public/maps/${id}.map`, 'utf8'));

      const before = map
        ? structuredClone(map.cells)
        : createMap().map((t, i) => ({
            x: i % MAP_SIZE,
      ...
      const positions = map
        ? map.terrain.filter((t) => t.type.toUpperCase() === 'TIBTRE01')
        : DEFAULT_ORE_MINES;
      ...
      expect(after.oreMines!.map(({ x, y }) => ({ x, y }))).toEqual(
      ...
      for (const { x, y } of positions) {
        ...
        const initialized = after.nativeMap
          ? after.nativeMap.cells.find((c) => c.x === x && c.y === y)!
          : after.tiles[y * after.width + x].nativeArt!;

        for (const art of [source, initialized]) {
          expect(art, `${id} (${x},${y}) must have original artwork`).toBeDefined();
          expect(
            roads.some(
              (set) => art.tileIndex >= set.first && art.tileIndex < set.first + set.count,
            ),
            `${id} (${x},${y}) tile ${art.tileIndex}/${art.subTile} overlaps original road art`,
          ).toBe(false);
        }

        expect([initialized.tileIndex, initialized.subTile]).toEqual([
          source.tileIndex,
          source.subTile,
        ]);
      }
  ...
  it('keeps the southern training road clear and the relocated drill in its ore field', () => {
    const before = createMap(),
      state = new Game({ ai: false }).state;

    const mine = state.oreMines!.find((m) => m.x === 20 && m.y === 51)!;
    expect(mine).toBeDefined();
    expect(before[51 * MAP_SIZE + 20]).toMatchObject({
      terrain: 'grass',
      nativeArt: { tileIndex: 0 },
    });
    ...
    expect(
      mine.fieldCells.some(
        (i) => Math.hypot((i % MAP_SIZE) - mine.x, Math.floor(i / MAP_SIZE) - mine.y) <= 1,
      ),
    ).toBe(true);
    ...
    for (let x = 22; x <= 24; x++) {
      ...
      expect(state.tiles[i]).toMatchObject({
        terrain: 'road',
        ore: 0,
        nativeArt: { tileIndex: 294, subTile: x - 22 },
      });
    }
  });
  ...
  it('places the original mine at each default field and animates with authored timing', () => {
    const game = new Game({ ai: false }),
      mine = game.state.oreMines![0];

    expect(game.state.oreMines).toHaveLength(6);
    expect(game.state.tiles[mine.y * game.state.width + mine.x]).toMatchObject({
      terrain: 'rock',
      ore: 0,
    });
  ...
  it('replenishes bounded nearby depleted ground without growing on occupied or impassable cells', () => {
    ...
    for (let dy = -2; dy <= 2; dy++)
      for (let dx = -2; dx <= 2; dx++)
        state.tiles[(mine.y + dy) * state.width + mine.x + dx] = {
          terrain: 'water',
          ore: 0,
          variant: 0,
        };

    const eligible = (mine.y + 1) * state.width + mine.x,
      occupied = mine.y * state.width + mine.x + 1;

    state.tiles[eligible].terrain = 'sand';
    state.tiles[occupied].terrain = 'grass';
    state.entities = [entity({ id: 1, type: 'power', x: mine.x + 1, y: mine.y, hp: 100 })];
    const before = state.tiles.map((t) => t.ore);

    for (let i = 0; i < 20; i++) {
      mine.animationStartedAt = state.time;
      state.time += 1.2;
      updateOreMines(state, game.defs);
    }
  ...
  it('replenishes after simulated time and pauses with the game', () => {
    ...
    game.state.oreMines = [mine];
    game.state.entities = game.state.entities.filter(
      (e) => e.type === 'conyard' || e.type === 'conyard_soviet',
    );
    ...
    for (let dy = -1; dy <= 1; dy++)
      for (let dx = -1; dx <= 1; dx++)
        if (dx || dy)
          game.state.tiles[(mine.y + dy) * game.state.width + mine.x + dx] = {
            terrain: 'sand',
            ore: 0,
            variant: 0,
          };

    const amount = () => game.state.tiles.reduce((sum, tile) => sum + tile.ore, 0),
      before = amount();
  ...
  it('leaves a healthy field idle without motion or added ore', () => {
    ...
    for (let i = 0; i < 3000; i++) {
      state.time += 1 / 30;
      updateOreMines(state, game.defs);
      expect(oreMineFrame(mine, state.time)).toBe(0);
    }

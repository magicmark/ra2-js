...
describe('TOML game rules and starting state', () => {
  it('loads linked prerequisites and different factions from a shared TOML ruleset', () => {
    ...
    expect(() => parseDefinitions('[units.bad]\nname="Broken"\ncategory="vehicles"\ncost=100\nrequires=["missing"]')).toThrow('unknown unit');
  });
  ...
  it('starts both sides with independent economies, ore access and only player vision', () => {
    ...
    for (const e of game.state.entities.filter(e => !isBuilding(game.defs[e.type]))) expect(footprintCells.has(`${Math.floor(e.x)},${Math.floor(e.y)}`)).toBe(false);
  });
});
...
describe('construction and per-side production', () => {
  ...
  it('spawns paid units in separate free cells and applies a barracks rally point', () => {
    ...
    game.build('gi');
    game.build('gi');
    advance(game, game.defs.gi.buildTime * 2 + .1);
    ...
    for (let i = 0; i < infantry.length; i++) for (let j = i + 1; j < infantry.length; j++)
      expect(Math.hypot(infantry[i].x - infantry[j].x, infantry[i].y - infantry[j].y)).toBeGreaterThanOrEqual(0.47);
  });
});
...
describe('pathfinding and commands', () => {
  ...
  it('ignores enemy selections and movement, harvest, stop, sell and repair commands', () => {
    const game = peaceful();
    const own = entity(game, 'grizzly'), enemy = entity(game, 'rhino', 1), refinery = entity(game, 'refinery_soviet', 1);
  ...
  it('lets opposing mobile traffic pass without permanent collision or overlap', () => {
    ...
    units[0].x = 19.5; units[0].y = 35.5;
    units[1].x = 25.5; units[1].y = 35.5;
    game.orderMove([units[0].id], 25.5, 35.5);
    game.orderMove([units[1].id], 19.5, 35.5);
    for (let i = 0; i < 120; i++) {
      game.tick(0.1);
      expect(Math.hypot(units[0].x - units[1].x, units[0].y - units[1].y)).toBeGreaterThanOrEqual(0.47);
    }
...
describe('ore miners', () => {
  ...
  it('keeps multiple miners depositing through shared routes for several cycles', () => {
    ...
    expect(Math.hypot(miners[0].x - miners[1].x, miners[0].y - miners[1].y)).toBeGreaterThanOrEqual(0.47);
  });
});
...
describe('combat, AI and match lifecycle', () => {
  it('does not allow targeted attacks into shroud and resolves visible combat with kills and effects', () => {
    const game = peaceful();
    const tank = entity(game, 'grizzly'), enemy = entity(game, 'rhino', 1);
    game.orderAttack([tank.id], enemy.id);
    expect(tank.targetId).toBeNull();
    enemy.x = 23.5; enemy.y = 41.5; enemy.hp = 30;
    advance(game, 0.5);
    game.orderAttack([tank.id], enemy.id);
    advance(game, 1.5);
  ...
  it('consumes an engineer to capture a visible enemy structure', () => {
    const game = peaceful();
    game.build('engineer');
    advance(game, game.defs.engineer.buildTime + .1);
    const engineer = entity(game, 'engineer'), target = entity(game, 'conyard', 1);
  ...
  it('builds an enemy economy, produces armor and launches an attacking force', () => {
    ...
    expect(startingBuildings.some(type => {
  ...
  it('declares either winner when a base is eliminated, stops simulation and restarts all state', () => {
    for (const losingSide of [0, 1]) {
      const game = peaceful();
      for (const e of game.state.entities) if (e.side === losingSide && isBuilding(game.defs[e.type])) e.hp = 0;
      advance(game, 0.1);

...
function arena() {
  ...
  game.state.entities = game.state.entities.filter(entity => entity === tank || entity.type === 'conyard');
...
describe('default game pace', () => {
  it('starts at Faster and moves a Grizzly twice as far per real second as Fast', () => {
    const current = arena(), previous = arena(); previous.game.setGameSpeed(1);
    expect(current.game.state.speed).toBe(2);
    expect(current.game.nativeGameSpeedIndex).toBe(1);
    advanceWallTime(current.game, 1); advanceWallTime(previous.game, 1);
    ...
    expect(current.game.state.sides[0].queues.structures[0].progress).toBeCloseTo(2 / current.game.defs.power.buildTime, 8);
  });
  ...
  it('preserves movement, turning and production at equal simulation time across render rates', () => {
    const run = (hz: number, speed?: number) => {
      ...
      return { time: game.state.time, x: tank.x, y: tank.y, facing: tank.facing, path: tank.path,
        money: game.state.sides[0].money, progress: game.state.sides[0].queues.structures[0].progress };
    };
  ...
  it('keeps pause and the fixed logic step at the faster default', () => {
    const { game, tank } = arena();
    game.state.paused = true; advanceWallTime(game, 1);
    expect(game.state.time).toBe(0); expect(tank.y).toBe(30.5);
    game.state.paused = false;
    game.tick(1 / 120); expect(game.state.time).toBe(0); expect(game.interpolation).toBeCloseTo(.5);
    game.tick(1 / 120); expect(game.state.time).toBeCloseTo(1 / 30);
  ...
  it('uses Faster on native maps and retains the chosen speed across restart and map changes', () => {
    ...
    for (const speed of [2, 1, .5]) {
      game.setGameSpeed(speed); game.restart(); expect(game.state.speed).toBe(speed);
      game.loadNativeMap(map); expect(game.state.speed).toBe(speed);
    }
  });
});

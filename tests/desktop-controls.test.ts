...
function fixture() {
  ...
  vi.stubGlobal('document', {
    documentElement: { classList: { contains: () => settingsOpen } },
  ...
  const renderer: ControlRenderer = {
    ...
    selectionBox: null,
    pick: () => picked ?? null,
    visible: (e: Entity) => e.x < 30,
    entityPoint: (e: Entity) => camera.screen(e.x, e.y),
  };
  ...
  const key = (
    ...
    Object.defineProperty(event, 'target', { value: new KeyTarget(kind) });
  ...
  const click = (entity?: Entity, options = {}) => {
    picked = entity;
    pointer('pointerdown', 300, 300, options);
    pointer('pointerup', 300, 300, options);
  };
...
describe('unit selection feedback', () => {
  it.each(['mouse', 'touch'])(
    'keeps George out of %s selection and select-all, while retaining friendly hover',
    (pointerType) => {
      const { game, click, controls, callbacks, key } = fixture();
      const george = game['spawn']('george', 0, 20, 20);
      ...
      expect(george.selected).toBe(false);
      expect(controls.cursor).toBe('default');
    },
  );

  it.each(['mouse', 'touch'])(
    'acknowledges a %s Sniper selection once and keeps orders and deselection separate',
    (pointerType) => {
      const { game, click, callbacks } = fixture();
      const sniper = game['spawn']('sniper', 0, 20, 20);
      click(sniper, { pointerType });
      expect(sniper.selected).toBe(true);
      expect(callbacks.ack).toHaveBeenCalledExactlyOnceWith(['sniper']);
      callbacks.ack.mockClear();
      click(undefined, { pointerType });
      expect(callbacks.ack).toHaveBeenCalledExactlyOnceWith();
      callbacks.ack.mockClear();
      click(sniper, { pointerType, shiftKey: true });
      expect(sniper.selected).toBe(false);
      expect(callbacks.ack).not.toHaveBeenCalled();
      click(undefined, { button: 2, pointerType });
      expect(callbacks.ack).not.toHaveBeenCalled();
    },
  );
  ...
  it('voices keyboard selection and group recall, filtering stale group members', () => {
    ...
    game.select([
      Math.max(
        ...game.state.entities.filter((e) => e.side === 0 && e.id < sniper.id).map((e) => e.id),
      ),
    ]);
    callbacks.ack.mockClear();
    key('m');
    expect(callbacks.ack).toHaveBeenCalledExactlyOnceWith(['sniper']);
    sniper.transportId = 123;
    callbacks.ack.mockClear();
    key('1');
    expect(callbacks.ack).not.toHaveBeenCalled();
    sniper.transportId = undefined;
    sniper.hp = 0;
    key('1');
    expect(callbacks.ack).not.toHaveBeenCalled();
  });
...
function productionFixture() {
  const f = fixture(),
    onPlace = vi.fn((type: string) => f.controls.setPlacement(type));
  ...
  const ui = Object.assign(Object.create(UI.prototype), {
    game: f.game,
    category: 'structures',
    actions: { onPlace, onCategoryChange },
    isModalOpen: () => false,
    renderCards: vi.fn(),
    update: vi.fn(),
    showToast: vi.fn(),
    closeBuildPanel: vi.fn(),
  });
...
describe('production hotkeys pick up ready buildings', () => {
  it('sounds only changed tabs while same-tab hotkeys still pick up the ready building', () => {
    ...
    key('r');
    key('q');
    expect(onCategoryChange).toHaveBeenCalledTimes(4);
    key('Escape');
    key('q');
    expect(renderer.placement).toBe('power');
    expect(onCategoryChange).toHaveBeenCalledTimes(4);
    ui.selectCategory('defenses');
    expect(onCategoryChange).toHaveBeenCalledTimes(5);
    ui.selectCategory('defenses');
    expect(onCategoryChange).toHaveBeenCalledTimes(5);
    ui.isModalOpen = () => true;
    ui.selectCategory('vehicles');
    expect(ui.category).toBe('defenses');
    expect(onCategoryChange).toHaveBeenCalledTimes(5);
  });
  ...
  it('selects ready structures/defenses, replaces the preview, and preserves paid queues on cancellation', () => {
    ...
    expect(game.build('pillbox')).toBe(true);
    game.tick(0.1);
    const before = JSON.stringify(game.state.sides[0]);
    key('q');
    expect(renderer.placement).toBe('power');
    expect(ui.category).toBe('structures');
    key('w');
    expect(renderer.placement).toBe('pillbox');
    expect(ui.category).toBe('defenses');
    key('q', { repeat: true });
    expect(renderer.placement).toBe('pillbox');
  ...
  it('only switches tabs for empty or unfinished queues without starting or resuming production', () => {
    ...
    key('e');
    key('r');
    expect(ui.category).toBe('vehicles');
  ...
  it('keeps ready buildings usable after losing prerequisites and at unique build limits', () => {
    ...
    game['spawn']('battlelab', 0, 30, 30);
    game.configureLocalTools({ instantBuild: true });
    expect(game.build('chronosphere')).toBe(true);
    expect(game.build('power')).toBe(true);
    game.tick(0.1);
    expect(game.canBuild('chronosphere').reason).toBe('Build limit reached');
    key('w');
    expect(renderer.placement).toBe('chronosphere');
  ...
  it('respects remapped production keys and preserves default D deployment', () => {
    ...
    game.select([gi.id]);
    key('d');
    expect(gi.deployed).toBe(true);
    expect(renderer.placement).toBeNull();
    ui.selectCategory('structures');
    expect(renderer.placement).toBeNull(); // Mouse tab remains just a tab.
  });
  ...
  it('keeps an existing preview when opening unit tabs and ignores guarded keyboard events', () => {
    ...
    key('q');
    key('e');
    expect(ui.category).toBe('infantry');
    expect(renderer.placement).toBe('power');
    key('r');
    expect(ui.category).toBe('vehicles');
    expect(renderer.placement).toBe('power');
    controls.resetBindings();
    expect(controls.getBindings().find((b) => b.id === 'deploy')!.key).toBe('D');
  });
});
...
describe('retail mouse commands', () => {
  it('targets a Chronosphere source and destination and cancels superweapon targeting', () => {
    ...
    controls.setMode('chrono-source');
    click();
    expect(controls.mode).toBe('chrono-destination');
    expect(activate).not.toHaveBeenCalled();
    click();
    expect(activate).toHaveBeenCalledWith('chronosphere', expect.any(Object), expect.any(Object));
    expect(controls.mode).toBe('select');
    controls.setMode('weather');
    key('Escape');
    expect(controls.mode).toBe('select');
  ...
  it('sends selected engineers into friendly damaged buildings from the battlefield and radar', () => {
    ...
    const engineer = game.state.entities.find((e) => e.type === 'gi')!,
      target = game.state.entities.find((e) => e.type === 'power')!;
    ...
    pick(target);
    pointer('pointermove', 300, 300);
    expect(controls.cursor).toBe('repair');
    click(target);
    expect(attack).toHaveBeenLastCalledWith([engineer.id], target.id);
    expect(engineer.selected).toBe(true);
    controls.radarOrder(target.x + 0.5, target.y + 0.5);
    ...
    pick(target);
    pointer('pointermove', 300, 300);
    expect(controls.cursor).toBe('enter');
    click(target);
    expect(attack).toHaveBeenLastCalledWith([engineer.id], target.id);
  });
  it('centers an unselected radar click and sends selected radar commands with planning and modifiers', () => {
    const { game, controls, camera } = fixture();
    controls.radarOrder(25, 30);
    expect(camera.world(camera.width / 2, camera.height / 2)).toEqual({ x: 25, y: 30 });
    const tank = game.state.entities.find((e) => e.type === 'grizzly')!;
    game.select([tank.id]);
    const move = vi.spyOn(game, 'orderMove');
    controls.radarOrder(22.5, 35.5, { ctrl: true, shift: true });
    expect(move).toHaveBeenLastCalledWith([tank.id], 22.5, 35.5, true);
    controls.executeCommand('planning');
    const route = vi.spyOn(game, 'orderWaypoints');
    controls.radarOrder(24, 35);
    expect(route).not.toHaveBeenCalled();
    controls.executeCommand('planning');
    expect(route).toHaveBeenCalledWith([tank.id], [{ x: 24, y: 35 }]);
    controls.radarOrder(24, 35, { button: 2 });
    expect(tank.selected).toBe(false);
  });
  it('uses left click to command; right click deselects or cancels without issuing a command', () => {
    ...
    click(undefined, { button: 2 });
    expect(tank.selected).toBe(false);
    expect(tank.path).toBe(path);
    expect(move).toHaveBeenCalledOnce();
    game.select([tank.id]);
    controls.setPlacement('power');
    click(undefined, { button: 2 });
    expect(renderer.placement).toBeNull();
    expect(tank.selected).toBe(true);
  });
  it('toggles selected units with Shift and deploys an already selected GI', () => {
    const { game, click } = fixture();
    const units = game.state.entities.filter((e) => e.type === 'gi');
    click(units[0]);
    click(units[1], { shiftKey: true });
    expect(units[0].selected && units[1].selected).toBe(true);
    click(units[0], { shiftKey: true });
    expect(units[0].selected).toBe(false);
    expect(units[1].selected).toBe(true);
    click(units[1]);
    expect(units[1].deployed).toBe(true);
    click(units[1]);
    expect(units[1].deployed).toBe(false);
  });
  it('routes attack-move, force fire, and force move modifiers before friendly selection', () => {
    ...
    const move = vi.spyOn(game, 'orderMove'),
      ...
      force = vi.spyOn(game, 'orderForceMove');

    click(friendly, { ctrlKey: true, shiftKey: true });
    expect(move).toHaveBeenLastCalledWith([tank.id], expect.any(Number), expect.any(Number), true);
    click(friendly, { ctrlKey: true });
    expect(attack).toHaveBeenLastCalledWith([tank.id], friendly.id, true);
    expect(friendly.selected).toBe(false);
    click(friendly, { altKey: true });
  ...
  it('right drag scrolls without deselecting and pointer cancellation cannot release an order', () => {
    const { game, controls, camera, pointer, click } = fixture();
    const tank = game.state.entities.find((e) => e.type === 'grizzly')!;
    click(tank);
    const x = camera.x;
    pointer('pointerdown', 300, 300, { button: 2 });
    pointer('pointermove', 350, 300, { button: 2 });
    pointer('pointerup', 350, 300, { button: 2 });
    expect(camera.x).toBeLessThan(x);
    expect(tank.selected).toBe(true);
    const move = vi.spyOn(game, 'orderMove');
    pointer('pointerdown');
...
describe('retail keyboard commands', () => {
  it('reassigns an occupied core key, unbinds its previous command, and restores defaults', () => {
    const { game, controls, key, callbacks } = fixture(),
      tank = game.state.entities.find((e) => e.type === 'grizzly')!;

    game.select([tank.id]);

    const stop = vi.spyOn(game, 'stop'),
      guard = vi.spyOn(game, 'guard');

    expect(controls.inspectBinding('stop', 'G')).toMatchObject({
      ok: true,
      conflict: { id: 'guard', label: 'Guard' },
    });
    ...
    key('g');
    expect(stop).toHaveBeenCalledWith([tank.id]);
    expect(guard).not.toHaveBeenCalled();
    controls.assignBinding('structures', 'B');
    key('q');
    expect(callbacks.category).not.toHaveBeenCalled();
    key('b');
    expect(callbacks.category).toHaveBeenCalledWith('structures');
    controls.resetBindings();
    key('g');
    expect(guard).toHaveBeenCalledWith([tank.id]);
    expect(controls.getBindings().every((binding) => binding.key === binding.defaultKey)).toBe(
      true,
    );
  });
  it('remaps held planning and preserves modal, text, and browser shortcut guards', () => {
    const { game, controls, key, click, settings } = fixture(),
      tank = game.state.entities.find((e) => e.type === 'grizzly')!;

    game.select([tank.id]);
    controls.assignBinding('planning', 'B');

    const route = vi.spyOn(game, 'orderWaypoints'),
      stop = vi.spyOn(game, 'stop');

    key('z');
    expect(controls.planning).toBe(false);
    key('b');
    expect(controls.planning).toBe(true);
    click();
    expect(route).not.toHaveBeenCalled();
    key('b', { type: 'keyup' });
    expect(route).toHaveBeenCalledOnce();
    controls.assignBinding('stop', 'C');
    key('c', { ctrlKey: true });
    key('c', { kind: 'input' });
    settings(true);
    key('c');
    ...
    for (const reserved of ['Escape', 'F1', '1', 'ArrowLeft', 'Enter', 'Ctrl+C', ''])
      expect(controls.assignBinding('stop', reserved).ok).toBe(false);
  ...
  it('persists replaced bindings without touching asset preferences and tolerates unavailable storage', () => {
    ...
    fixture().controls.assignBinding('stop', 'G');
    const restored = fixture().controls;
    ...
    expect(() => denied.assignBinding('stop', 'B')).not.toThrow();
    expect(() => denied.resetBindings()).not.toThrow();
  });
  it('adjusts continuous scroll speed while preserving direct dragging and modal suppression', () => {
    const { controls, camera, key, pointer, settings } = fixture(),
      pan = vi.spyOn(camera, 'pan');

    expect(controls.scrollRate).toBe(1);
    key('ArrowLeft');
    controls.tick(0.1);
    expect(pan).toHaveBeenLastCalledWith(50, 0);
    controls.setScrollRate(2);
    controls.tick(0.1);
    expect(pan).toHaveBeenLastCalledWith(100, 0);
    ...
    pointer('pointerup', 350, 300, { button: 2 });
    expect(pan).toHaveBeenLastCalledWith(50, 0);
    controls.setScrollRate(99);
    expect(controls.scrollRate).toBe(3);
    controls.setScrollRate(NaN);
    expect(controls.scrollRate).toBe(3);
    controls.setScrollRate(0);
    expect(controls.scrollRate).toBe(0.25);
    const calls = pan.mock.calls.length;
    key('ArrowLeft');
    settings(true);
    controls.tick(0.1);
    expect(pan).toHaveBeenCalledTimes(calls);
  });
  it('shares native command-bar actions with hotkeys, including initial team assignment, disbanding and persistent planning', () => {
    const { game, controls, click } = fixture();
    const unit = game.state.entities.find((e) => e.type === 'gi')!;
    game.select([unit.id]);
    controls.executeCommand('team1');
    game.select([]);
    controls.executeCommand('team1');
    expect(unit.selected).toBe(true);
    controls.executeCommand('team1', { clear: true });
    game.select([]);
    controls.executeCommand('team1');
    ...
    expect(controls.planning).toBe(false);
    expect(route).toHaveBeenCalledOnce();
    controls.executeCommand('deploy');
  ...
  it('maps P to all units, QWER tabs, Esc options, and leaves multiplayer A unassigned', () => {
    ...
    expect(game.state.entities.filter((e) => e.selected)).toHaveLength(6);

    for (const k of ['q', 'w', 'e', 'r']) key(k);
    expect(callbacks.category.mock.calls.flat()).toEqual([
      'structures',
      'defenses',
      'infantry',
      'vehicles',
    ...
    key('Escape');
    expect(callbacks.options).toHaveBeenCalledOnce();
  });
  it('selects the current type onscreen, then across the map on a second T', () => {
    const { game, key } = fixture();
    const units = game.state.entities.filter((e) => e.type === 'gi');
    units[3].x = 50;
    game.select([units[0].id]);
    key('t');
    expect(units.filter((e) => e.selected)).toHaveLength(3);
    key('t');
    expect(units.filter((e) => e.selected)).toHaveLength(4);
  });
  it('assigns and recalls teams and camera bookmarks with browser defaults suppressed', () => {
    const { game, key, camera } = fixture();
    const tank = game.state.entities.find((e) => e.type === 'grizzly')!;
    game.select([tank.id]);
    expect(key('1', { ctrlKey: true }).defaultPrevented).toBe(true);
    game.select([]);
    key('1');
    expect(tank.selected).toBe(true);
    camera.center(30, 30);
    const bookmark = { x: camera.x, y: camera.y };
    expect(key('F1', { ctrlKey: true }).defaultPrevented).toBe(true);
    camera.center(15, 15);
    key('F1');
    expect({ x: camera.x, y: camera.y }).toEqual(bookmark);
  });
  it('supports stop, guard, scatter, deploy, repair/sell modes, and follow', () => {
    const { game, controls, key, camera, click } = fixture();
    const unit = game.state.entities.find((e) => e.type === 'gi')!;
    game.select([unit.id]);
    game.orderMove([unit.id], 21, 38);
    key('s');
    expect(unit.path).toHaveLength(0);
    const guard = vi.spyOn(game, 'guard');
    key('g');
    expect(guard).toHaveBeenCalledWith([unit.id]);
    key('d');
    expect(unit.deployed).toBe(true);
    key('x');
    expect(unit.deployed).toBe(false);
    expect(unit.path.length).toBeGreaterThan(0);
    key('f');
    unit.x = 22;
    unit.y = 35;
    unit.previous = { x: 22, y: 35 };
    controls.tick(0.05);
    expect(camera.world(camera.width / 2, camera.height / 2)).toEqual({ x: 22, y: 35 });
    const power = game.state.entities.find((e) => e.type === 'power')!;
    power.hp -= 100;
    const repair = vi.spyOn(game, 'repair');
    key('k');
    expect(controls.mode).toBe('repair');
    click(power);
    expect(repair).toHaveBeenCalledWith(power.id);
    key('l');
    expect(controls.mode).toBe('sell');
    key('Escape');
    expect(controls.mode).toBe('select');
  });
  it('queues waypoints while Z is held and starts the route only on release', () => {
    const { game, key, click } = fixture();
    ...
    expect(unit.path).toHaveLength(0);
    key('z', { type: 'keyup' });
    expect(route).toHaveBeenCalledWith([unit.id], [expect.any(Object), expect.any(Object)]);
  });
  it('cycles creation order, previous selection, and health bands', () => {
    ...
    expect(units[1].selected).toBe(true);
    key('n');
    expect(units[0].selected).toBe(true);
    units[0].hp = units[0].maxHp * 0.4;
    units[1].hp = units[1].maxHp * 0.2;
    key('u');
    expect(units[0].selected || units[1].selected).toBe(false);
    key('u');
    expect(units[0].selected).toBe(true);
    key('u');
    expect(units[1].selected).toBe(true);
  ...
  it('does not hijack text, modals, source gate, browser shortcuts, or button Space/Enter', () => {
    ...
    key('p', { kind: 'input' });
    key('p', { kind: 'dialog' });
    expect(game.state.entities.some((e) => e.selected)).toBe(false);
    settings(true);
    key('p');
    settings(false);
    enable(false);
    ...
    expect(game.state.entities.some((e) => e.selected)).toBe(false);
    enable(true);
    expect(key('r', { ctrlKey: true }).defaultPrevented).toBe(false);
    expect(key(' ').defaultPrevented).toBe(false);
    expect(key('Enter').defaultPrevented).toBe(false);
    expect(controls.mode).toBe('select');
  });
  it('follows the interpolated presentation point between logic frames', () => {
    const { game, controls, key, camera } = fixture();
    const unit = game.state.entities.find((e) => e.type === 'grizzly')!;
    game.select([unit.id]);
    game.orderMove([unit.id], 22.5, 36.5);
    key('f');
    game.tick(0.05);
    controls.tick(0.05);

    const point = camera.world(camera.width / 2, camera.height / 2),
      alpha = game.interpolation;
  ...
  it('resets stale groups and gestures on restart and suppresses camera scrolling while a modal is open', () => {
    const { game, controls, key, camera, settings } = fixture();
    const unit = game.state.entities.find((e) => e.type === 'gi')!;
    game.select([unit.id]);
    key('1', { ctrlKey: true });
    game.restart();
...
describe('native contextual cursors', () => {
  it.each(['miner', 'warminer'])(
    'shows attack over ore for a selected %s while clicks still harvest',
    (type) => {
      const { game, controls, camera, pointer, callbacks } = fixture();

      const miner = game.state.entities.find((e) => e.type === 'miner')!,
        tank = game.state.entities.find((e) => e.type === 'grizzly')!;

      miner.type = type;
      camera.center(30.5, 30.5);
      const index = 30 * game.state.width + 30;
      Object.assign(game.state.tiles[index], { terrain: 'grass', ore: 500 });
      game.state.explored[index] = 1;
      game.stop([miner.id, tank.id]);
      game.select([miner.id, tank.id]);

      const harvest = vi.spyOn(game, 'orderHarvest'),
        attack = vi.spyOn(game, 'orderAttack'),
        move = vi.spyOn(game, 'orderMove');

      pointer('pointermove', 500, 350);
      expect(callbacks.cursor).toHaveBeenLastCalledWith('attack');
      expect(harvest).not.toHaveBeenCalled();
      pointer('pointerdown', 500, 350);
      expect(controls.cursor).toBe('attack');
      pointer('pointerup', 500, 350);
      expect(controls.cursor).toBe('attack');
      expect(harvest).toHaveBeenCalledExactlyOnceWith([miner.id, tank.id], 30, 30);
      expect(miner.order).toBe('harvest');
      expect(tank.order).toBe('guard');
      expect(attack).not.toHaveBeenCalled();
      expect(move).not.toHaveBeenCalled();
      game.select([miner.id]);
      expect(controls.cursor).toBe('attack');
      pointer('pointermove', 500, 350, { altKey: true });
      expect(controls.cursor).toBe('move');
      pointer('pointermove', 500, 350, { ctrlKey: true, shiftKey: true });
      expect(controls.cursor).toBe('attackmove');
      pointer('pointermove', 500, 350, { ctrlKey: true, altKey: true });
      expect(controls.cursor).toBe('guard');
      pointer('pointermove', 500, 350);
      game.state.tiles[index].ore = 0;
      expect(controls.cursor).toBe('move');
      game.state.tiles[index].ore = 500;
      game.select([tank.id]);
      expect(controls.cursor).toBe('move');
      game.select([]);
      expect(controls.cursor).toBe('default');
  ...
  it('offers IFV entry only to eligible infantry and clicks board only infantry in a mixed selection', () => {
    ...
    const tank = game.state.entities.find((e) => e.type === 'grizzly')!,
      gi = game.state.entities.find((e) => e.type === 'gi')!;

    const ifv = game['spawn']('ifv', 0, gi.x + 1, gi.y);
    pick(ifv);
    pointer('pointermove', 500, 350);
    game.select([tank.id]);
    expect(controls.cursor).toBe('select');
    click(ifv);
    game.tick(0.1);
    expect(tank.transportId).toBeUndefined();
    game.select([gi.id]);
    expect(controls.cursor).toBe('enter');
    game.select([tank.id, gi.id]);
    expect(controls.cursor).toBe('enter');
    click(ifv);
    game.tick(0.1);
    expect(ifv.passengers).toEqual([gi]);
    expect(gi.transportId).toBe(ifv.id);
    expect(tank.transportId).toBeUndefined();
    const other = game.state.entities.find((e) => e.type === 'gi' && e !== gi)!;
    game.select([other.id]);
    expect(controls.cursor).toBe('select');
    game.deploy([ifv.id]);
    expect(controls.cursor).toBe('enter');
    game.select([ifv.id]);
  ...
  it('tracks selection, modifiers, terrain and real repair/sell eligibility without issuing orders', () => {
    ...
    const tank = game.state.entities.find((e) => e.type === 'grizzly')!,
      gi = game.state.entities.find((e) => e.type === 'gi')!,
      power = game.state.entities.find((e) => e.type === 'power')!;

    camera.center(30.5, 30.5);
    game.state.tiles[30 * game.state.width + 30].terrain = 'grass';
    pointer('pointermove', 500, 350);
    expect(controls.cursor).toBe('default');
    game.select([tank.id]);
    controls.tick(0);
    expect(callbacks.cursor).toHaveBeenLastCalledWith('move');
    const move = vi.spyOn(game, 'orderMove');
    pointer('pointermove', 500, 350, { ctrlKey: true, shiftKey: true });
    expect(controls.cursor).toBe('attackmove');
    pointer('pointermove', 500, 350, { ctrlKey: true, altKey: true });
    expect(controls.cursor).toBe('guard');
    pointer('pointermove', 500, 350, { ctrlKey: true });
    expect(controls.cursor).toBe('attack');
    pointer('pointermove', 500, 350);
    game.state.tiles[30 * game.state.width + 30].terrain = 'water';
    expect(controls.cursor).toBe('move-blocked');
    pick(gi);
    game.select([gi.id]);
    expect(controls.cursor).toBe('deploy');
    pick(power);
    controls.setMode('repair');
    expect(controls.cursor).toBe('repair-blocked');
    power.hp--;
    expect(controls.cursor).toBe('repair');
    controls.setMode('sell');
    expect(controls.cursor).toBe('sell');
    pick(tank);
    expect(controls.cursor).toBe('sell-blocked');
    expect(move).not.toHaveBeenCalled();
    key('Escape');
    pointer('pointerleave');
    expect(callbacks.cursor).toHaveBeenLastCalledWith('default');
  });
  it('shows eight-direction scrolling and suppresses gameplay cursors behind dialogs and the source gate', () => {
    const { controls, camera, pointer, settings, enable } = fixture();
    camera.center(30, 30);
    pointer('pointermove', 1, 1);
    expect(controls.cursor).toBe('scroll-nw');
    camera.center(-3, -3);
    expect(controls.cursor).toBe('scroll-nw-blocked');
    settings(true);
    expect(controls.cursor).toBe('default');
    settings(false);
    enable(false);

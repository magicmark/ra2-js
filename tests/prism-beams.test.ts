...
function arena(type = 'prism_tank') {
  ...
  const attacker = spawn(type, 0, 20.5, 30.5),
    target = spawn('rhino', 1, 25.5, 30.5);
...
describe('Prism firing', () => {
  it('emits a 15-frame beam at the original SREF muzzle without changing instant damage, audio or reload', () => {
    ...
    frames(game, 14);
    expect(beams(game)).toContain(beam);
    frames(game, 1);
    expect(beams(game)).toEqual([]);
    expect(target.hp).toBe(950);
  });
  ...
  it('draws exactly the existing three half-damage branches, excluding allies, air, passengers and distant units', () => {
    ...
    const extras = [0, 1, 2, 3].map((i) => spawn('rhino', 1, 25.7 + i * 0.1, 31));

    const ally = spawn('rhino', 0, 25.5, 31),
      air = spawn('harrier', 1, 25.5, 31);

    const passenger = spawn('conscript', 1, 25.5, 31),
      far = spawn('rhino', 1, 28, 30.5);

    passenger.transportId = 999;

    for (const entity of [...extras, ally, air, passenger, far])
      Object.assign(entity, { hp: 1000, maxHp: 1000, cooldown: 999 });
    frames(game, 1);
    ...
    expect(fragments.every((e) => e.x === target.x && e.y === target.y && e.height === 0)).toBe(
      true,
    );
  ...
  it('keeps the firing snapshot when either tank moves or dies; pauses, expires and resets with the simulation', () => {
    ...
    frames(game, 30);
    expect(beam).toEqual(snapshot);
    game.state.paused = false;
    frames(game, 1);
    expect(beam.to).toEqual(snapshot.to);
    expect(beam.x).toBe(snapshot.x);
    frames(game, 14);
    expect(beams(game)).toEqual([]);
    game.restart();
  ...
  it('makes ground force-fire visible without damaging an unrelated target or inventing fragments', () => {
    const { game, attacker, target } = arena();
    game.orderForceFire([attacker.id], 24.5, 30.5);
    frames(game, 1);
    expect(beams(game)).toHaveLength(1);
    expect(beams(game)[0].to).toEqual({ x: 24.5, y: 30.5 });
  ...
  it('shows the shared Prism Tower weapon at its own original firing height', () => {
    ...
    game['fireProjectile'](
      ...
      target,
      120,
      game.defs.prism_tower.verses,
      2,
    );
    expect(beams(game)).toHaveLength(1);
    expect(beams(game)[0]).toMatchObject({ height: (378 / 256) * 30 + 4, life: 0.5 });
    expect(target.hp).toBe(880);
  });

  it.each(['grizzly', 'mirage_tank', 'ifv'])('leaves %s out of the Prism visual path', (type) => {
    const { game } = arena(type);
    frames(game, 1);
    expect(beams(game)).toEqual([]);
  });
});

...
describe('mobile contacts in fog of war', () => {
  it.each(['conscript', 'rhino', 'harrier', 'destroyer'])('uses the occupied cell for %s, not explored terrain or an adjacent visible cell', type => {
    const game = new Game({ ai: false });
    const enemy = (game as any).spawn(type, 1, 30.5, 30.5);
    enemy.revealed = true; // Old discovery state must never override current sight.
    game.state.fog.fill(0); game.state.explored.fill(1);
    ...
    expect(revealedEntity(game.state, game.defs, enemy)).toBe(false);
    delete enemy.transportId; enemy.hp = 0;
    expect(revealedEntity(game.state, game.defs, enemy)).toBe(false);
  });
  ...
  it('drops a moving enemy target and its command line when it leaves sight, and rejects new attacks until it returns', () => {
    ...
    const attacker = (game as any).spawn('grizzly', 0, 30.5, 30.5);
    const enemy = (game as any).spawn('engineer', 1, 34.5, 30.5);
    (game as any).rebuildBlocked(); (game as any).updateVision();
    ...
    enemy.x = 50.5; enemy.previous = { x: enemy.x, y: enemy.y };
    ...
    game.tick(1 / 30);
    expect(attacker.targetId).toBeNull(); expect(attacker.path).toEqual([]);
    expect(attacker.order).toBe('guard'); expect(attacker.firedAt).toBeUndefined();
    game.orderAttack([attacker.id], enemy.id);
    expect(attacker.targetId).toBeNull();
    enemy.x = 34.5; (game as any).updateVision();
    game.orderAttack([attacker.id], enemy.id);
    expect(attacker.targetId).toBe(enemy.id);
    expect(game.commandPath(attacker.id)).toBeDefined();
  });

...
function arena() {
  ...
  const spawn = (type: string, x: number, y: number, side = 0): Entity =>
    g['spawn'](type, side, x, y);
...
describe('Harrier airfield parking', () => {
  it('spawns four east-facing, landed aircraft on the four original pads and leaves them parked', () => {
    ...
    expect(planes.map((e) => [e.x, e.y])).toEqual([
      [21.5, 30.5],
      [21.5, 31.5],
      [22.5, 30.5],
      [22.5, 31.5],
  ...
  it('uses another airfield when the first is full, reserves pads during flight, and reuses a destroyed aircraft pad', () => {
    ...
    const planes = Array.from({ length: 4 }, build),
      second = spawn('radar', 28, 30);

    g.orderMove([planes[0].id], 18.5, 35.5);
    advance(g, 5);
  ...
  it('returns four empty aircraft to their own exact pads, rearms, and stays landed', () => {
    const { g, build } = arena(),
      planes = Array.from({ length: 4 }, build);
  ...
  it('takes off for an attack and resumes its target after rearming at its reserved pad', () => {
    const { g, build, spawn } = arena(),
      plane = build();

    const target = spawn('power_soviet', 26, 35, 1);
    target.hp = target.maxHp = 10000;
    g.orderAttack([plane.id], target.id);
    advance(g, 0.1);
  ...
  it('reassigns returning aircraft to separate free pads when their home is destroyed', () => {
    ...
    for (const plane of planes)
      expect(plane).toMatchObject({ homeId: second.id, landed: true, ammo: 1 });
  });
  ...
  it('preserves parking and retreat orders while its airfield is temporarily disabled', () => {
    ...
    g.orderMove([flying.id], 24.5, 35.5);
    advance(g, 5);
    parked.ammo = 0;
    advance(g, 2);
    ...
    flying.ammo = 0;
    g.orderMove([flying.id], 25.5, 35.5);
    advance(g, 6);
    ...
    expect(flying.x).toBeCloseTo(25.5);
    expect(flying.y).toBeCloseTo(35.5);
    home.disabledUntil = undefined;
    advance(g, 4);

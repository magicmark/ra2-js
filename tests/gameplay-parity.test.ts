...
function arena() {
  ...
  game.state.entities = game.state.entities.filter(
    (e) =>
      e.type === 'conyard' || e.type === 'gi' || e.type === 'grizzly' || e.type === 'power_soviet',
  );

  const gi = game.state.entities.find((e) => e.type === 'gi')!,
  ...
  const scout = game.state.entities.find((e) => e.type === 'conyard' && e.side === 0)!;
  scout.x = 22;
  scout.y = 23;
...
function advance(game: Game, seconds: number, hz = 30) {
  for (let i = 0; i < Math.round(seconds * hz); i++) game.tick(1 / hz);
}

describe('individual TOML definition files', () => {
  it('loads one named entity per file and resolves prerequisites after merging', () => {
    const files = import.meta.glob<string>('../src/data/units/*.toml', {
      query: '?raw',
      import: 'default',
      eager: true,
    });
    ...
    expect(
      Object.values(definitions).filter(
        (def) => !def.mapOnly && !CUSTOM_UNIT_IDS.some((id) => id === def.id),
      ),
    ).toHaveLength(48);
    ...
    expect(Object.keys(definitions).sort()).toEqual(
      [...Object.keys(nativeIds), ...CUSTOM_UNIT_IDS].sort(),
    );
    expect(
      Object.values(definitions)
        .filter((def) => def.mapOnly)
        .map((def) => def.id)
        .sort(),
    ).toEqual(['hornet', 'tech_airport', 'tech_oil']);

    const tiny = {
      'first.toml':
        '[units.first]\nname="First"\ncategory="structures"\ncost=10\nrequires=["second"]',
      'second.toml': '[units.second]\nname="Second"\ncategory="infantry"\ncost=20',
    };

    expect(parseDefinitionFiles(tiny).first.requires).toEqual(['second']);
    expect(() => parseDefinitionFiles({ 'first.toml': tiny['first.toml'] })).toThrow(
      'unknown unit second',
    );
    expect(() => parseDefinitionFiles({ 'other.toml': tiny['second.toml'] })).toThrow(
      'must contain only',
    );
    expect(() =>
      parseDefinitionFiles({ ...tiny, 'duplicate/first.toml': tiny['first.toml'] }),
    ).toThrow('Duplicate');
  });
});
...
describe('functional retail unit commands', () => {
  it('expires shared command feedback after 25 logic frames, and refreshes it on orders and selection', () => {
    ...
    game.select([tank.id]);
    advance(game, 24 / 30);
    expect(game.commandPath(tank.id)).toBeDefined();
    game.tick(1 / 30);
    expect(tank.selected).toBe(true);
    expect(tank.path.length).toBeGreaterThan(0);
    expect(game.commandPath(tank.id)).toBeUndefined();
    game.select([tank.id]);
    expect(game.commandPath(tank.id)).toBeDefined();
    ...
    advance(game, 25 / 30);
    expect(game.commandPath(tank.id)).toBeUndefined();
    game.orderMove([gi.id], 20.5, 36.5);
    expect(game.commandPath(tank.id)).toBeDefined(); // A shared timer, not an independent lifetime per unit.
    game.restart();
    expect(game.commandPath(tank.id)).toBeUndefined();
  });
  ...
  it('reports command goals and queued waypoints without exposing navigation cells or stale stopped orders', () => {
    ...
    expect(game.commandPath(tank.id)).toEqual({
      points: [
        { x: 15.5, y: 34.5 },
        { x: 19.5, y: 34.5 },
      ],
    ...
    game.orderGuard([tank.id], gi.x, gi.y, gi.id);
    gi.x += 0.25;
    ...
    game.orderMove([yard.id], 27, 37);
    expect(game.commandPath(yard.id)).toEqual({ points: [{ x: 27, y: 37 }], attack: false });
  ...
  it('lets opposing vehicles commit to clear detours without overlap or permanent oscillation', () => {
    ...
    gi.x = 25.5;
    gi.y = 35.5;
    gi.previous = { x: gi.x, y: gi.y };
    gi.facing = Math.PI;
    game.orderMove([tank.id], 25.5, 35.5);
    game.orderMove([gi.id], 19.5, 35.5);

    for (let frame = 0; frame < 360; frame++) {
      game.tick(1 / 30);
      expect(Math.hypot(tank.x - gi.x, tank.y - gi.y)).toBeGreaterThanOrEqual(0.47);
    }

    expect(tank.x).toBeGreaterThan(24);
    expect(gi.x).toBeLessThan(21);
  });
  it('turns before departing a stopped traffic reroute, including the observed blocked reversal', () => {
    ...
    const specifications = [
      { type: 'grizzly', x: 19.5, y: 35.5, facing: 0 },
      { type: 'grizzly', x: 25.5, y: 35.5, facing: Math.PI },
      { type: 'grizzly', x: 22.5, y: 35.5, facing: 0 },
      { type: 'gi', x: 21.5, y: 33.5, facing: Math.PI / 2 },
      { type: 'gi', x: 22.5, y: 33.5, facing: Math.PI / 2 },
      { type: 'gi', x: 23.5, y: 33.5, facing: Math.PI / 2 },
    ];

    const units = specifications.map((specification, index) => ({
      ...(specification.type === 'gi' ? gi : tank),
      ...specification,
      id: 100 + index,
      path: [],
      previous: { x: specification.x, y: specification.y },
    ...
    game.orderMove(
      units.slice(3).map((e) => e.id),
      22.5,
      38.5,
    );
    const stopped = [0, 0];
    let alignedDepartures = 0;

    for (let frame = 0; frame < 600; frame++) {
      const before = units.slice(0, 2).map((e) => ({ x: e.x, y: e.y }));
      game.tick(1 / 30);

      for (let i = 0; i < 2; i++) {
        const dx = units[i].x - before[i].x,
          dy = units[i].y - before[i].y;
        ...
        if (stopped[i] >= 5) {
          ...
          expect(Math.abs(Math.atan2(Math.sin(mismatch), Math.cos(mismatch)))).toBeLessThan(
            Math.PI / 36,
          );
          alignedDepartures++;
        }
  ...
  it('force move crushes an enemy infantry destination while escort follows fractional mobile coordinates safely', () => {
    ...
    crush.target.y = 10;
    crush.gi.side = 1;
    crush.game.orderForceMove([crush.tank.id], crush.gi.x, crush.gi.y);
    advance(crush.game, 4);
    ...
    escort.game.orderMove([escort.gi.id], 24.5, 34.5);
    advance(escort.game, 5);
  ...
  it('deploys a GI with authored range, weapon armor response and rate, then undeploys on a move order', () => {
    const { game, gi, target } = arena();
    target.x = 25; // 4.5 cells from the GI, beyond its ordinary M60.
    game.stop([gi.id]);
    advance(game, 0.3);
    ...
    expect(gi.x).toBe(20.5);
    expect(target.hp).toBeLessThan(10000);
    expect(gi.cooldown).toBeLessThanOrEqual(definitions.gi.deployedFireRate!);
    game.orderMove([gi.id], 20.5, 34.5);
    expect(gi.deployed).toBe(false);
    advance(game, 1);
    expect(gi.y).toBeGreaterThan(30.5);
  });
  it('stop holds position while guard can pursue enemies in sight', () => {
    ...
    game.stop([gi.id]);
    advance(game, 0.5);
    expect(gi.path).toHaveLength(0);
    expect(gi.x).toBe(20.5);
    game.guard([gi.id]);
    advance(game, 0.5);
    expect(gi.x).toBeGreaterThan(20.5);
  });
  it('scatter separates the selected units and does not command enemy units', () => {
    ...
    game.scatter([gi.id, tank.id, target.id]);
    advance(game, 1);
    expect(Math.hypot(gi.x - 20.5, gi.y - 30.5)).toBeGreaterThan(0.5);
  ...
  it('force fire damages friendly targets, including a ground tile under a friendly unit', () => {
    const { game, gi, tank } = arena();
    tank.x = 19.5;
    game.orderAttack([tank.id], gi.id);
    expect(tank.targetId).toBeNull();
    game.orderAttack([tank.id], gi.id, true);
    advance(game, 0.5);
    expect(gi.hp).toBeLessThan(gi.maxHp);
    const hp = gi.hp;
    tank.cooldown = 0;
    game.orderForceFire([tank.id], gi.x, gi.y);
    advance(game, 0.1);
    expect(gi.hp).toBeLessThan(hp);
    expect(game.state.sides[0].kills).toBe(0);
  });
  it('executes multiple waypoints and clears the remaining route with stop', () => {
    const { game, tank, target } = arena();
    target.x = 50;
    target.y = 10;
    game.orderWaypoints(
      [tank.id],
    ...
    advance(game, 6);
    expect(tank.x).toBeCloseTo(19.5);
    expect(tank.y).toBeCloseTo(34.5);
    game.orderWaypoints(
      [tank.id],
      [
        { x: 19.5, y: 36.5 },
    ...
    advance(game, 0.2);
    game.stop([tank.id]);
    const point = { x: tank.x, y: tank.y };
    advance(game, 6);
    expect({ x: tank.x, y: tank.y }).toEqual(point);
  });
});
...
describe('simulation frame and speed independence', () => {
  it('uses the supplied executable integer Speed conversion for ground infantry and vehicles', () => {
    ...
    game.orderMove([gi.id], 20.5, 39.5);
    game.orderMove([tank.id], 15.5, 39.5);
    advance(game, 1);
  ...
  it('provides stable interpolation samples through fractional frames, catch-up and pause', () => {
    ...
    expect(tank.x).toBe(start.x);
    expect(tank.y).toBe(start.y);
    game.tick(1 / 60);
    expect(game.interpolation).toBeCloseTo(0);
    expect(tank.previous).toEqual(start);
    expect(tank.y).toBeGreaterThan(start.y);
    game.tick(0.15);
    expect(game.interpolation).toBeCloseTo(0.5);
    expect(tank.previous!.y).toBeLessThan(tank.y);
    ...
    game.state.paused = true;
    game.tick(0.2);
    expect(tank.previous).toBe(previous);
    expect(game.interpolation).toBe(alpha);
  });
  it('preserves constant speed across path-cell boundaries without a braking pulse each tile', () => {
    ...
    for (let frame = 0; frame < 60; frame++) {
      ...
      game.tick(1 / 30);
      expect(Math.hypot(tank.x - before.x, tank.y - before.y)).toBeCloseTo(
        game.defs[tank.type].speed / 30,
        8,
      );
    }
  });
  it('produces equal time, movement and construction for 5/10/30/60/144Hz visible frames', () => {
    const results = [5, 10, 30, 60, 144].map((hz) => {
      ...
      game.orderMove([tank.id], 22.5, 36.5);
      advance(game, 3, hz);
  ...
  it('scales the same fixed steps at .5x/1x/2x while paused and extreme deltas do not fast-forward', () => {
    ...
    game.state.paused = false;
    game.tick(500);
    expect(game.state.time).toBeLessThanOrEqual(0.25);

    for (const dt of [NaN, Infinity, -1]) game.tick(dt);
    expect(game.state.time).toBeLessThanOrEqual(0.25);
  });
  it('uses integer authored ROF frames plus the native zero-to-two-frame rearm jitter', () => {
    const { game, tank, target, gi } = arena();
    game.state.entities = game.state.entities.filter((e) => e.id !== gi.id);
    tank.x = 20.5;
    tank.y = 30.5;
    ...
    expect(10000 - target.hp).toBeCloseTo(3 * 65 * 0.65, 8);
  });
});
...
const nativeIds = {
  ...
  miner: 'CMIN',
  power_soviet: 'NAPOWR',
  refinery_soviet: 'NAREFN',
  barracks_soviet: 'NAHAND',
  warfactory_soviet: 'NAWEAP',
  radar_soviet: 'NARADR',
  sentry: 'NALASR',
  conscript: 'E2',
  rhino: 'HTNK',
...
function ini(source: string) {
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    else if (section && clean.includes('=')) {
      const [key, ...value] = clean.split('=');
      section[key.trim().toLowerCase()] = value.join('=').trim();
    }
  }
...
describe.skipIf(!process.env.RA2_ASSET_DIR)(
  'data verified against original Westwood rules/art',
  () => {
    it("applies the original ROF multiplier at each roster unit's authored rank", () => {
      ...
      for (const [type, name] of Object.entries(nativeIds)) {
        ...
        for (const rank of [0, 1, 2] as const) {
          ...
          const ranked = entity({ type, rank });
          expect(rankFireRateMultiplier(ranked), `${type} rank ${rank}`).toBe(
            enabled ? Number(rules.GENERAL.veteranrof) : 1,
          );
        }
      }
    });
    it('matches the four original airfield docks and aircraft parking direction', () => {
      ...
      for (const [index, offset] of AIRFIELD_DOCKING_OFFSETS.entries()) {
        expect([offset.x * 256, offset.y * 256, 0]).toEqual(
          art.GAAIRC[`dockingoffset${index}`].split(',').map(Number),
        );
      }
    ...
    it('matches all authored roster stats, foundations, costs, weapons and frame-based build/attack timings', () => {
      const rules = ini(readFileSync(join(process.env.RA2_ASSET_DIR!, 'rules.ini'), 'utf8'));
      const art = ini(readFileSync(join(process.env.RA2_ASSET_DIR!, 'art.ini'), 'utf8'));
      expect(Object.keys(nativeIds).sort()).toEqual(
        Object.keys(definitions)
          .filter((id) => !CUSTOM_UNIT_IDS.some((customId) => customId === id))
          .sort(),
      );
      ...
      for (const [id, name] of Object.entries(nativeIds)) {
        ...
        expect(def.hp, id).toBe(Number(source.strength));
        expect(def.armor, id).toBe(source.armor);
        ...
        if (def.baseNormal !== undefined)
          expect(def.baseNormal, id).toBe((source.basenormal ?? 'yes') === 'yes');
        ...
        if (def.mapOnly && isBuilding(def)) {
          ...
          expect(source.techlevel, id).toBe('-1');
          expect(source.capturable, id).toBe('yes');
          expect(source.needsengineer, id).toBe('yes');
          expect(source.unsellable, id).toBe('yes');
          expect(def.cost, id).toBe(Number(source.cost ?? 0));
          expect(isBuilding(def), id).toBe(true);
          expect(def.damage, id).toBe(0);
          expect(def.speed, id).toBe(0);
          expect(def.producer, id).toBeUndefined();

          if (source.sight !== undefined) expect(def.sight, id).toBe(Number(source.sight));
        } else {
          expect(def.cost, id).toBe(Number(source.cost));
          expect(def.sight, id).toBe(Number(source.sight));
          expect(def.buildTime, id).toBeCloseTo(
            ((Number(source.cost) / 1000) * Number(rules.GENERAL.buildspeed) * 900) / 30,
            8,
          );
        }

        if (isBuilding(def))
          expect(def.footprint, id).toEqual(
            art[(source.image ?? name).toUpperCase()].foundation.split('x').map(Number),
          );
        else {
          expect(def.nativeSpeed, id).toBe(Number(source.speed));

          if (id !== 'rocketeer')
            expect(def.speed, id).toBe(
              (Math.min(255, Math.trunc((Math.min(100, Number(source.speed)) * 256) / 100)) * 30) /
                256,
            );

          if (source.rot) expect(def.rot, id).toBe(Number(source.rot));
        }

        if (def.damage > 0) {
          const weapon = rules[(source.primary ?? source.weapon1).toUpperCase()];
          expect(def.damage, id).toBe(Number(weapon.damage));
          expect(def.range, id).toBe(Number(weapon.range));
          expect(def.fireRate, id).toBeCloseTo(Number(weapon.rof) / 30, 8);
          // Requested Mirage balance: 50% more damage to infantry armor, with all other native verses retained.
          expect(def.verses, id).toEqual(
            rules[weapon.warhead.toUpperCase()].verses
              ?.split(',')
              .map(
                (n, index) =>
                  (Number(n.trim().replace('%', '')) *
                    (id === 'mirage_tank' && index < 3 ? 1.5 : 1)) /
                  100,
              ),
          );
        }

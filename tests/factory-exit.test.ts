...
const advance = (game: Game, seconds: number) => {
  for (let n = 0; n < Math.round(seconds * 30); n++) game.tick(1 / 30);
};
...
function arena(soviet = false) {
  ...
  const side = soviet ? 1 : 0,
    suffix = soviet ? '_soviet' : '';

  game.state.sides[side].money = 1_000_000;
  const spawn = (type: string, x: number, y: number): Entity => game['spawn'](type, side, x, y);
  spawn(`refinery${suffix}`, 20, 38);
  spawn(`barracks${suffix}`, 25, 38);
  spawn(`power${suffix}`, 20, 42);
  const factory = spawn(`warfactory${suffix}`, 20, 30);
  factory.rally = undefined;
  game['rebuildBlocked']();
  ...
  const queue = (type = soviet ? 'rhino' : 'grizzly') => {
    ...
    item.progress = 1;
    item.spent = game.defs[type].cost;
...
describe('War Factory vehicle exit', () => {
  for (const soviet of [false, true])
    it(`${soviet ? 'Soviet' : 'Allied'} vehicles start concealed inside and drive continuously through the +X doorway`, () => {
      const { game, factory, queue, produced } = arena(soviet),
        lane = factoryExitLane(factory);
      ...
      while (unit.factoryExit) {
        ...
        expect(unit.y).toBe(lane.end.y);
        previous = unit.x;
      }

      expect(unit.x).toBe(lane.end.x);
      advance(game, 5);
      expect(unit.x).toBeGreaterThan(lane.end.x + 1);
    });

  it('serializes queued vehicles, clears the mouth without a rally, and preserves paid blocked production', () => {
    const { game, queue, produced, side } = arena();
    queue();
    const waiting = queue();
    advance(game, 1 / 30);

    const first = produced()[0],
      money = game.state.sides[side].money;

    advance(game, 0.5);
    expect(produced()).toEqual([first]);
    expect(game.state.sides[side].queues.vehicles).toEqual([waiting]);
    expect(waiting.progress).toBe(1);
    expect(game.state.sides[side].money).toBe(money);
    advance(game, 8);
    ...
    expect(Math.hypot(tanks[0].x - tanks[1].x, tanks[0].y - tanks[1].y)).toBeGreaterThan(0.48);
  });

  it('holds a finished unit when the actual exit is blocked, then emerges normally when cleared', () => {
    const { game, factory, queue, produced, spawn, side } = arena(),
      end = factoryExitLane(factory).end;

    const blocker = spawn('grizzly', end.x, end.y);
    const item = queue();
    advance(game, 1);
    expect(produced()).toEqual([]);
    expect(game.state.sides[side].queues.vehicles).toEqual([item]);
    blocker.x += 3;
    advance(game, 1 / 30);
  ...
  it('uses another owned factory when the first doorway is blocked', () => {
    const { game, factory, queue, spawn } = arena();
    const end = factoryExitLane(factory).end;
    spawn('grizzly', end.x, end.y);
    const second = spawn('warfactory', 30, 30);
    game['rebuildBlocked']();
    queue();
    advance(game, 1 / 30);
  ...
  it('stops for an obstruction introduced during departure and never opens the foundation to ordinary units', () => {
    const { game, factory, queue, produced, spawn } = arena();
    queue();
    advance(game, 1 / 30);

    const unit = produced()[0],
      blocker = spawn('grizzly', factory.x + 4, factory.y + 1);

    advance(game, 1);
    const x = unit.x;
    advance(game, 1);
    expect(unit.x).toBe(x);
    expect(unit.factoryExit).toBeDefined();
    expect(game['isPassable'](factory.x + 4, factory.y + 1, blocker)).toBe(false);
    blocker.x += 5;
    advance(game, 5);
    expect(unit.factoryExit).toBeUndefined();
  });

  it('finishes the exit before obeying its rally and cannot be stopped or deployed inside', () => {
    const { game, factory, queue, produced, spawn } = arena();
    factory.rally = { x: 30.5, y: 36.5 };
    spawn('service_depot', 30, 40);
    game['rebuildBlocked']();
    queue('mcv');
    advance(game, 1 / 30);
    const unit = produced()[0];
    ...
    game.stop([unit.id]);
    game.orderMove([unit.id], 10, 10);
    game.deploy([unit.id]);
    expect(unit.factoryExit).toBeDefined();
    expect(unit.order).toBe('move');
    advance(game, 20);
    expect(unit.factoryExit).toBeUndefined();
    expect(unit.x).toBeCloseTo(30.5);
    expect(unit.y).toBeCloseTo(36.5);
  });

  it('hands produced miners to harvesting only after they leave the factory', () => {
    const { game, factory, queue, produced } = arena();
    factory.rally = { x: 50.5, y: 50.5 };
    game.state.tiles[34 * game.state.width + 29].ore = 500;
    queue('miner');
    advance(game, 1 / 30);
    const unit = produced()[0];
    expect(unit.order).toBe('move');
    expect(unit.cargo).toBe(0);
    let harvested = false;

    for (let i = 0; i < 450; i++) {
      advance(game, 1 / 30);

      if (unit.cargo > 0) harvested = true;
    }
  ...
  it('releases the factory lane when Chronosphere relocates a departing vehicle', () => {
    ...
    const sphere = spawn('chronosphere', 30, 40);
    spawn('power', 35, 40);
    spawn('power', 38, 40);
    game['rebuildBlocked']();
    queue();
    advance(game, 1 / 30);
    const unit = produced()[0];
    sphere.recharge = game.defs.chronosphere.recharge;
    expect(game.activateSuperweapon('chronosphere', { x: 40.5, y: 45.5 }, unit)).toBe(true);
    expect(unit.factoryExit).toBeUndefined();
    advance(game, 5);
    expect(unit.x).toBe(40.5);
    expect(unit.y).toBe(45.5);
  });

  it('keeps a visibly departing Mirage targetable until it stops outside', () => {
    const { game, queue, produced, spawn } = arena();
    spawn('battlelab', 30, 40);
    queue('mirage_tank');
    advance(game, 1 / 30);
    const unit = produced()[0];
    const enemy = game['spawn']('rhino', 1, unit.x + 4, unit.y);
    expect(game['canTarget'](enemy, unit)).toBe(true);
    unit.factoryExit = undefined;
    expect(game['canTarget'](enemy, unit)).toBe(false);
  });
  ...
  it('pauses the departure, scales it with game speed, and completes after factory sale or destruction', () => {
    for (const removal of ['sale', 'destroy'] as const) {
      ...
      if (removal === 'sale') expect(game.sell(factory.id)).toBe(true);
      else factory.hp = 0;
      advance(game, 5);
      expect(unit.factoryExit).toBeUndefined();
      expect(unit.x).toBeGreaterThan(factory.x + 5);
    }
  });
});

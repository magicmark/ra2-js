...
function congestion(type: 'gi' | 'grizzly', corridor: boolean) {
  ...
  game.state.entities = game.state.entities.filter(e => isBuilding(game.defs[e.type]) || e === mover || e === blocker);
  for (let y = 32; y <= 38; y++) for (let x = 17; x <= 28; x++) {
    const tile = game.state.tiles[y * game.state.width + x];
    tile.terrain = corridor && y !== 35 ? 'rock' : 'grass'; tile.ore = 0;
  }
  ...
  if (corridor) {
    ...
    game.state.entities.push(second); blockers.push(second);
  }
...
describe('allied congestion', () => {
  it.each(['gi', 'grizzly'] as const)('%s waits without flipping in a blocked corridor and resumes when its ally moves', type => {
    const { game, mover, blockers } = congestion(type, true);
    let turns = 0;
    for (let frame = 0; frame < 360; frame++) {
      const before = mover.facing;
      game.tick(1 / 30);
      for (const blocker of blockers) expect(Math.hypot(mover.x - blocker.x, mover.y - blocker.y)).toBeGreaterThanOrEqual(.48 - 1e-8);
      if (frame >= 120) turns += Math.abs(Math.atan2(Math.sin(mover.facing - before), Math.cos(mover.facing - before)));
    }
    expect(mover.x).toBeLessThan(blockers[0].x);
    expect(mover.order).toBe('move');
    expect(turns).toBeLessThan(.01);
  ...
  it.each(['gi', 'grizzly'] as const)('%s routes around a stationary ally without repeated direction reversals', type => {
    ...
    for (let frame = 0; frame < 600; frame++) {
      ...
      game.tick(1 / 30);
      for (const blocker of blockers) expect(Math.hypot(mover.x - blocker.x, mover.y - blocker.y)).toBeGreaterThanOrEqual(.48 - 1e-8);
      const turn = Math.abs(Math.atan2(Math.sin(mover.facing - before), Math.cos(mover.facing - before)));

...
function fixture() {
  const sample = { sampleRate: 22050, channels: [new Float32Array([0, 0.5, -0.5, 0])] };

  const bank: OriginalSounds = new Map(
    [
      'MenuClick',
      'MenuTab',
      'CommandBar',
      ...
      {
        samples: [sample],
        volume: name === 'SniperSelect' ? 0.9 : 0.6,
        speech: name.startsWith('EVA_'),
      },
    ]),
  );
...
it('voices Sniper selection through effects settings while orders retain their command cue', async () => {
  const { audio, context, gains, sources } = fixture(),
    played = vi.spyOn(audio, 'play');

  audio.effectsVolume = 5;
  audio.musicVolume = 0;
  await audio.acknowledge(['gi', 'sniper', 'sniper']);
  expect(played).toHaveBeenLastCalledWith('SniperSelect');
  expect(sources).toHaveLength(1);
  expect(gains[0].gain.setValueAtTime).toHaveBeenLastCalledWith(0.5, 1);
  expect(gains[1].gain.setValueAtTime).toHaveBeenLastCalledWith(0.9, 1);
  context.currentTime++;
  audio.effectsVolume = 0;
  await audio.acknowledge(['sniper']);
  audio.effectsVolume = 10;
  audio.enabled = false;
  await audio.acknowledge(['sniper']);
  expect(sources).toHaveLength(1);
  audio.enabled = true;
  ...
  context.currentTime++;
  await audio.acknowledge();
  expect(played).toHaveBeenLastCalledWith('CommandBar');
  context.currentTime++;
  await audio.acknowledge(['gi']);
  expect(played).toHaveBeenLastCalledWith('CommandBar');
...
it('plays the original tab cue through effects settings, independently of music volume', async () => {
  ...
  audio.effectsVolume = 5;
  audio.musicVolume = 0;
  await audio.tabChanged();
  expect(played).toHaveBeenLastCalledWith('MenuTab');
  expect(sources).toHaveLength(1);
  expect(gains[0].gain.setValueAtTime).toHaveBeenLastCalledWith(0.5, 1);
  expect(gains[1].gain.setValueAtTime).toHaveBeenLastCalledWith(0.6, 1);
  context.currentTime++;
  audio.effectsVolume = 0;
  await audio.tabChanged();
  audio.effectsVolume = 10;
...
it('uses original sample buffers and authored volume; master volume and mute affect active playback', async () => {
  ...
  expect(context.createBuffer).toHaveBeenCalledWith(1, 4, 22050);
  expect(gains[0].gain.setValueAtTime).toHaveBeenLastCalledWith(0.5, 1);
  expect(gains[1].gain.setValueAtTime).toHaveBeenLastCalledWith(0.6, 1);
  audio.effectsVolume = 2;
  expect(gains[0].gain.setValueAtTime).toHaveBeenLastCalledWith(0.2, 1);
  audio.enabled = false;
...
it('plays weapon-specific effects once and suppresses unseen effects and burst overload', async () => {
  ...
  const effect = (sourceType: string): Effect => ({
    kind: 'shot',
    sourceType,
    x: 1,
    y: 1,
  ...
  const first = effect('gi');
  audio.effects([first], () => true);
  audio.effects([first], () => true);
  expect(played).toHaveBeenCalledTimes(1);
  expect(played).toHaveBeenLastCalledWith('GIAttack');
  audio.effects([effect('gi')], () => true);
  expect(sources).toHaveLength(1);
  audio.effects([{ ...effect('gi'), deployed: true }, effect('grizzly')], () => true);
  expect(sources).toHaveLength(3);
  expect(played).toHaveBeenLastCalledWith('GrizzlyTankAttack');
  const hidden = effect('grizzly');
  audio.effects([hidden], () => false);
  context.currentTime++;
  audio.effects([hidden], () => true);
  expect(sources).toHaveLength(3);
...
it('queues simultaneous EVA notifications once, prevents overlapping speech, and stops sounds on reset', async () => {
  ...
  const event = (sound?: string): GameEvent => ({
    id: 1,
    text: 'Ready',
    kind: 'success',
    time: 0,
  ...
  expect(played).toHaveBeenLastCalledWith('MenuClick');
  audio.reset();
  expect(sources.slice(1).every((source) => source.stop.mock.calls.length > 0)).toBe(true);
...
it('uses the simulated IFV passenger weapon instead of missile audio', async () => {
  ...
  for (const [passengerType, sound] of [
    ['gi', 'IFVAttackGround'],
    ['spy', 'IFVAttackGround'],
    ['tanya', 'SealAttack'],
    ['sniper', 'SniperAttack'],
    ...
    audio.effects(
      [{ kind: 'shot', sourceType: 'ifv', passengerType, x: 1, y: 1, life: 0.16, maxLife: 0.16 }],
      () => true,
    );
...
it.each(['mute', 'volume'])(
  'drops queued EVA messages on %s without replaying them later',
  async (mode) => {
    const { audio, sources, context } = fixture();
    await audio.unlock();
    audio.play('EVA_UnitReady');
    audio.play('EVA_ConstructionComplete');

    if (mode === 'mute') audio.enabled = false;

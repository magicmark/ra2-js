...
The Game Files entry is an explicit browser extension to access the preserved URL/Enter/import flow. It occupies the otherwise unused Load row; it does not claim native saved-game loading. Native Load/Save/Delete controls are absent because saved-game persistence is not implemented. Abort ends the skirmish and returns to the same ready-cache Continue gate without downloading. Music volume is implemented with original gameplay playback (see ORIGINAL_AUDIO.md). Separate voice volume and soundtrack playlist controls remain absent. Show Hidden Objects remains absent until its obscured-object behavior exists; it must never reveal shrouded units.
